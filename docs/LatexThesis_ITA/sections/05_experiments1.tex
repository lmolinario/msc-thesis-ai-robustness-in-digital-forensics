...
La Tabella~\ref{tab:results-comparative-robustness-profile} estende il confronto
precedente concentrandosi sulla robustezza operativa rispetto alle due famiglie
di manipolazione considerate: trasformazioni anti-forensi e perturbazioni
adversariali. Per ciascun sistema vengono riportati l'andamento aggregato sotto
trasformazioni anti-forensi, l'andamento aggregato sotto perturbazioni
adversariali, il caso più critico osservato in ciascuna famiglia e una sintesi
del profilo operativo. Il confronto non deve essere interpretato come ranking
assoluto, ma come lettura comparativa dei diversi profili di rischio introdotti
dai sistemi valutati.
...
\caption{Profilo comparativo di robustezza operativa sotto trasformazioni anti-forensi e perturbazioni adversariali.}
\label{tab:results-comparative-robustness-profile}
\begin{tabularx}{\textwidth}{@{}p{2.45cm} p{1.45cm} p{1.45cm} p{2.75cm} p{2.75cm} X@{}}
\toprule
\textbf{Sistema} &
\textbf{Anti acc./drop} &
\textbf{Adv acc./drop} &
\textbf{Caso anti-forensic più critico} &
\textbf{Caso adversarial più critico} &
\textbf{Profilo operativo} \\
\midrule
...
0.973 / 0.005 &
0.695 / 0.283 &
Gaussian blur: drop 0.013; 17 W$\rightarrow$NW &
Sigma Zero: accuracy 0.015; drop 0.963; 489 W$\rightarrow$NW &
Baseline clean più elevata tra i proxy, ma vulnerabilità estrema nello scenario
white-box contro Sigma Zero e degradazione rilevante sotto FGSM. \\

ResNet18 &
0.957 / 0.007 &
0.955 / 0.009 &
Histogram modification: drop 0.030; 23 W$\rightarrow$NW &
FGSM: drop 0.046; 27 W$\rightarrow$NW &
Profilo più stabile rispetto a EfficientNet-B0 per gli attacchi generati contro
il target white-box, ma con sensibilità anti-forense osservabile sulle modifiche
tonali. \\
...
FGSM: drop 0.013; 8 W$\rightarrow$NW &
Stabilità osservata elevata rispetto alle perturbazioni considerate, ma
comportamento OOD molto esposto all'assorbimento semantico verso
\texttt{weapon}. \\

Magnet AXIOM / Magnet.AI &
0.940 / 0.010 &
0.923 / 0.027 &
Gaussian blur: FNR 0.120; 60 FN &
FGSM: accuracy 0.867; FNR 0.192; 96 FN &
Comportamento relativamente conservativo, ma con aumento dei falsi negativi su
alcune perturbazioni, rilevante per il triage automatico. \\

Excire Foto 2025 \texttt{D50} &
0.941 / 0.003 &
0.904 / 0.040 &
Histogram modification: accuracy 0.932; FPR 0.088 &
Sigma Zero: accuracy 0.742; FPR 0.442 &
Profilo dipendente dalla soglia di retrieval: nella configurazione intermedia
riduce i falsi negativi, ma può produrre over-triggering semantico, soprattutto
su Sigma Zero. \\

Cellebrite Inseyets &
0.960 / 0.004 &
0.955 / 0.009 &
Histogram modification: accuracy 0.949; FNR 0.060 &
FGSM: accuracy 0.918; FNR 0.106 &
Profilo commerciale più stabile nel bundle considerato, senza degradazione
estrema su Sigma Zero, ma con sensibilità residua a FGSM e ad alcune
trasformazioni anti-forensi. \\
...
\vspace{0.4em}
\footnotesize
\textit{Nota.} Per i modelli proxy, i valori anti-forensic e adversarial sono
calcolati come media delle cinque condizioni sperimentali di ciascuna famiglia.
Per i sistemi commerciali, i valori riportano le metriche aggregate normalizzate
sulle rispettive famiglie di input. Il drop è definito come
\(\text{accuracy}_{clean}-\text{accuracy}_{perturbed}\). Per Excire Foto 2025
viene riportata la configurazione \texttt{D50}, utilizzata come riferimento
comparativo principale. W$\rightarrow$NW indica transizioni da \texttt{weapon} a
\texttt{non\_weapon}; FN indica falsi negativi; FNR indica false negative rate;
FPR indica false positive rate.
\end{table}

La Tabella~\ref{tab:results-comparative-robustness-profile} evidenzia che la
robustezza operativa non coincide con la sola prestazione clean. EfficientNet-B0,
pur essendo il modello proxy con la migliore baseline nominale, mostra il profilo
più fragile nello scenario white-box sotto \gls{sigmazero}. ResNet18 e
\gls{clip} risultano più stabili rispetto agli attacchi generati contro
EfficientNet-B0, ma presentano criticità differenti: ResNet18 conserva una
maggiore confidenza sugli input \gls{ood}, mentre \gls{clip} tende ad assorbire
molti campioni fuori distribuzione nella classe \texttt{weapon}. Nel livello
commerciale, \gls{cellebriteinseyets} mostra il profilo più stabile nel bundle
considerato, \gls{magnetai} mantiene un comportamento relativamente conservativo
ma con falsi negativi non trascurabili, mentre \gls{excirefoto2025} evidenzia
una dipendenza marcata dalla soglia di retrieval e un rischio di over-triggering
semantico su alcune perturbazioni.




Le configurazioni \texttt{D20} e \texttt{D80} di \gls{excirefoto2025} non sono
incluse nella Tabella~\ref{tab:results-comparative-robustness-profile} come
valori da mediare con \texttt{D50}, poiché rappresentano profili operativi
differenti. \texttt{D20} descrive un uso restrittivo del semantic retrieval,
adatto a contenere falsi positivi e segnalazioni \gls{ood}, ma più esposto al
rischio di mancata segnalazione della classe \texttt{weapon}. \texttt{D80}
descrive invece un uso permissivo, utile quando l'obiettivo prioritario è
massimizzare il recall, ma associato a un aumento rilevante del rumore operativo
e del weapon flag rate sugli input \gls{ood}. La scelta di \texttt{D50} come
riferimento comparativo deriva quindi dal suo profilo intermedio: rispetto a
\texttt{D20}, riduce il rischio di falsi negativi; rispetto a \texttt{D80},
limita l'aumento dei falsi positivi e delle segnalazioni \gls{ood}.
