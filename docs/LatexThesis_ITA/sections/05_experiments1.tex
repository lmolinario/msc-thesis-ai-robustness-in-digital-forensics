...
trasformazione applicata. valori pari a zero indicano stabilità; valori negativi indicano
un lieve incremento dell'accuracy nella condizione perturbata. Tali incrementi,
quando presenti, non sono interpretati come prova di robustezza generale, ma come
oscillazioni dipendenti dalla distribuzione dei campioni e dalla specifica
...
originale con il valore ottenuto escludendo i file con sensitive hits. Per
\gls{excirefoto2025} viene riportata la configurazione \texttt{D50}, utilizzata
come riferimento principale nella discussione comparativa.
...
accuracy rimangono comprese tra -0.0004 e +0.0015, mentre le variazioni del
...
0.032 &
0.292 &
Migliore equilibrio commerciale nel clean set, con OOD weapon flag rate più
...
la clean accuracy più elevata tra i proxy, mentre Cellebrite Inseyets ottiene il
profilo clean più solido tra i tool commerciali. Tuttavia, il comportamento sugli
input \gls{ood} evidenzia una dimensione ulteriore del rischio. \gls{clip}
assegna la classe \texttt{weapon} alla maggioranza degli input \gls{ood}, con un
OOD weapon rate pari a 0.836. Questo comportamento non implica necessariamente
la produzione di evidenza forense affidabile, ma indica una forte attrazione
semantica verso il concetto di arma anche per immagini esterne al task binario.
...
\gls{excirefoto2025} in configurazione \texttt{D50} pari a 0.340 e
\gls{cellebriteinseyets} pari a 0.292. Questi risultati suggeriscono una risposta
...
Migliore profilo commerciale aggregato sotto trasformazioni anti-forensi, pur
...
\gls{cellebriteinseyets} ottiene la migliore accuracy anti-forense aggregata,
pari a 0.960, seguito da \gls{excirefoto2025} in configurazione \texttt{D50} e
da \gls{magnetai}. Tuttavia, l'interpretazione operativa varia da tool a tool.
\gls{magnetai} risulta più esposto ai falsi negativi sotto Gaussian blur.
\gls{excirefoto2025} mostra invece una criticità più legata all'aumento dei
falsi positivi sotto Histogram modification. \gls{cellebriteinseyets} mantiene
il profilo più stabile, ma anche in questo caso le modifiche tonali producono un
effetto misurabile sui falsi negativi. Questi risultati indicano che la
robustezza anti-forense deve essere valutata non solo attraverso l'accuracy
aggregata, ma anche attraverso la direzione degli errori e il loro impatto sulla
prioritizzazione investigativa.
...
53 FN &
Migliore stabilità commerciale aggregata anche nello scenario adversarial, ma
...
risulta sensibilmente influenzato da \gls{fgsm}, con FNR pari a 0.192. Dal punto
di vista forense, questo è un errore critico, perché la mancata segnalazione di
immagini \texttt{weapon} può portare alla deprioritizzazione di materiale
rilevante. \gls{excirefoto2025} in configurazione \texttt{D50} reagisce invece
in modo diverso: Sigma Zero produce soprattutto un aumento del false positive
rate, pari a 0.442, generando over-triggering semantico piuttosto che
occultamento sistematico della classe \texttt{weapon}. \gls{cellebriteinseyets}
mostra la migliore stabilità adversarial aggregata tra i tool commerciali, ma
\gls{fgsm} produce comunque un FNR pari a 0.106. Anche il sistema commerciale
più stabile rimane quindi sensibile ad alcune perturbazioni specifiche.
...
livello operativo, attraverso categorie osservabili, tag esportati, matching
rate, output non mappabili, falsi negativi, falsi positivi e comportamento sugli
input \gls{ood}. Questa limitazione non rappresenta una debolezza del protocollo
...
adottano meccanismi proprietari di classificazione, tagging o retrieval.
Secondo, i tool commerciali mostrano una stabilità aggregata superiore rispetto
al proxy più vulnerabile, ma presentano comunque failure mode operativamente
rilevanti, in particolare falsi negativi su immagini \texttt{weapon} sotto
\gls{fgsm} e falsi positivi in configurazioni permissive di semantic retrieval.
...
I risultati sperimentali hanno implicazioni operative rilevanti per l'impiego di
sistemi \gls{ai}-based nei workflow di \gls{df}. L'analisi mostra che la
prestazione nominale su input clean non è sufficiente per valutare
l'affidabilità di un sistema utilizzato per il triage forense di immagini. In un
contesto investigativo, infatti, assumono particolare importanza la direzione
degli errori, il comportamento su input fuori distribuzione, la sensibilità a
manipolazioni realistiche o deliberate e la possibilità di ricondurre ogni output
automatico all'artefatto sperimentale corrispondente.

\paragraph{Falsi negativi e rischio di mancata prioritizzazione.}

La prima implicazione riguarda i falsi negativi sulla classe
\texttt{weapon}. In un workflow di triage, un falso negativo è particolarmente
critico perché può ridurre la probabilità che un contenuto investigativamente
rilevante venga sottoposto a revisione umana. Questo rischio riguarda sia i
modelli proxy sia gli strumenti commerciali black-box. In condizioni clean,
EfficientNet-B0 produce 8 falsi negativi, ResNet18 ne produce 15 e \gls{clip}
ne produce 6. Tuttavia, nelle condizioni perturbate il rischio può aumentare in
modo sostanziale.

Il caso più evidente è EfficientNet-B0 sotto \gls{sigmazero}, dove l'accuracy
scende a 0.015 e vengono introdotte 489 transizioni
...
mostrano che una baseline clean elevata non consente, da sola, di dedurre
l'affidabilità operativa del sistema in presenza di input manipolati. Dal punto
di vista forense, la criticità principale non è soltanto la riduzione
dell'accuracy, ma il fatto che immagini appartenenti alla classe
investigativamente rilevante possano essere ricondotte alla classe negativa.

\paragraph{Input out-of-distribution e assorbimento operativo.}

La seconda implicazione riguarda gli input \gls{ood}. Le immagini fuori
distribuzione non devono essere valutate come una normale terza classe
supervisionata del task binario, ma come stress test della capacità del sistema
di gestire input ambigui, sintetici, borderline o semanticamente distanti dalla
distribuzione attesa. In un'indagine reale, tali contenuti possono comparire in
modo naturale all'interno di collezioni multimediali eterogenee e possono essere
sottoposti agli stessi strumenti automatici usati per il triage delle immagini
\texttt{weapon}/\texttt{non\_weapon}.

Il comportamento osservato nei modelli proxy evidenzia profili differenti.
EfficientNet-B0 riconduce alla classe \texttt{weapon} il 36.96\% delle
...
\texttt{D50} riconduce alla classe \texttt{weapon} il 34.00\% degli input
\gls{ood}, mentre \gls{cellebriteinseyets} presenta un weapon flag rate pari al
29.20\%.

Questi valori non rappresentano errori binari in senso stretto, poiché i
campioni \gls{ood} non appartengono alla ground truth
\texttt{weapon}/\texttt{non\_weapon}. Essi descrivono invece il rischio di
assorbimento operativo di contenuti fuori distribuzione entro una classe
investigativamente sensibile. Tale rischio può aumentare il carico di revisione
dell'analista, generare prioritizzazioni fuorvianti o attribuire rilevanza
operativa a contenuti che richiedono una valutazione contestuale più accurata.

\paragraph{Configurazioni operative e sistemi di semantic retrieval.}
...
scelta che modifica direttamente il profilo di rischio del workflow. La
configurazione \texttt{D20} riduce falsi positivi e segnalazioni \gls{ood}, ma
aumenta il rischio di falsi negativi. La configurazione \texttt{D80} massimizza
il recall sulla classe \texttt{weapon}, ma accresce il rumore operativo e il
carico di revisione umana. La configurazione \texttt{D50} rappresenta una
posizione intermedia nel contesto sperimentale considerato.

Questo risultato è particolarmente rilevante perché mostra che, nei sistemi di
retrieval, l'affidabilità operativa non dipende soltanto dal software utilizzato,
ma anche dalla configurazione adottata, documentata e mantenuta tracciabile nel
workflow. Due analisti che utilizzano lo stesso strumento con soglie differenti
possono ottenere profili di errore diversi, soprattutto in termini di falsi
positivi, falsi negativi e weapon flag rate sugli input \gls{ood}. Per questa
ragione, la configurazione operativa deve essere considerata parte integrante
della metodologia di valutazione e non un dettaglio accessorio.

\paragraph{Tracciabilità e auditabilità del processo.}
...
riproducibilità e la documentazione del processo assumono valore metodologico,
operativo e probatorio.

Il forensic evaluation bundle, composto da 11\,500 file, consente di collegare
ogni risultato ai corrispondenti artefatti tramite identificativi neutri e hash
\gls{sha256}/\gls{md5}. Questa impostazione riduce il rischio che la valutazione
dipenda da path, nomi di file o informazioni semantiche esposte ai tool. Inoltre,
la presenza di manifest separati consente di distinguere tra errori di
classificazione, mancati rilevamenti, output non interpretabili, limiti del
mapping operativo e problemi derivanti dai formati di export dei singoli
strumenti.

\paragraph{Limiti della valutazione black-box.}
...
dall'ambiente \texttt{Cellebrite Physical Analyzer 10.9.0.3029}.

La comparazione tra tali strumenti deve quindi essere interpretata come confronto
operativo di triage, non come confronto tra classificatori omogenei o
completamente ispezionabili. In altri termini, la valutazione non misura la
qualità interna dei modelli proprietari, ma il comportamento osservabile dei
sistemi quando vengono applicati allo stesso forensic evaluation bundle e quando
i loro output vengono normalizzati rispetto al task sperimentale.
...
uniforme del grado di certezza. Per questa ragione, metriche come AUROC non sono
state assunte come metrica centrale del confronto commerciale. La valutazione si
concentra invece su \gls{tp}, \gls{tn}, \gls{fp}, \gls{fn}, matching rate,
output non interpretabili, weapon recall, \gls{fnr}, \gls{fpr}, balanced
accuracy ove applicabile e weapon flag rate sui campioni \gls{ood}.

\paragraph{Limiti relativi ai metadati embedded.}
...
informazioni embedded nei file originari.

Questo aspetto è particolarmente rilevante in ambito forense, poiché alcuni
strumenti possono indicizzare informazioni non strettamente visuali, quali campi
EXIF, XMP, IPTC o metadati descrittivi residui. La presenza di tali informazioni
non invalida il protocollo sperimentale, ma richiede documentazione esplicita e
controlli di sensitività. In questa tesi, i casi individuati sono trattati come
limite operativo documentato della cecità del bundle e non come fattore
invalidante dei risultati, poiché non incidono sugli artefatti adversariali o
anti-forensi e producono variazioni trascurabili sulle metriche aggregate.

\paragraph{Limiti relativi alla selezione delle perturbazioni.}
...
robustezza operativa, non come benchmark completo di sicurezza del machine
learning.

In particolare, gli attacchi model-dependent sono generati rispetto a
...
valutata empiricamente, ma non assunta come proprietà generale. Questo limite è
metodologicamente rilevante: la severità osservata su EfficientNet-B0 sotto
\gls{sigmazero} non deve essere automaticamente estesa agli altri sistemi, così
come la maggiore stabilità apparente di alcuni tool commerciali non deve essere
interpretata come certificazione generale di robustezza adversarial.
...
operativa richiede quindi valutazioni esplicite su dati clean, input \gls{ood},
perturbazioni adversariali, trasformazioni anti-forensi e output commerciali
normalizzati, mantenendo sempre documentati limiti, configurazioni e assunzioni
