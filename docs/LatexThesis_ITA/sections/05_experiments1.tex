...
relativi \texttt{bundle\_id} sono stati incrociati con la ground truth, le
...
\texttt{non\_weapon}. Per gli strumenti black-box, \gls{axiom} \gls{magnetai}   ed
\gls{excire}  classificano correttamente 9 casi clean su 11, mentre
\gls{cellebriteinseyets} classifica correttamente tutti gli 11 campioni clean.
...
comunque classificati come \texttt{non\_weapon} da \gls{axiom} \gls{magnetai} ed
\gls{excire}. Questo risultato suggerisce che la presenza di termini
descrittivi nei metadati embedded non determina automaticamente una
classificazione positiva da parte dei tool black-box.

Nei 4 casi \gls{ood}, \gls{axiom} \gls{magnetai}   ed \gls{excire} producono
sempre una classificazione \texttt{weapon}, mentre \gls{cellebriteinseyets} produce
tre classificazioni \texttt{weapon} e una classificazione \texttt{non\_weapon}.
