...
\begin{tabularx}{\textwidth}{@{}p{2.55cm} c c p{2.65cm} p{2.65cm} X@{}}
\toprule
\textbf{Sistema} &
\textbf{Anti acc./drop} &
\textbf{Adv acc./drop} &
...
0.973 / 0.005 &
0.695 / 0.283 &
\gls{gaussianblur}: drop 0.013; 17 W(\rightarrow)NW &
\gls{sigmazero}: accuracy 0.015; drop 0.963; 489 W(\rightarrow)NW &
Baseline clean più elevata tra i proxy, ma vulnerabilità estrema nello scenario
white-box contro \gls{sigmazero} e degradazione rilevante sotto \gls{fgsm}. \\

ResNet18 &
0.957 / 0.007 &
0.955 / 0.009 &
\gls{histogrammodification}: drop 0.030; 23 W(\rightarrow)NW &
\gls{fgsm}: drop 0.046; 27 W(\rightarrow)NW &
Profilo più stabile rispetto a EfficientNet-B0 rispetto agli attacchi generati
contro il target white-box, ma con sensibilità anti-forense osservabile sulle
modifiche tonali. \\
...
\gls{fgsm}: drop 0.013; 8 W(\rightarrow)NW &
Robustezza apparente elevata rispetto alle perturbazioni considerate, ma
comportamento \gls{ood} molto esposto all'assorbimento semantico verso
\texttt{weapon}. \\

\gls{magnetaxiomtool}/\gls{magnetai} &
0.940 / 0.010 &
0.923 / 0.027 &
\gls{gaussianblur}: \gls{fnr} 0.120; 60 FN &
\gls{fgsm}: accuracy 0.867; \gls{fnr} 0.192; 96 FN &
...
0.941 / 0.003 &
0.904 / 0.040 &
\gls{histogrammodification}: accuracy 0.932; \gls{fpr} 0.088 &
\gls{sigmazero}: accuracy 0.742; \gls{fpr} 0.442 &
...
0.960 / 0.004 &
0.955 / 0.009 &
\gls{histogrammodification}: accuracy 0.949; \gls{fnr} 0.060 &
\gls{fgsm}: accuracy 0.918; \gls{fnr} 0.106 &
Profilo commerciale più stabile nel bundle considerato, senza degradazione
estrema su \gls{sigmazero}, ma con sensibilità residua a \gls{fgsm} e ad alcune
trasformazioni anti-forensi. \\
...
(\text{accuracy}\textit{{clean}-\text{accuracy}}{perturbed}). Per
\gls{excirefoto2025} viene riportata la configurazione \texttt{D50}, utilizzata
come riferimento comparativo principale. W(\rightarrow)NW indica transizioni da
\texttt{weapon} a \texttt{non\_weapon}; FN indica falsi negativi.
\end{table}
...
considerato, \gls{magnetai} mantiene un comportamento relativamente
conservativo ma con falsi negativi non trascurabili, mentre
\gls{excirefoto2025} evidenzia una dipendenza marcata dalla soglia di retrieval e
un rischio di over-triggering semantico su alcune perturbazioni.
