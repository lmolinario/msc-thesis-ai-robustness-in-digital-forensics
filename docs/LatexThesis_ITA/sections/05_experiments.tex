...
Dataset finale & \textbf{Congelato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize 1500 immagini suddivise in tre classi: 500 \texttt{weapon}, 500 \texttt{non\_weapon} e 500 \texttt{ood}.} \\[3pt]

Subset binario & \textbf{Congelato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize 1000 immagini, composte da 500 campioni \texttt{weapon} e 500 campioni \texttt{non\_weapon}. Utilizzato per gli split clean, gli attacchi adversariali e le trasformazioni anti-forensi.} \\[3pt]

Split clean e OOD & \textbf{Completati} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Cinque fold binari clean e un insieme di valutazione out-of-distribution separato, derivati dai manifest congelati.} \\[3pt]

Addestramento dei modelli proxy & \textbf{Completato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Checkpoint fold-aware prodotti per \texttt{efficientnet\_b0}, \texttt{resnet18} e \texttt{clip}.} \\[3pt]

Valutazione dei modelli proxy & \textbf{Completata} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Livello proxy valutato su input clean, OOD, adversariali e anti-forensi; output a livello di predizione e tabelle metriche finali.} \\[3pt]

Attacchi adversariali & \textbf{Generati} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Cinque famiglie di perturbazioni: \texttt{fgsm}, \texttt{superdeepfool}, \texttt{sigma\_zero}, \texttt{one\_pixel} e \texttt{color\_shift}.} \\[3pt]

Trasformazioni anti-forensi & \textbf{Generate} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Cinque trasformazioni model-agnostic: \texttt{jpeg\_recompression}, \texttt{resample\_resize}, \texttt{gaussian\_blur}, \texttt{histogram\_modification} e \texttt{contrast\_stretching}.} \\[3pt]

Forensic evaluation bundle & \textbf{Generato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize 11\,500 file predisposti per la valutazione black-box, includendo campioni clean, OOD, adversariali e anti-forensi.} \\[3pt]

Magnet AXIOM / Magnet.AI & \textbf{Normalizzato / consolidato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Bundle processato ed export normalizzato rispetto alla ground truth sperimentale. Il tag \texttt{Possible weapons} è ricondotto alla predizione positiva per la classe \texttt{weapon}.} \\[3pt]

X-Ways Forensics / Excire Photo AI & \textbf{Normalizzato / consolidato} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Valutato come sistema black-box di semantic retrieval, non come classificatore nativo \texttt{weapon}/\texttt{non\_weapon}. Tre configurazioni operative del parametro \texttt{Distance limit}: \texttt{D20}, \texttt{D50} e \texttt{D80}.} \\[3pt]

Cellebrite UFED & \textbf{Non incluso nella comparazione quantitativa} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Lo strumento rimane rilevante nella pipeline operativo-forense, ma non sono disponibili export normalizzati, riproducibili e direttamente associabili al forensic evaluation bundle. La sua rilevanza è discussa sul piano metodologico e procedurale.} \\[3pt]

Oxygen Forensic Detective & \textbf{Non incluso nella comparazione quantitativa} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Lo strumento rimane rilevante nella pipeline operativo-forense, ma la sua inclusione quantitativa richiederebbe output strutturati, normalizzabili e riconducibili in modo univoco alla ground truth sperimentale.} \\[3pt]

Casi di studio di explainability & \textbf{Completati / integrati} \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Attribution map basate su Integrated Gradients generate per cinque casi rappresentativi di EfficientNet-B0. L'analisi ha valore qualitativo e diagnostico; non costituisce una metrica primaria di valutazione.} \\
...
Il totale di 2500 predizioni per modello è ottenuto valutando le 500 immagini
...
\begin{tabular}{@{}l l@{}}
\toprule
\textbf{Tool} & \textbf{Stato} \\
\midrule

Magnet AXIOM / Magnet.AI & Consolidato \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Il forensic evaluation bundle è stato processato e l'export normalizzato rispetto alla ground truth sperimentale. Il tag \texttt{Possible weapons} è utilizzato come segnale operativo positivo per la classe \texttt{weapon}.} \\[3pt]

X-Ways / Excire Photo AI -- \texttt{D20} & Consolidato \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Configurazione restrittiva di semantic retrieval. Riduce i falsi positivi e il weapon flag rate sugli \gls{ood}, ma aumenta il rischio di falsi negativi sulla classe \texttt{weapon}.} \\[3pt]

X-Ways / Excire Photo AI -- \texttt{D50} & Consolidato \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Configurazione intermedia, adottata come setting principale per il confronto operativo. Presenta, nel contesto sperimentale considerato, un profilo equilibrato tra alto richiamo sulla classe \texttt{weapon} e controllo gestibile dei falsi positivi.} \\[3pt]

X-Ways / Excire Photo AI -- \texttt{D80} & Consolidato \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Configurazione permissiva. Massimizza il recall sulla classe \texttt{weapon}, ma aumenta sensibilmente il \gls{fpr} e il weapon flag rate sugli input \gls{ood}.} \\[3pt]

Cellebrite UFED & Non incluso nella comparazione quantitativa \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize Lo strumento rimane rilevante nel workflow forense, ma non sono disponibili output normalizzati e comparabili rispetto al forensic evaluation bundle.} \\[3pt]

Oxygen Forensic Detective & Non incluso nella comparazione quantitativa \\
\multicolumn{2}{@{}p{\linewidth}@{}}{\footnotesize La valutazione quantitativa richiederebbe moduli automatici, export strutturati e mapping verificabile rispetto alla ground truth sperimentale.} \\
...
Clean & 0.950 & 0.930 & 0.070 & 0.030 \\
\multicolumn{5}{@{}p{\linewidth}@{}}{\footnotesize Baseline commerciale su 1\,000 immagini non perturbate.} \\[3pt]

Perturbed & 0.932 & 0.899 & 0.101 & 0.035 \\
\multicolumn{5}{@{}p{\linewidth}@{}}{\footnotesize Valori calcolati su 10\,000 immagini adversariali e anti-forensi; il degrado principale riguarda l'aumento dei falsi negativi sulla classe \texttt{weapon}.} \\[3pt]

OOD & -- & -- & -- & -- \\
\multicolumn{5}{@{}p{\linewidth}@{}}{\footnotesize Weapon flag rate pari a 0.360: 180 immagini \gls{ood} su 500 segnalate come \texttt{Possible weapons}. Le metriche binarie standard non sono applicabili.} \\
...
a ridurre falsi positivi e recuperi semanticamente deboli. La configurazione
\texttt{D50} è adottata come setting principale di questa valutazione perché
presenta, nel contesto sperimentale considerato, il profilo di compromesso più
equilibrato tra recall della classe \texttt{weapon} e controllo dei falsi
positivi. La scelta della soglia operativa ottimale rimane tuttavia dipendente
dal contesto investigativo specifico, dai requisiti di auditabilità e dalla
tolleranza al rumore nel workflow di revisione. La
configurazione \texttt{D80} rappresenta invece un'impostazione permissiva,
...
\texttt{D20} & 0.927 & 0.880 & 0.120 & 0.026 & 0.238 \\
\multicolumn{6}{@{}p{\linewidth}@{}}{\footnotesize\textit{Configurazione restrittiva}: limita falsi positivi e segnalazioni \gls{ood}, ma incrementa il rischio di mancata segnalazione di immagini \texttt{weapon}.} \\[4pt]

\texttt{D50} & 0.944 & 0.958 & 0.042 & 0.070 & 0.340 \\
\multicolumn{6}{@{}p{\linewidth}@{}}{\footnotesize\textit{Configurazione intermedia}: rappresenta, nel contesto sperimentale considerato, un compromesso equilibrato tra recall della classe \texttt{weapon} e controllo dei falsi positivi.} \\[4pt]

\texttt{D80} & 0.915 & 0.986 & 0.014 & 0.156 & 0.522 \\
\multicolumn{6}{@{}p{\linewidth}@{}}{\footnotesize\textit{Configurazione permissiva}: massimizza il recall, ma produce un aumento marcato dei falsi positivi e delle segnalazioni su immagini fuori distribuzione.} \\
...
ma può non recuperare contenuti \texttt{weapon} rilevanti. \texttt{D50} appare
come la configurazione più equilibrata per un workflow di triage, poiché mantiene
un recall elevato sulla classe \texttt{weapon} senza produrre il livello di
rumore osservato in \texttt{D80}. Quest'ultimo, pur riducendo drasticamente i
...
\texttt{D50} mantiene un compromesso più equilibrato tra richiamo e selettività
nel contesto sperimentale considerato.
...
EfficientNet-B0 & 0.978 & 0.016 & 0.370 \\
\multicolumn{4}{@{}p{\linewidth}@{}}{\footnotesize Migliore baseline clean tra i proxy, ma vulnerabilità marcata ad alcune perturbazioni adversariali, in particolare \gls{sigmazero} e \gls{fgsm}.} \\[3pt]

ResNet18 & 0.964 & 0.030 & 0.438 \\
\multicolumn{4}{@{}p{\linewidth}@{}}{\footnotesize Buona baseline e maggiore stabilità rispetto a EfficientNet-B0 in alcuni scenari adversariali, pur con vulnerabilità osservabili.} \\[3pt]

\gls{clip} & 0.927 & 0.012 & 0.836 \\
\multicolumn{4}{@{}p{\linewidth}@{}}{\footnotesize Accuracy clean inferiore ai modelli supervisionati, ma forte tendenza a segnalare come \texttt{weapon} immagini fuori distribuzione.} \\[3pt]

Magnet AXIOM / Magnet.AI & 0.950 & 0.070 & 0.360 \\
\multicolumn{4}{@{}p{\linewidth}@{}}{\footnotesize Comportamento relativamente conservativo: basso \gls{fpr}, ma rischio non trascurabile di falsi negativi sulla classe \texttt{weapon}.} \\[3pt]

X-Ways / Excire Photo AI \texttt{D50} & 0.944 & 0.042 & 0.340 \\
\multicolumn{4}{@{}p{\linewidth}@{}}{\footnotesize Setting principale di triage: recall elevato sulla classe \texttt{weapon}, con incremento controllato dei falsi positivi rispetto alle configurazioni più permissive.} \\
