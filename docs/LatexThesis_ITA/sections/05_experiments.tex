...
Questo comportamento è coerente con la natura zero-shot di \gls{clip}: il modello
non è addestrato direttamente sul task binario \texttt{weapon}/\texttt{non\_weapon}
e la tendenza osservata a produrre un recall elevato sulla classe
\texttt{weapon}, al costo di un maggiore \gls{fpr}, riflette il meccanismo di
matching semantico adottato dal modello più che una scelta progettuale esplicita
orientata alla sensibilità.
...
\gls{advml}, ma come stressor sperimentali utili a valutare l'affidabilità
...
parametri. In termini operativi, tale risultato implica un \gls{fnr} prossimo all'unità
per la classe \texttt{weapon} nello scenario white-box considerato: in tale
configurazione, il modello risulta operativamente non affidabile per il triage
automatico di immagini contenenti armi, nonostante la baseline clean risulti la
più elevata tra i modelli proxy. Questo scenario presuppone che l'avversario
disponga di accesso completo ai parametri del modello; in condizioni di accesso
parziale o nullo, il degrado osservabile potrebbe essere inferiore.
...
clean. Nel caso di \gls{clip}, tale comportamento è coerente con l'uso di
rappresentazioni visivo-testuali più semantiche, potenzialmente meno sensibili ad
alcune perturbazioni pixel-level ottimizzate contro un modello supervisionato
specifico.
...
alcuni strumenti forensi possono indicizzare anche metadati embedded nei file
...
L'audit ha rilevato 15 file su 11,500 contenenti termini semanticamente o
sperimentalmente sensibili nei metadati embedded. Tali casi non interessano gli
artefatti adversariali o anti-forensi, ma soltanto 11 immagini clean e 4 immagini
...
relativi \texttt{bundle_id} sono stati incrociati con la ground truth, le
predizioni dei modelli proxy e gli output normalizzati degli strumenti
...
\texttt{non_weapon}. Per gli strumenti black-box, Magnet AXIOM/Magnet.AI ed
Excire Foto 2025 classificano correttamente 9 casi clean su 11, mentre
Cellebrite Inseyets classifica correttamente tutti gli 11 campioni clean.

Due casi risultano particolarmente rilevanti dal punto di vista interpretativo:
\texttt{bundle_000320} e \texttt{bundle_000948}. Entrambi sono immagini
\texttt{weapon} e contengono metadati esplicitamente weapon-related, ma vengono
comunque classificati come \texttt{non_weapon} da Magnet AXIOM/Magnet.AI ed
Excire Foto 2025. Questo risultato suggerisce che la presenza di termini
descrittivi nei metadati embedded non determina automaticamente una
classificazione positiva da parte dei tool black-box.

Nei 4 casi \gls{ood}, Magnet AXIOM/Magnet.AI ed Excire Foto 2025 producono
sempre una classificazione \texttt{weapon}, mentre Cellebrite Inseyets produce
tre classificazioni \texttt{weapon} e una classificazione \texttt{non_weapon}.
Questi casi vengono interpretati come parte del più generale rischio operativo
associato a input fuori distribuzione e semanticamente weapon-adjacent, per i
quali non è possibile stabilire se l'output derivi dal contenuto visuale, dai
metadati embedded o da una combinazione dei due fattori.

L'esclusione dei 15 file non modifica in modo sostanziale le metriche aggregate
dei tool commerciali. Pertanto, i casi individuati vengono mantenuti nel
benchmark e discussi come controllo di sensitività e limite operativo
documentato della cecità del bundle, piuttosto che come fattore invalidante dei
...
\caption{Confronto delle metriche clean per Magnet.AI ed Excire Photo AI nelle configurazioni \texttt{D20}, \texttt{D50} e \texttt{D80}. La figura riporta accuracy, recall della classe \texttt{weapon}, \gls{fnr} e \gls{fpr}.}
...
sono stati individuati cinque file di export complessivi; ai fini della
valutazione quantitativa, la ricostruzione delle segnalazioni
\texttt{Possible weapons} è stata effettuata utilizzando il file contenente
l'output predittivo rilevante per il task sperimentale.
...
chiarisce che il valore aggregato è influenzato dalla composizione sbilanciata
del bundle, nel quale le immagini perturbate sono numericamente prevalenti
rispetto ai campioni clean. Le metriche complessive devono quindi essere lette
insieme alla disaggregazione per condizione sperimentale riportata nella
...
degradazione più marcata rispetto alle trasformazioni anti-forensi: considerando
l'insieme completo delle metriche normalizzate per famiglia di perturbazione, il
\gls{fnr} medio adversariale è pari a 0.116, contro 0.087 per le trasformazioni
anti-forensi.

Questo risultato conferma che, nel caso di Magnet.AI, le
...
Magnet.AI rispetto a tutte le perturbazioni considerate nel forensic evaluation
bundle. Tra gli attacchi adversariali, \gls{fgsm} costituisce la condizione più
critica per il tool, con accuracy pari a 0.867, recall sulla classe
\texttt{weapon} pari a 0.808 e \gls{fnr} pari a 0.192. Tra le trasformazioni
anti-forensi, \gls{gaussianblur} produce il valore di \gls{fnr} più elevato,
pari a 0.120.


\begin{table}[H]
\centering
\caption{Prestazioni di Magnet.AI nelle condizioni adversariali e anti-forensi.}
...
Nel complesso, Magnet.AI mostra una buona accuratezza complessiva in condizioni
clean, pari a 0.950, con un \gls{fpr} contenuto pari a 0.030; tuttavia, il
\gls{fnr} pari a 0.070 indica che anche in assenza di perturbazioni una quota non
trascurabile di immagini \texttt{weapon} non viene segnalata.
Tale valore aumenta in presenza di alcune perturbazioni adversariali, con un
impatto operativo rilevante per il triage. In questa prospettiva, il dato più
rilevante non è la sola riduzione di accuracy, ma l'aumento dei falsi negativi:
in uno scenario di triage, un'immagine \texttt{weapon} non segnalata dal modulo
...
La configurazione \texttt{D20} rappresenta un'impostazione restrittiva,
orientata a ridurre falsi positivi e recuperi semanticamente deboli;
\texttt{D50} costituisce la configurazione intermedia della sensitivity
analysis; \texttt{D80} rappresenta invece un'impostazione permissiva,
orientata a massimizzare il recall ma esposta a un aumento rilevante dei
falsi positivi.
...
\caption{\gls{fpr} aggregato dei tool commerciali per famiglia di perturbazione, includendo trasformazioni anti-forensi e perturbazioni adversariali. Excire Photo AI è riportato nelle tre configurazioni operative \texttt{D20}, \texttt{D50} e \texttt{D80}.}
...
\caption{Accuracy drop dei tool commerciali black-box sotto perturbazioni adversariali. La figura evidenzia in particolare il comportamento anomalo di Excire Photo AI nelle configurazioni \texttt{D50} e \texttt{D80} in corrispondenza di \gls{sigmazero}.}
...
perturbazioni generate nello scenario white-box considerato, pur con
vulnerabilità osservabili. \\
...
ma con un aumento rilevante del rumore operativo. La scelta di \texttt{D50} come riferimento comparativo deriva dal profilo
intermedio osservato nella sensitivity analysis: rispetto a \texttt{D20}, riduce
il rischio di mancata segnalazione della classe \texttt{weapon}; rispetto a
\texttt{D80}, limita il rumore operativo e il weapon flag rate sugli input
\gls{ood}.
...
per i modelli proxy sia per gli strumenti commerciali black-box.
...
limitato ai modelli proxy: Magnet.AI segnala una quota non trascurabile di
immagini \gls{ood} come \texttt{Possible weapons}, mentre Excire Foto 2025 varia
in modo significativo al variare del parametro \texttt{Distance limit}. Questo
risultato mostra che la gestione degli \gls{ood} dipende non solo dal modello o
dal tool, ma anche dalla configurazione operativa adottata.
...
di ispezionare modello, training set, soglie interne o preprocessing
proprietario. Inoltre, Magnet AXIOM/Magnet.AI, Excire Foto 2025 e Cellebrite
Inseyets producono output di natura diversa. Magnet.AI espone un tag operativo
riconducibile alla categoria \texttt{Possible weapons}; Excire Foto 2025 è stato
valutato come sistema di semantic retrieval, la cui decisione operativa dipende
dalla soglia di similarità configurata; Cellebrite Inseyets produce categorie e
classificazioni proprietarie esportate attraverso l'ambiente di analisi
forense. La comparazione tra tali strumenti deve quindi essere interpretata come
confronto operativo di triage, non come confronto tra classificatori omogenei.

Un'ulteriore implicazione riguarda la configurazione operativa dei sistemi di
semantic retrieval. Nel caso di Excire Foto 2025, il parametro
...
Un ulteriore limite controllato riguarda la possibile presenza residuale di
metadati embedded nei file immagine. L'audit dedicato ha individuato un numero
limitato di casi, analizzati separatamente nella
Sezione~\ref{subsec:results-embedded-metadata-sensitivity}; tali casi non
modificano in modo sostanziale le metriche aggregate, ma confermano che la
...
informazioni non strettamente visuali, quali campi EXIF, XMP, IPTC o metadati
descrittivi residui.

La selezione delle perturbazioni, inoltre, non mira a coprire in modo esaustivo
l'intero spazio dell'\gls{advml}, ma a costruire uno stress test operativo
coerente con la finalità forense della tesi. Gli attacchi adversariali e le
trasformazioni anti-forensi sono quindi utilizzati come strumenti sperimentali
per valutare la robustezza operativa e non come benchmark completo di sicurezza
...
perturbazioni adversariali, e che tool commerciali con buona baseline possono
comunque segnalare come rilevanti immagini fuori distribuzione o produrre profili
di errore sostanzialmente diversi al variare delle soglie operative adottate.
Questi valori indicano che l'affidabilità operativa non può essere dedotta dalla
sola performance nominale su dati clean, ma richiede validazione sperimentale
esplicita nelle condizioni attese di utilizzo.

In conclusione, l'\gls{ai} può supportare il triage e la prioritizzazione delle
