...
separatamente. I valori sono bundle-weighted e riflettono la composizione
sperimentale del bundle, non una valutazione assoluta o certificativa dei
prodotti.
\end{table}
...
0.036 &
0.260 &
Valore di clean accuracy più elevato tra i tool commerciali e più basso assorbimento OOD tra
...
37 FN &
Valore più elevato di accuracy commerciale aggregata nello scenario adversarial, con basso
...
Il risultato più rilevante in prospettiva forense riguarda i falsi negativi sulla
classe \texttt{weapon}. Nel subset clean, tali errori sono già presenti, ma il
rischio aumenta in modo marcato in alcune condizioni perturbate. Il caso più
estremo è EfficientNet-B0 sotto \gls{sigmazero}, in cui l'accuracy scende a
0.015 e vengono introdotte 489 transizioni
\texttt{weapon}\(\rightarrow\)\texttt{non\_weapon}; anche \gls{fgsm} sullo stesso
modello produce 321 transizioni nella medesima direzione. In un workflow di
triage, questo tipo di errore è più critico di una riduzione aggregata di
accuracy, perché può ridurre la probabilità che contenuti investigativamente
rilevanti vengano sottoposti a revisione.

Il comportamento \gls{ood} rappresenta un secondo elemento autonomo di rischio.
I campioni fuori distribuzione non sono trattati come una terza classe
supervisionata, ma come verifica della tendenza dei sistemi ad assorbire input
anomali o semanticamente ambigui nella classe operativa \texttt{weapon}. I
modelli proxy e i tool commerciali mostrano tutti weapon flag rate non nulli sui
campioni \gls{ood}; tali valori non rappresentano errori binari in senso stretto,
ma descrivono il rischio di priorità investigativa impropria o di aumento del
carico di revisione umana.

La valutazione dei tool commerciali deve essere interpretata con particolare
prudenza. \gls{magnetaxiomtool}/\gls{magnetai}, \gls{excirefoto2025},
\gls{cellebriteinseyets} e \gls{griffeye}/\gls{t3kcore} sono valutati come
sistemi black-box o quasi black-box: non sono disponibili architettura, training
set, preprocessing interno, soglie decisionali, meccanismi di retrieval o logica
proprietaria di generazione delle categorie e dei bookmark. Il confronto è quindi
un confronto operativo di triage sul forensic evaluation bundle, non un confronto
tra classificatori omogenei né una certificazione generale dei prodotti. Per la
stessa ragione, metriche come AUROC non sono state assunte come metriche centrali
del confronto commerciale.

I limiti sperimentali principali sono quattro. Primo, gli attacchi
model-dependent sono generati rispetto a EfficientNet-B0 e devono essere letti
alla luce dello scenario white-box considerato; la loro trasferibilità verso
altri modelli e tool commerciali viene osservata empiricamente, ma non assunta
come proprietà generale. Secondo, le trasformazioni anti-forensi considerate non
esauriscono l'intero spazio delle manipolazioni realistiche possibili. Terzo, le
metriche aggregate sui tool commerciali sono bundle-weighted e dipendono dalla
composizione deliberatamente stressante del bundle. Quarto, l'audit sui metadati
embedded ha individuato un limite residuale della cecità del bundle, quantitativamente
contenuto ma metodologicamente rilevante per future validazioni.

In sintesi, il capitolo mostra che la robustezza operativa in ambito forense è
una proprietà multidimensionale: dipende non solo dalla prestazione nominale, ma
anche dal tipo di errore, dalla condizione dell'input, dal comportamento fuori
distribuzione, dalla configurazione del tool, dalla tracciabilità del bundle e
dal ruolo dell'analista umano. Il Capitolo successivo riprende queste evidenze
per discuterne le implicazioni metodologiche, operative e giuridiche nel contesto
dell'uso di strumenti \gls{ai}-assisted nei workflow di \gls{df}.
