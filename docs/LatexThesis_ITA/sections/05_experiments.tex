...
\gls{ai}-based per la classificazione e il triage di immagini in ambito forense.
L'analisi non mira a costruire un ranking assoluto basato sulla sola accuracy,
ma a valutare come modelli proxy e strumenti commerciali black-box reagiscano a
input clean, campioni \gls{ood}, trasformazioni anti-forensi e perturbazioni
adversariali. La lettura dei risultati mantiene quindi un focus esplicitamente
forense: falsi negativi sulla classe \texttt{weapon}, assorbimento operativo degli
input fuori distribuzione, tracciabilità del bundle, normalizzazione degli output
black-box e necessità di revisione umana \cite{costantini2019digital,sanna2024preliminary,sanna2025pixelperfect}.
...
\textbf{Consolidato} &
Valutato come piattaforma commerciale black-box di media triage, mediante
bookmark semantici automatici generati da \gls{t3kcore}; il bookmark
\texttt{CORE/Violence/Firearm} è ricodificato come predizione positiva
\texttt{weapon}. \\
...
5000
&
Cinque perturbazioni adversariali applicate al subset binario di 1000 immagini,
con 1000 file per ciascuna perturbazione. \\
...
\codepath{results/metrics/final_robustness_metrics.csv}. Per gli strumenti commerciali,
le metriche aggregate sono derivate da \codepath{results/metrics/forensic_tools_metrics.csv},
mentre le predizioni normalizzate e i controlli di audit sono ricavati dai corrispondenti export
normalizzati in \codepath{evaluation/forensic_tools/}. Gli output dei tool
black-box vengono quindi analizzati attraverso una rappresentazione comune,
indipendente dal formato nativo di esportazione.

Un'ulteriore cautela riguarda l'interpretazione della confidenza. I valori di
confidence riportati per i modelli proxy sono utilizzati come indicatori
intra-modello e non come probabilità calibrate direttamente comparabili tra
architetture diverse. Questa cautela è ancora più rilevante nel confronto con i
tool commerciali black-box, che non espongono necessariamente score numerici,
soglie decisionali o misure di incertezza omogenee. Per tale ragione, nel
capitolo la confidenza viene usata come elemento diagnostico ausiliario, non come
misura probatoria autonoma.
...
Questo comportamento non deve essere interpretato come effetto di una
classificazione zero-shot. Nel protocollo sperimentale adottato, \gls{clip} è
utilizzato come visual encoder pre-addestrato e congelato, adattato al task
\texttt{weapon}/\texttt{non\_weapon} mediante una testa binaria supervisionata.
La tendenza osservata a produrre un recall elevato sulla classe \texttt{weapon},
al costo di un maggiore \gls{fpr}, è quindi più correttamente letta come effetto
della rappresentazione visuale pre-addestrata e della calibrazione della testa
binaria rispetto al task specifico, non come risultato di un meccanismo zero-shot.
La minore confidenza media suggerisce inoltre che la maggiore propensione a
segnalare immagini come \texttt{weapon} non coincide necessariamente con predizioni
ad alta sicurezza.
...
Infatti, \gls{clip} riconduce alla classe \texttt{weapon} l'83.56\% delle
...
Sezione~\ref{sec:results-scope}, il drop prestazionale è interpretato come
variazione rispetto alla baseline clean. I valori positivi indicano una
riduzione della prestazione, mentre valori pari a zero o negativi sono trattati
come stabilità o oscillazioni specifiche della distribuzione valutata, non come
prova di robustezza generale.
...
\vspace{0.4em}
\footnotesize
\textit{Nota.} Il drop è definito nella Sezione~\ref{sec:results-scope};
valori positivi indicano degradazione rispetto alla baseline clean, mentre valori
negativi indicano un lieve incremento nella specifica distribuzione valutata.
\end{table}
...
\caption{Accuracy drop sotto trasformazioni anti-forensi. Il drop è definito nella Sezione~\ref{sec:results-scope}; valori positivi indicano degradazione rispetto alla baseline clean.}
...
In coerenza con la convenzione definita nella
Sezione~\ref{sec:results-scope}, il drop prestazionale è interpretato come
variazione rispetto alla baseline clean. In questa sezione, eventuali valori
negativi o nulli non sono letti come prova di robustezza adversarial generale,
ma come effetti specifici della trasferibilità delle perturbazioni generate
contro il modello target e della distribuzione dei campioni valutati.

La Tabella~\ref{tab:results-adversarial-robustness} riporta, per ciascuna
perturbazione adversariale, l'accuracy perturbata e l'accuracy drop rispetto
alla baseline clean.
...
\caption{Accuracy drop sotto perturbazioni adversariali. Il drop è definito nella Sezione~\ref{sec:results-scope}. La degradazione più marcata riguarda EfficientNet-B0 sotto \gls{sigmazero}, nello scenario white-box considerato.}
...
Un controllo specifico sui metadati embedded residui nella vista cieca del
bundle viene discusso nella
Sezione~\ref{subsec:results-embedded-metadata-sensitivity}, dopo la presentazione
degli output normalizzati degli strumenti commerciali, poiché la sua
interpretazione richiede il confronto con i risultati black-box osservati.
...
completa di classificazione. La valutazione si basa quindi esclusivamente sugli output osservabili prodotti
dai tool, quali tag assegnati, categorie esportate, risultati di retrieval e
bookmark semantici, nonché sul matching tra i file del forensic evaluation
bundle e la ground truth sperimentale.

Il confronto commerciale include quattro strumenti finali:
\gls{magnetaxiomtool}/\gls{magnetai}, \gls{excirefoto2025},
\gls{cellebriteinseyets} e \gls{griffeye}/\gls{t3kcore}. Poiché
\gls{excirefoto2025} è valutato attraverso tre configurazioni operative del
parametro \texttt{Distance limit}, il confronto metrico produce sei profili
commerciali normalizzati: \gls{magnetaxiomtool}/\gls{magnetai},
\gls{excirefoto2025} \texttt{D20}, \gls{excirefoto2025} \texttt{D50},
\gls{excirefoto2025} \texttt{D80}, \gls{cellebriteinseyets} e
\gls{griffeye}/\gls{t3kcore}.

Gli strumenti commerciali non sono trattati come classificatori omogenei.
\gls{magnetai} espone categorie operative di image categorization;
\gls{excirefoto2025} opera come motore di semantic retrieval;
\gls{cellebriteinseyets} produce output di media analysis esportati
dall'ambiente Cellebrite; \gls{griffeye}/\gls{t3kcore} produce bookmark
semantici automatici. La comparazione è quindi operativa e black-box: ogni output
viene ricondotto al task \texttt{weapon}/\texttt{non\_weapon} mediante regole di
normalizzazione esplicite e documentate.

Le perturbazioni adversariali incluse nel bundle non devono essere interpretate
come attacchi white-box ottimizzati contro i tool commerciali proprietari. Esse
sono artefatti generati nel protocollo controllato dei modelli proxy e sono qui
utilizzate per osservare, in modalità black-box, la sensibilità operativa degli
strumenti commerciali a input manipolati. Inoltre, le metriche aggregate sui
tool commerciali sono metriche \emph{bundle-weighted}: descrivono il
comportamento nel forensic evaluation bundle, la cui composizione è
intenzionalmente orientata alla robustezza, e non una stima assoluta della
...
La Tabella~\ref{tab:results-commercial-overview} riporta una sintesi bundle-weighted
delle metriche dei sistemi commerciali e delle configurazioni
operative considerate. Per tutti i profili commerciali, la normalizzazione ha
prodotto matching completo rispetto al forensic evaluation bundle
(11\,500/11\,500 file), senza righe non mappate o output non interpretabili. Per
\gls{excirefoto2025}, le tre righe corrispondono alle configurazioni
\texttt{D20}, \texttt{D50} e \texttt{D80} del parametro \texttt{Distance limit}.
La configurazione \texttt{D50} viene utilizzata come riferimento principale nelle
discussioni comparative, poiché rappresenta il compromesso intermedio tra
configurazione restrittiva e configurazione permissiva.
...
\caption{Sintesi bundle-weighted della valutazione black-box dei sistemi commerciali.}
...
bundle ricondotti a un output normalizzato. \textit{Clean FNR} indica il false
negative rate osservato sulla baseline non perturbata. \textit{Acc.} indica
l'accuracy sul task binario interpretabile; \textit{Bal.} indica la balanced
accuracy; \(R_w\) indica il recall per la classe \texttt{weapon}; \gls{fnr} e
\gls{fpr} indicano rispettivamente false negative rate e false positive rate sul
bundle binario complessivo; \textit{OOD W-rate} indica la quota di campioni
\gls{ood} ricondotti operativamente alla classe \texttt{weapon}. Le metriche
binarie sono calcolate sui campioni \texttt{weapon}/\texttt{non\_weapon}; i
campioni \gls{ood} sono analizzati separatamente. I valori sono bundle-weighted e
riflettono la composizione sperimentale del bundle, non una valutazione assoluta
o certificativa dei prodotti.
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/fig_forensic_tools_global_metrics_comparison.pdf}
\caption{Confronto globale delle metriche aggregate dei tool commerciali black-box sul forensic evaluation bundle. Il grafico confronta accuracy, balanced accuracy, recall \texttt{weapon}, FNR, FPR e OOD weapon flag rate dopo la normalizzazione degli output.}
\label{fig:results-forensic-tools-global-comparison}
\end{figure}

La lettura della Tabella~\ref{tab:results-commercial-overview} e della
Figura~\ref{fig:results-forensic-tools-global-comparison} richiede cautela.
Le metriche non descrivono l'accuratezza interna dei sistemi commerciali in
senso assoluto, ma il comportamento osservato dopo la normalizzazione degli
output nel task operativo \texttt{weapon}/\texttt{non\_weapon}. In particolare,
il caso di \gls{excirefoto2025} mostra chiaramente l'effetto della soglia di
retrieval: \texttt{D20} riduce il tasso di falsi positivi e il weapon flag rate
sui campioni \gls{ood}, ma aumenta i falsi negativi; \texttt{D80} massimizza il
recall sulla classe \texttt{weapon}, ma incrementa in modo marcato i falsi
positivi e le segnalazioni \texttt{weapon} sui campioni \gls{ood}.
\texttt{D50} rappresenta pertanto una configurazione intermedia, utile per il
confronto operativo con \gls{magnetai}, \gls{cellebriteinseyets} e
\gls{griffeye}/\gls{t3kcore}.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{images/fig_forensic_tools_ood_weapon_flag_rate.pdf}
\caption{Weapon flag rate dei tool commerciali black-box sui campioni \gls{ood}.}
\label{fig:results-forensic-tools-ood-weapon-flag-rate}
\end{figure}

La Figura~\ref{fig:results-forensic-tools-ood-weapon-flag-rate} evidenzia che
anche gli strumenti commerciali con buone prestazioni sul task binario
presentano un tasso non nullo di assorbimento dei campioni \gls{ood} nella
classe operativa \texttt{weapon}. Tale comportamento è particolarmente rilevante
per il triage forense, poiché può aumentare il carico di revisione umana e
attribuire priorità investigativa a contenuti semanticamente ambigui o esterni
al task principale.
...
\gls{magnetaxiomtool}/\gls{magnetai}, \gls{excirefoto2025},
\gls{cellebriteinseyets} e \gls{griffeye}/\gls{t3kcore}, distinguendo
prestazioni clean, comportamento sugli input perturbati, rischio di falsi
negativi sulla classe \texttt{weapon} e weapon flag rate sui campioni \gls{ood}.
...
Come indicato nella Sezione~\ref{sec:results-commercial}, il matching tra
export commerciale e forensic evaluation bundle è completo per tutti i profili
valutati. Per \gls{magnetai}, le metriche binarie sono quindi calcolate sulle
11\,000 immagini appartenenti alle classi operative \texttt{weapon} e
\texttt{non\_weapon}; le 500 immagini \gls{ood} sono analizzate separatamente
mediante il weapon flag rate.
...
0.116 &
0.038 &
Prestazione aggregata sulle 5 perturbazioni adversariali considerate. \\
...
evaluation bundle. Il dettaglio per condizione mostra un incremento dei falsi negativi soprattutto
sotto \gls{fgsm}, con accuracy pari a 0.867, recall sulla classe
\texttt{weapon} pari a 0.808 e \gls{fnr} pari a 0.192. Tra le
...
modo sostanziale i falsi positivi e riconduce alla classe \texttt{weapon} oltre
...
La valutazione di \gls{cellebriteinseyets} costituisce uno dei livelli
commerciali black-box della pipeline sperimentale. Il tool viene considerato come sistema
...
Come indicato nella Sezione~\ref{sec:results-commercial}, il matching tra
export commerciale e forensic evaluation bundle è completo per tutti i profili
valutati. Per \gls{cellebriteinseyets}, le metriche binarie sono quindi calcolate
sulle 11\,000 immagini appartenenti alle classi operative \texttt{weapon} e
\texttt{non\_weapon}; le 500 immagini \gls{ood} sono analizzate separatamente
mediante il weapon flag rate.
...
forensic evaluation bundle. Nel dettaglio per condizione, \gls{fgsm} produce il maggiore incremento dei
falsi negativi tra le perturbazioni adversariali, con accuracy pari a 0.918,
recall sulla classe \texttt{weapon} pari a 0.894 e \gls{fnr} pari a 0.106. Tra le trasformazioni
...
Il profilo osservato differisce sia da quello dei modelli proxy sia da quello
degli altri tool commerciali. \gls{cellebriteinseyets} non mostra una
degradazione estrema sotto \gls{sigmazero}: in tale condizione l'accuracy è pari
a 0.970, con \gls{fnr} pari a 0.042 e \gls{fpr} pari a 0.018. La condizione
adversarial più critica è invece \gls{fgsm}, che produce il valore più elevato
di falsi negativi tra gli attacchi considerati. Questo risultato deve essere
interpretato con cautela, poiché il tool rimane black-box e non è possibile
stabilire se la maggiore stabilità osservata derivi da differenze
architetturali, preprocessing interno, logiche proprietarie di media analysis o
caratteristiche specifiche dell'esportazione valutata.

Nel complesso, \gls{cellebriteinseyets} mostra un buon profilo nel forensic
evaluation bundle considerato: la clean accuracy è pari a 0.964, il \gls{fnr}
clean è pari a 0.032 e il weapon flag rate sui campioni \gls{ood} è pari a
0.292. Tali valori non devono essere interpretati come certificazione di
robustezza generale, ma come comportamento osservato nel protocollo sperimentale
definito. Rispetto agli altri strumenti commerciali, \gls{cellebriteinseyets}
mantiene un recall elevato sulla classe \texttt{weapon}; tuttavia,
\gls{griffeye}/\gls{t3kcore} presenta il valore più alto di accuracy commerciale aggregata
e il più basso false positive rate nel protocollo considerato. Il confronto tra tool commerciali deve
quindi essere interpretato come un trade-off tra recall, falsi negativi, falsi
positivi e comportamento sugli input \gls{ood}, non come individuazione di un
sistema universalmente superiore.

Anche in questo caso, l'output automatico deve essere trattato come supporto al
triage e non come decisione conclusiva, soprattutto in presenza di input
perturbati, fuori distribuzione o semanticamente ambigui.
...
La valutazione di \gls{griffeye}/\gls{t3kcore} introduce un ulteriore paradigma
di output commerciale black-box nel protocollo sperimentale. A differenza di
strumenti che espongono categorie, classificazioni o risultati di retrieval,
\gls{griffeye} è stato valutato attraverso i bookmark semantici automatici
generati da \gls{t3kcore}. In coerenza con la metodologia definita nel
Capitolo~\ref{chap:methodology}, tali bookmark sono trattati come output
operativi osservabili e non come spiegazioni locali del processo decisionale
interno.

Come indicato nella Sezione~\ref{sec:results-commercial}, il matching tra
export commerciale e forensic evaluation bundle è completo per tutti i profili
valutati. Per \gls{griffeye}/\gls{t3kcore}, le metriche binarie sono quindi
calcolate sulle 11\,000 immagini appartenenti alle classi operative
\texttt{weapon} e \texttt{non\_weapon}; le 500 immagini \gls{ood} sono
analizzate separatamente mediante il weapon flag rate. Il mapping operativo
considera positivo soltanto il bookmark \texttt{CORE/Violence/Firearm}, mentre
gli altri bookmark semanticamente collegati ad armi, violenza o contesto
militare sono conservati come informazione raw ma non sono utilizzati come
positivi primari. Questa scelta rende il mapping volutamente conservativo: il
basso \gls{fpr} osservato per \gls{griffeye}/\gls{t3kcore} deve quindi essere
letto anche come effetto della regola di normalizzazione adottata, non come prova
autonoma di superiorità intrinseca del tool. Una mappatura più permissiva,
includendo bookmark weapon-adjacent come positivi secondari, avrebbe potuto
aumentare il recall ma anche il numero di falsi positivi.

\begin{table}[H]
\centering
\caption{Prestazioni complessive di \gls{griffeye}/\gls{t3kcore} sul task binario \texttt{weapon}/\texttt{non\_weapon}.}
...
\textit{Nota.} Le metriche sono calcolate sulle 11\,000 immagini appartenenti al
task binario \texttt{weapon}/\texttt{non\_weapon}. Le 500 immagini \gls{ood} non
sono incluse nel calcolo dell'accuracy e sono analizzate separatamente. Il basso
\gls{fpr} riflette anche il mapping conservativo basato sul solo bookmark
\texttt{CORE/Violence/Firearm} come positivo primario.
\end{table}

\begin{table}[H]
\centering
\caption{Comportamento di \gls{griffeye}/\gls{t3kcore} per condizione sperimentale.}
...
0.051 &
0.007 &
Valori calcolati su 10\,000 immagini perturbate: 5\,000 adversariali e
5\,000 anti-forensi. \\
...
-- &
-- &
Weapon flag rate pari a 0.260: 130 immagini \gls{ood} su 500 sono state
ricondotte operativamente alla classe \texttt{weapon}. Le metriche binarie
standard non sono applicabili. \\
...
del sistema a ricondurre contenuti fuori distribuzione o semanticamente
weapon-adjacent alla classe operativa \texttt{weapon}. Nel caso di
\gls{griffeye}/\gls{t3kcore}, 130 immagini \gls{ood} su 500 sono state
segnalate come \texttt{weapon}, con un weapon flag rate pari a 0.260. Tale
valore non rappresenta un errore binario in senso stretto, poiché i campioni
\gls{ood} non appartengono alla ground truth \texttt{weapon}/\texttt{non\_weapon};
descrive piuttosto il grado di assorbimento operativo degli input fuori
distribuzione nel task binario di triage. Il valore osservato è inferiore a
quello di \gls{magnetai}, \gls{excirefoto2025} in configurazione \texttt{D50} e
\gls{cellebriteinseyets}, ma conferma che anche un sistema con basso
\gls{fpr} sul task binario può produrre segnalazioni operative su contenuti
esterni al dominio principale.

La Tabella~\ref{tab:results-griffeye-attack-metrics} riporta il comportamento di
\gls{griffeye}/\gls{t3kcore} rispetto a tutte le perturbazioni considerate nel
forensic evaluation bundle. Nel dettaglio per condizione, \gls{sigmazero} produce il valore di \gls{fnr}
più elevato tra le perturbazioni adversariali, con accuracy pari a 0.959,
recall sulla classe \texttt{weapon} pari a 0.926 e \gls{fnr} pari a 0.074. \gls{fgsm}
produce un comportamento molto vicino, con accuracy pari a 0.960 e \gls{fnr}
pari a 0.072. Tra le trasformazioni anti-forensi, \gls{histogrammodification} produce il
valore di \gls{fnr} più elevato, pari a 0.068, mentre
\gls{contraststretching} produce il valore di \gls{fpr} più elevato, pari a
0.010.
...
\renewcommand{\arraystretch}{1.12}
\setlength{\tabcolsep}{4pt}
\caption{Prestazioni di \gls{griffeye}/\gls{t3kcore} nelle condizioni adversariali e anti-forensi.}
...
Il profilo osservato differisce sia da quello dei modelli proxy sia da quello
degli altri tool commerciali. \gls{griffeye}/\gls{t3kcore} non mostra una
degradazione estrema sotto \gls{sigmazero}, a differenza di quanto osservato per
EfficientNet-B0. In tale condizione, tuttavia, si registra il valore più elevato
di falsi negativi per il tool, pari a 37, e il recall sulla classe
\texttt{weapon} scende a 0.926. Questo risultato suggerisce una sensibilità
residua a perturbazioni sparse o localizzate, pur in assenza di un collasso
prestazionale comparabile a quello osservato in alcuni modelli proxy.

Nel complesso, \gls{griffeye}/\gls{t3kcore} presenta il profilo commerciale più selettivo nel protocollo
considerato in termini di accuracy, balanced accuracy e contenimento
dei falsi positivi. Il \gls{fpr} complessivo pari a 0.007 è sensibilmente
inferiore rispetto agli altri strumenti commerciali valutati, mentre il recall
sulla classe \texttt{weapon}, pari a 0.951, rimane elevato ma non massimo.
\gls{cellebriteinseyets}, infatti, mantiene un recall leggermente superiore sulla
classe \texttt{weapon}, mentre \gls{griffeye}/\gls{t3kcore} presenta un profilo
più selettivo e meno incline alla generazione di falsi positivi.

Questi valori non devono essere interpretati come certificazione di robustezza
generale, ma come comportamento osservato nel protocollo sperimentale definito.
Anche in questo caso, il tool rimane black-box: non è possibile stabilire se il
profilo osservato dipenda dall'architettura interna del modulo, dal
preprocessing, dalle soglie proprietarie, dai criteri di generazione dei
bookmark o dalle caratteristiche specifiche dell'export valutato. Gli output
automatici devono quindi essere trattati come supporto al triage e non come
decisione conclusiva, soprattutto in presenza di input perturbati, fuori
distribuzione o semanticamente ambigui.
...
Dopo la normalizzazione degli output commerciali, è stato eseguito un controllo
di sensitività sui metadati embedded residui presenti nella vista cieca del
forensic evaluation bundle. La cecità del bundle è stata progettata
principalmente a livello di nomi dei file, percorsi e struttura delle directory;
tuttavia, alcuni strumenti black-box possono indicizzare anche metadati embedded
nei file immagine. Per questa ragione, il controllo è stato mantenuto nel
Capitolo~\ref{chap:results} come audit metodologico, ma non come risultato
primario di robustezza.

L'audit ha controllato 11\,500 file della directory cieca
\codepath{datasets/forensic_evaluation_bundle/blind_tool_input/files}, rilevando
7\,640 file con metadati embedded e 15 file contenenti termini semanticamente o
sperimentalmente sensibili. Tali 15 casi non interessano gli artefatti
adversariali o anti-forensi, ma soltanto 11 immagini clean e 4 immagini
\gls{ood}. Di conseguenza, il fenomeno non incide direttamente sul confronto
principale relativo alla robustezza sotto perturbazioni adversariali e
trasformazioni anti-forensi, ma rappresenta un limite operativo residuale della
cecità del bundle.

Due aspetti sono metodologicamente rilevanti. In primo luogo, la presenza di
termini descrittivi nei metadati non determina automaticamente una classificazione
positiva da parte degli strumenti black-box: alcuni campioni \texttt{weapon} con
metadati weapon-related risultano comunque non segnalati da almeno una parte dei
tool valutati. In secondo luogo, sui quattro casi \gls{ood} con sensitive hits,
non è possibile stabilire dai soli output osservabili se il flag \texttt{weapon}
derivi dal contenuto visuale, dai metadati embedded o da una combinazione dei due
fattori. Tali casi sono quindi interpretati come parte del più generale rischio
operativo associato a input \gls{ood} e semanticamente weapon-adjacent.
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{4pt}
\caption{Sintesi del controllo di sensitività sui metadati embedded residui.}
...
11\,500 &
0.130\% &
Quota molto limitata del forensic evaluation bundle. \\

Metriche binarie complessive &
11 &
11\,000 &
0.100\% &
Impatto quantitativo trascurabile sulle metriche binarie aggregate. \\

Clean baseline &
11 &
1\,000 &
1.100\% &
Impatto limitato alla baseline clean; le variazioni osservate dell'accuracy
rimangono nell'ordine di pochi millesimi. \\

Adversarial robustness &
0 &
5\,000 &
0.000\% &
Nessun caso con sensitive hits appartiene agli artefatti adversariali. \\

Anti-forensic robustness &
0 &
5\,000 &
0.000\% &
Nessun caso con sensitive hits appartiene agli artefatti anti-forensi. \\

OOD weapon flag rate &
4 &
500 &
0.800\% &
Impatto limitato al weapon flag rate sui campioni \gls{ood}; la variazione non
modifica l'interpretazione qualitativa dei risultati. \\

\bottomrule
\end{tabularx}
\end{table}

Il leave-out sensitivity check, calcolato escludendo i 15 file con sensitive
hits, conferma che le metriche aggregate non cambiano in modo sostanziale: le
variazioni di clean accuracy rimangono comprese tra -0.0009 e +0.0015, mentre le
variazioni del weapon flag rate sui campioni \gls{ood} rimangono inferiori a
0.006 in valore assoluto. Pertanto, i casi individuati vengono mantenuti nel
benchmark e discussi come limite operativo documentato della cecità del bundle,
non come fattore invalidante dei risultati sperimentali. La presenza residuale di
metadati embedded suggerisce tuttavia che, in studi successivi o in validazioni
operative, la neutralizzazione dei metadati dovrebbe essere inclusa come controllo
preventivo nella fase di costruzione del bundle cieco.
...
I risultati presentati nelle sezioni precedenti consentono di interpretare la
robustezza dei sistemi valutati su tre livelli complementari. Il primo livello
riguarda i modelli proxy, che permettono di osservare in modo controllato e
trasparente il comportamento di classificatori basati su \gls{ai} in presenza di
input clean, \gls{ood}, anti-forensi e adversariali. Il secondo livello riguarda
i tool commerciali, valutati come sistemi black-box attraverso i soli output
osservabili ed esportabili. Il terzo livello riguarda il forensic evaluation
bundle, che consente di sottoporre modelli proxy e tool commerciali alle stesse
famiglie di input, mantenendo tuttavia distinta la valutazione white-box dei
proxy dalla valutazione operativa black-box dei sistemi commerciali.

Il confronto non deve quindi essere interpretato come un ranking diretto tra
modelli proxy e prodotti commerciali. I modelli proxy operano su un task binario
controllato, con etichette esplicite, score di confidenza e predizioni
riproducibili. I tool commerciali, invece, espongono categorie proprietarie, risultati di semantic retrieval,
output di media analysis o bookmark semantici automatici che devono essere
normalizzati nelle etichette operative \texttt{weapon}, \texttt{non\_weapon} e,
dove necessario, in categorie non informative o non mappabili. Il valore del
confronto è quindi metodologico e operativo: esso permette di comprendere quali
tipi di errore possono emergere quando l'automazione \gls{ai}-based viene
utilizzata come supporto al triage forense di immagini.

\paragraph{Prestazioni clean e comportamento out-of-distribution.}

La Tabella~\ref{tab:results-comparative-clean-ood} riassume le prestazioni clean
e il comportamento sugli input \gls{ood}. La clean accuracy misura la prestazione
nominale sul task binario, mentre l'OOD weapon rate misura la frequenza con cui
immagini esterne alla distribuzione prevista vengono comunque ricondotte alla
classe operativa \texttt{weapon}. In un workflow forense, queste due dimensioni
devono essere lette congiuntamente: un sistema può ottenere una buona accuratezza
su dati clean, ma generare comunque rischio operativo se tende ad assorbire input
anomali nella classe di interesse investigativo.

\begin{table}[H]
\centering
\caption{Confronto tra prestazioni clean e comportamento OOD dei modelli proxy e dei sistemi commerciali black-box.}
...
0.016 &
0.370 &
Valore di clean accuracy più elevato tra i proxy, con basso tasso di falsi negativi sulla
classe \texttt{weapon}; tuttavia, una quota non trascurabile di input OOD viene
ricondotta alla classe \texttt{weapon}. \\
...
0.030 &
0.438 &
Baseline clean solida, ma con FNR superiore a EfficientNet-B0 e maggiore
assorbimento OOD verso \texttt{weapon}; la confidenza media sugli OOD rende tale
comportamento rilevante per il triage. \\
...
0.012 &
0.836 &
Accuracy clean inferiore rispetto ai modelli supervisionati, ma FNR molto
contenuto; il principale rischio operativo è l'elevato weapon flag rate sugli
input OOD. \\

Magnet AXIOM / Magnet.AI &
Commerciale black-box &
0.950 &
0.070 &
0.360 &
Prestazione clean elevata e comportamento relativamente conservativo, ma con un
rischio non trascurabile di mancata segnalazione di immagini \texttt{weapon}. \\
...
0.042 &
0.340 &
Configurazione intermedia del semantic retrieval, con un buon compromesso tra
recall sulla classe \texttt{weapon}, falsi positivi e assorbimento OOD. \\
...
0.032 &
0.292 &
Buon equilibrio nel clean set, con recall elevato sulla classe
\texttt{weapon} e OOD weapon flag rate inferiore rispetto a
\gls{magnetai} ed \gls{excirefoto2025} in configurazione \texttt{D50}. \\
...
0.036 &
0.260 &
Clean accuracy più elevata tra i tool commerciali nel mapping conservativo adottato; basso
assorbimento OOD, ma con una quota residua di falsi negativi sulla classe
\texttt{weapon}. \\
...
Il confronto sui dati clean mostra che la prestazione nominale non è sufficiente
per descrivere la robustezza operativa di un sistema. EfficientNet-B0 raggiunge
la clean accuracy più elevata tra i proxy, mentre
\gls{griffeye}/\gls{t3kcore} ottiene la clean accuracy più elevata tra i tool
commerciali considerati, ma tale risultato va letto alla luce del mapping
conservativo adottato per il bookmark \texttt{CORE/Violence/Firearm}.
\gls{cellebriteinseyets} mostra tuttavia un recall \texttt{weapon} leggermente
più elevato nel protocollo aggregato adottato. Il comportamento sugli
input \gls{ood} evidenzia una dimensione ulteriore del rischio: anche gli
strumenti commerciali con buona accuracy clean presentano weapon flag rate non
nulli su contenuti esterni al task.

EfficientNet-B0 e ResNet18 mostrano OOD weapon rate inferiori, rispettivamente
pari a 0.370 e 0.438. Tuttavia, la loro maggiore confidenza media sugli input
\gls{ood} rende tali errori potenzialmente più critici in un processo automatico
di prioritizzazione. I tool commerciali presentano valori più contenuti:
\gls{magnetai} raggiunge un OOD weapon flag rate pari a 0.360,
\gls{excirefoto2025} in configurazione \texttt{D50} pari a 0.340,
\gls{cellebriteinseyets} pari a 0.292 e
\gls{griffeye}/\gls{t3kcore} pari a 0.260. Questi risultati mostrano una minore
propensione operativa dei tool commerciali, nel protocollo adottato, ad assorbire
i campioni fuori distribuzione nella classe \texttt{weapon}. Tuttavia, poiché i
meccanismi interni, le soglie e la calibrazione dei tool commerciali non sono
accessibili, tali valori non devono essere interpretati come prova di effettiva
capacità di riconoscimento OOD, ma come comportamento osservabile dopo la
normalizzazione black-box e rispetto alle regole di mapping adottate.

\paragraph{Robustezza rispetto alle trasformazioni anti-forensi.}

La Tabella~\ref{tab:results-comparative-antiforensic} riporta il comportamento
comparativo dei sistemi sotto trasformazioni anti-forensi. Queste manipolazioni
non sono ottimizzate contro uno specifico confine decisionale del modello. Esse
rappresentano invece trasformazioni realistiche a livello di immagine, che possono
derivare da ricompressione, ridimensionamento, sfocatura, alterazioni tonali,
ricondivisione dei file o manipolazioni intenzionali. Per questa ragione, anche
drop apparentemente contenuti possono avere rilievo operativo.

\begin{table}[H]
\centering
\caption{Confronto della robustezza sotto trasformazioni anti-forensi.}
...
\textbf{Sistema} &
\textbf{Anti acc./drop} &
\textbf{Condizione più critica} &
\textbf{Errore principale} &
\textbf{Interpretazione di robustezza} \\
\midrule

EfficientNet-B0 &
0.973 / 0.005 &
Gaussian blur; acc. 0.965; drop 0.013 &
17 induced errors, tutti W$\rightarrow$NW &
Robustezza anti-forense complessivamente elevata, ma la sfocatura introduce un
rischio mirato di falsi negativi sulla classe \texttt{weapon}. \\

ResNet18 &
0.957 / 0.007 &
Histogram modification; acc. 0.934; drop 0.030 &
39 induced errors; 23 W$\rightarrow$NW e 16 NW$\rightarrow$W &
Profilo stabile in media, ma più sensibile alle alterazioni tonali, con errori
in entrambe le direzioni operative. \\

CLIP &
0.928 / -0.001 &
Histogram modification; 13 induced errors &
Numero limitato di errori indotti &
Elevata stabilità rispetto alle trasformazioni considerate; la criticità
principale resta il comportamento OOD, non la degradazione anti-forense. \\

Magnet AXIOM / Magnet.AI &
0.940 / 0.010 &
Gaussian blur; FNR 0.120 &
60 FN &
Buona robustezza aggregata, ma alcune trasformazioni aumentano i falsi negativi
su \texttt{weapon}, con impatto diretto sul triage. \\

Excire Foto 2025 D50 &
0.941 / 0.003 &
Histogram modification; acc. 0.932; FPR 0.088 &
Aumento dei falsi positivi &
Profilo anti-forense stabile nella configurazione D50, con criticità più legata
al rumore operativo che alla mancata rilevazione di \texttt{weapon}. \\

Cellebrite Inseyets &
0.960 / 0.004 &
Histogram modification; acc. 0.949; FNR 0.060 &
Falsi negativi residui su immagini \texttt{weapon} &
Buon profilo commerciale aggregato sotto trasformazioni anti-forensi, pur
con sensibilità residua alle modifiche tonali. \\

Magnet Griffeye / T3K CORE &
0.974 / 0.004 &
Histogram modification; acc. 0.963; FNR 0.068 &
34 FN &
Profilo commerciale più selettivo sotto trasformazioni anti-forensi, con
\gls{fpr} molto contenuto ma falsi negativi residui sotto modifiche tonali. \\
...
\vspace{0.4em}
\footnotesize
\textit{Nota.} Per i modelli proxy, i valori aggregati sono calcolati sulle
cinque trasformazioni anti-forensi considerate. Per i sistemi commerciali, i
valori derivano dagli output normalizzati sul forensic evaluation bundle. Il drop
è definito nella Sezione~\ref{sec:results-scope}. W$\rightarrow$NW indica
transizioni da \texttt{weapon} a \texttt{non\_weapon}; FN indica falsi negativi;
FNR indica false negative rate; FPR indica false positive rate.
\end{table}
...
\caption{Accuracy drop dei tool commerciali black-box sotto trasformazioni anti-forensi.}
\label{fig:results-forensic-tools-antiforensic-drop-heatmap}
\end{figure}

La Figura~\ref{fig:results-forensic-tools-antiforensic-drop-heatmap} integra la
lettura tabellare mostrando che, per i tool commerciali, le trasformazioni
anti-forensi producono in generale drop contenuti. Le variazioni più evidenti si
osservano per \gls{excirefoto2025} in configurazione \texttt{D80} sotto
Histogram modification e, in misura minore, per \gls{magnetai} sotto Gaussian
blur. \gls{griffeye}/\gls{t3kcore} mantiene un drop contenuto, pur presentando
falsi negativi residui nella condizione più critica.

I risultati anti-forensi mostrano una degradazione generalmente contenuta sia
nei modelli proxy sia nei tool commerciali. Questo risultato è importante perché
distingue l'effetto operativo di trasformazioni realistiche dall'effetto più
aggressivo delle perturbazioni adversariali. Per EfficientNet-B0, la condizione
più critica è il Gaussian blur, che produce 17 errori indotti, tutti nella
direzione \texttt{weapon}\(\rightarrow\)\texttt{non\_weapon}. Anche se il drop
aggregato è limitato, questa direzione dell'errore è forensicamente rilevante,
perché corrisponde a mancate segnalazioni di immagini contenenti armi.

ResNet18 mostra la degradazione anti-forense più marcata sotto Histogram
modification, con accuracy pari a 0.934 e 39 errori indotti. A differenza di
EfficientNet-B0, il pattern di errore è bidirezionale: sono presenti sia mancate
rilevazioni della classe \texttt{weapon}, sia falsi flag \texttt{weapon} su
immagini \texttt{non\_weapon}. \gls{clip} rimane invece relativamente stabile
rispetto alle trasformazioni testate. Questo conferma che, nel presente
protocollo, la principale criticità di \gls{clip} non riguarda la degradazione
anti-forense, ma la tendenza ad assorbire input \gls{ood} nella classe
\texttt{weapon}.

Anche i tool commerciali mostrano una degradazione aggregata limitata.
\gls{griffeye}/\gls{t3kcore} presenta il valore più elevato di accuracy anti-forense aggregata,
pari a 0.974, seguito da \gls{cellebriteinseyets},
\gls{excirefoto2025} in configurazione \texttt{D50} e \gls{magnetai}. Tuttavia,
l'interpretazione operativa varia da tool a tool. \gls{griffeye}/\gls{t3kcore}
mantiene un \gls{fpr} molto contenuto, ma mostra falsi negativi residui sotto
Histogram modification. \gls{cellebriteinseyets} mantiene un recall elevato, ma
con un \gls{fpr} superiore rispetto a \gls{griffeye}/\gls{t3kcore}. \gls{magnetai} risulta più
esposto ai falsi negativi sotto Gaussian blur, mentre \gls{excirefoto2025}
mostra una criticità più legata all'aumento dei falsi positivi sotto Histogram
modification.

\paragraph{Robustezza rispetto alle perturbazioni adversariali.}

La Tabella~\ref{tab:results-comparative-adversarial} sintetizza il profilo di
robustezza adversarial. A differenza delle trasformazioni anti-forensi, le
perturbazioni adversariali sono progettate per stressare il comportamento
decisionale dei sistemi basati su \gls{ai}. Per questo motivo producono effetti
più eterogenei e più dipendenti dal modello. I risultati evidenziano una
distinzione importante tra vulnerabilità white-box dei proxy e comportamento
operativo black-box dei tool commerciali.

\begin{table}[H]
\centering
\caption{Confronto della robustezza sotto perturbazioni adversariali.}
...
EfficientNet-B0 &
0.695 / 0.283 &
Sigma Zero; acc. 0.015; drop 0.963 &
963 induced errors; 489 W$\rightarrow$NW e 474 NW$\rightarrow$W &
Vulnerabilità proxy più marcata nello scenario white-box; l'elevata clean accuracy
non garantisce robustezza adversarial. \\

ResNet18 &
0.955 / 0.009 &
FGSM; acc. 0.918; drop 0.046 &
47 induced errors; 27 W$\rightarrow$NW e 20 NW$\rightarrow$W &
Profilo adversarial più stabile rispetto a EfficientNet-B0, ma con degradazione
residua sotto FGSM. \\

CLIP &
0.927 / 0.000 &
FGSM; acc. 0.914; drop 0.013 &
23 induced errors; 8 W$\rightarrow$NW e 15 NW$\rightarrow$W &
Degradazione limitata rispetto agli attacchi testati; la stabilità adversarial
non elimina il rischio OOD. \\

Magnet AXIOM / Magnet.AI &
0.923 / 0.027 &
FGSM; acc. 0.867; FNR 0.192 &
96 FN &
Robustezza aggregata discreta, ma FGSM aumenta sensibilmente i falsi negativi
sulla classe \texttt{weapon}. \\

Excire Foto 2025 D50 &
0.904 / 0.040 &
Sigma Zero; acc. 0.742; FPR 0.442 &
221 FP &
La perturbazione produce soprattutto over-triggering semantico, più che
occultamento sistematico di immagini \texttt{weapon}. \\

Cellebrite Inseyets &
0.955 / 0.009 &
FGSM; acc. 0.918; FNR 0.106 &
53 FN &
Buona stabilità commerciale aggregata anche nello scenario adversarial, con
incremento dei falsi negativi soprattutto sotto FGSM. \\

Magnet Griffeye / T3K CORE &
0.969 / 0.009 &
Sigma Zero; acc. 0.959; FNR 0.074 &
37 FN &
Accuracy commerciale aggregata più elevata nello scenario adversarial; il basso
\gls{fpr} riflette anche il mapping conservativo, mentre resta una sensibilità
residua a perturbazioni sparse. \\
...
\footnotesize
\textit{Nota.} Per i modelli proxy, i valori aggregati sono calcolati sulle
cinque perturbazioni adversariali considerate. Per i sistemi commerciali, i
valori derivano dagli output normalizzati sul forensic evaluation bundle. Il drop
è definito nella Sezione~\ref{sec:results-scope}. W$\rightarrow$NW indica
transizioni da \texttt{weapon} a \texttt{non\_weapon}; FN indica falsi negativi;
FNR indica false negative rate; FPR indica false positive rate.
...
La Figura~\ref{fig:results-forensic-tools-adversarial-drop-heatmap} evidenzia
che la risposta dei tool commerciali alle perturbazioni adversariali è
fortemente dipendente dal sistema e dalla configurazione. \gls{excirefoto2025}
risulta particolarmente sensibile a \gls{sigmazero} nelle configurazioni
\texttt{D50} e \texttt{D80}, mentre \gls{cellebriteinseyets} e
\gls{griffeye}/\gls{t3kcore} mostrano drop aggregati più contenuti. Questo
risultato rafforza l'interpretazione secondo cui la robustezza operativa dei
tool commerciali black-box non è uniforme, ma dipende dal tipo di output
osservabile, dal mapping adottato e dalla configurazione del sistema.


Il confronto adversarial mostra che la degradazione più severa si verifica nel
livello proxy, in particolare per EfficientNet-B0 sotto Sigma Zero. Il modello
passa da una clean accuracy pari a 0.978 a una accuracy pari a 0.015, con 963
errori indotti. Questo risultato mostra, nel protocollo considerato, che un modello può essere molto
accurato in condizioni nominali e, allo stesso tempo, estremamente fragile
rispetto a perturbazioni adversariali ottimizzate. Anche \gls{fgsm} produce una
degradazione significativa per EfficientNet-B0, con accuracy pari a 0.575 e drop
pari a 0.403. La vulnerabilità osservata non è quindi limitata a una singola
condizione adversarial.

ResNet18 e \gls{clip} mostrano un profilo più stabile nel presente setting
sperimentale. ResNet18 è maggiormente influenzato da \gls{fgsm}, ma il drop
rimane molto inferiore rispetto a quello osservato per EfficientNet-B0.
\gls{clip} risulta ancora meno degradato dalle perturbazioni testate. Tuttavia,
questo risultato deve essere interpretato con cautela: gli attacchi non sono
stati ottimizzati primariamente contro \gls{clip} come target white-box, e
\gls{clip} presenta già una diversa criticità sugli input \gls{ood}. Pertanto,
la stabilità adversarial osservata per \gls{clip} non deve essere generalizzata
oltre il threat model valutato.

I tool commerciali mostrano un pattern differente. \gls{magnetai} non riproduce
la degradazione catastrofica osservata su EfficientNet-B0 sotto Sigma Zero, ma
risulta sensibilmente influenzato da \gls{fgsm}, con \gls{fnr} pari a 0.192. Dal
punto di vista forense, questo è un errore critico, perché la mancata
segnalazione di immagini \texttt{weapon} può portare alla deprioritizzazione di
materiale rilevante.

\gls{excirefoto2025} in configurazione \texttt{D50} reagisce invece in modo
diverso: Sigma Zero produce soprattutto un aumento del false positive rate, pari
a 0.442, generando over-triggering semantico piuttosto che occultamento
sistematico della classe \texttt{weapon}. \gls{cellebriteinseyets} mostra un
recall elevato e una degradazione contenuta, ma mantiene un tasso di falsi
positivi superiore rispetto a \gls{griffeye}/\gls{t3kcore}.
Nel mapping conservativo adottato, \gls{griffeye}/\gls{t3kcore} presenta il valore
più elevato di accuracy commerciale aggregata e il false positive rate più basso, pur conservando
falsi negativi residui sulla classe \texttt{weapon}.

Nel complesso, il confronto commerciale non identifica un tool universalmente
superiore, ma evidenzia profili di rischio differenti.
\gls{griffeye}/\gls{t3kcore} presenta il profilo più selettivo e il minore
rumore operativo in termini di falsi positivi; \gls{cellebriteinseyets} mantiene
un recall leggermente più elevato sulla classe \texttt{weapon};
\gls{excirefoto2025} mostra un comportamento fortemente dipendente dalla soglia
di retrieval; \gls{magnetai} presenta un profilo più conservativo ma con maggiore
rischio di falsi negativi. Questa eterogeneità conferma che la robustezza
operativa dei software commerciali black-box è tool-dependent e deve essere
interpretata rispetto al tipo di output osservabile, al mapping adottato e al
ruolo dell'analista nel workflow.

\paragraph{Sensibilità alla soglia e interpretazione black-box.}

I risultati di \gls{excirefoto2025} richiedono una lettura separata, perché il
sistema è stato valutato come motore di semantic retrieval e non come
classificatore forense nativo. Il parametro \texttt{Distance limit} definisce il grado di restrittività
operativa del recupero semantico. La configurazione \texttt{D20} è
restrittiva: riduce i falsi positivi e i weapon flag sugli input \gls{ood}, ma
aumenta il rischio di mancata segnalazione di immagini \texttt{weapon}. La
configurazione \texttt{D80} è permissiva: massimizza il recall, ma aumenta i
falsi positivi e l'assegnazione di immagini non pertinenti o perturbate alla
query semantica relativa alle armi. La configurazione \texttt{D50} è stata
utilizzata come principale punto di confronto perché rappresenta un profilo
intermedio.

Questa dipendenza dalla soglia è particolarmente evidente sotto Sigma Zero. In
configurazione \texttt{D20}, \gls{excirefoto2025} ottiene accuracy pari a 0.815,
FNR pari a 0.180 e FPR pari a 0.190. In configurazione \texttt{D50}, l'accuracy
scende a 0.742, mentre il FNR migliora a 0.074 e il FPR aumenta a 0.442. In
configurazione \texttt{D80}, il recall migliora ulteriormente, ma il FPR sale a
0.662. Lo stesso tool può quindi supportare priorità forensi differenti in base
al punto operativo scelto: \texttt{D20} è più adatto a contenere il rumore,
\texttt{D80} a massimizzare il recall, e \texttt{D50} a bilanciare falsi
negativi e falsi positivi.

La natura black-box dei tool commerciali limita inoltre il tipo di conclusioni
che possono essere formulate. A differenza dei proxy, questi sistemi non
espongono confini decisionali, score calibrati, architetture, dati di training o
logiche interne di soglia. I loro output possono quindi essere valutati solo a livello operativo, attraverso
categorie osservabili, tag esportati, risultati di retrieval, bookmark
semantici, matching rate, output non mappabili, falsi negativi, falsi positivi e
comportamento sugli input \gls{ood}. Questa limitazione non rappresenta una
debolezza del protocollo di valutazione; al contrario, riflette la reale
condizione in cui gli analisti forensi interagiscono con strumenti commerciali
\gls{ai}-assisted.

\paragraph{Interpretazione operativa complessiva.}

L'analisi comparativa conferma che la robustezza nel triage forense di immagini
assistito da \gls{ai} è una proprietà multidimensionale. La clean accuracy misura
la prestazione nominale, ma non cattura l'assorbimento degli input \gls{ood}, la
sensibilità alle perturbazioni, il rischio di falsi negativi, l'over-triggering
semantico o l'effetto delle soglie nei sistemi black-box. Nei workflow di
\gls{df}, queste dimensioni sono almeno altrettanto rilevanti dell'accuracy
aggregata, perché influenzano l'attenzione dell'analista, la prioritizzazione dei
contenuti e la strategia di revisione manuale.

Emergono tre risultati principali. Primo, i modelli proxy sono utili perché
rendono osservabili pattern di degradazione controllati e permettono di misurare
confidence shift, errori indotti e direzione degli errori. Tuttavia, il loro
comportamento non può essere trasferito automaticamente ai tool commerciali, che
adottano meccanismi proprietari di classificazione, tagging, retrieval o
generazione di bookmark semantici.
Secondo, i tool commerciali mostrano profili di rischio differenziati: alcuni
privilegiano il recall, altri riducono i falsi positivi, altri ancora dipendono
fortemente dalla configurazione di retrieval o dal mapping semantico adottato.
Terzo, il comportamento \gls{ood} resta un rischio autonomo: esso può aumentare
il carico dell'analista, produrre prioritizzazioni fuorvianti e generare flag
relativi ad armi su materiale non appartenente al task binario previsto.

Nel complesso, i risultati supportano una lettura prudente dell'automazione
\gls{ai}-based in ambito forense. I sistemi valutati possono fornire un supporto
utile per il triage, la prioritizzazione e la revisione su larga scala di
contenuti multimediali. Tuttavia, i loro output non devono essere trattati come
evidenza conclusiva né come sostituti della validazione esperta. In particolare,
le mancate rilevazioni di immagini \texttt{weapon}, le assegnazioni OOD ad alta
confidenza e gli errori dipendenti dalla soglia devono essere documentati
esplicitamente quando strumenti \gls{ai}-assisted vengono utilizzati in workflow
forensi. Il protocollo FAIR-Lab risponde a questa esigenza combinando dataset
congelati, manifest tracciabili, riproducibilità basata su hash, famiglie di
perturbazioni controllate, normalizzazione degli output black-box e auditabilità
manuale dell'interpretazione finale.
...
\gls{excirefoto2025}, \gls{cellebriteinseyets} o
\gls{griffeye}/\gls{t3kcore}, che rimangono sistemi black-box nel contesto della
presente valutazione. Esse forniscono invece un riferimento diagnostico
controllato, limitato al modello proxy white-box, utile per discutere in modo
più concreto alcuni rischi operativi osservabili in un contesto \gls{df}
human-in-the-loop.

Le figure XAI riportate in questa sezione seguono la struttura introdotta nel
Capitolo~\ref{chap:methodology}: immagine di input, heatmap di
\gls{ig}, overlay dell'attribuzione sull'immagine originale e maschera delle
regioni con attribuzione più elevata. A differenza dell'esempio metodologico,
qui le visualizzazioni sono utilizzate per discutere casi rappresentativi della
valutazione sperimentale. La maschera di attribuzione non deve essere
interpretata come una segmentazione semantica o come ground truth dell'oggetto,
ma come una sintesi visuale delle aree che hanno maggiormente contribuito alla
predizione del modello proxy. Analogamente, la confidence riportata è lo score
del modello proxy e non una probabilità calibrata.
...
{Caso corretto su input pulito \texttt{xai\_case\_0001}. Il modello proxy
EfficientNet-B0 classifica correttamente un'immagine \texttt{clean} con etichetta
manuale \texttt{weapon} come \texttt{weapon}.}
...
{\textbf{scenario}: \texttt{clean};
 \textbf{etichetta manuale}: \texttt{weapon};
 \textbf{predizione}: \texttt{weapon};
...
{Falso negativo su input pulito \texttt{xai\_case\_0006}. Un'immagine
\texttt{clean} con etichetta manuale \texttt{weapon} viene classificata come
\texttt{non\_weapon}.}
...
{\textbf{scenario}: \texttt{clean};
 \textbf{etichetta manuale}: \texttt{weapon};
 \textbf{predizione}: \texttt{non\_weapon};
...
{Caso \gls{ood} \texttt{xai\_case\_0009}. Un campione fuori distribuzione viene
classificato come \texttt{weapon} con elevata confidenza.}
...
{\textbf{scenario}: \texttt{ood};
 \textbf{etichetta manuale}: \texttt{ood};
 \textbf{predizione}: \texttt{weapon};
...
{Falso negativo anti-forense \texttt{xai\_case\_0010}. Un'immagine
\texttt{weapon} sottoposta a \gls{histogrammodification} viene classificata come
\texttt{non\_weapon}.}
...
{Falso positivo adversarial ad alta confidenza \texttt{xai\_case\_0015}. Un'immagine
\texttt{non\_weapon} perturbata mediante \gls{sigmazero} viene classificata come
\texttt{weapon}.}
...
{\textbf{scenario}: \gls{sigmazero};
 \textbf{etichetta manuale}: \texttt{non\_weapon};
 \textbf{predizione}: \texttt{weapon};
 \textbf{confidence}: 1.000}
...
classificata come \texttt{weapon} con confidenza pari a 1.000. Ai fini della
...
I risultati del Capitolo~\ref{chap:results} mostrano che la robustezza operativa
di sistemi \gls{ai}-based per il triage forense di immagini non può essere
dedotta dalla sola accuracy clean. L'interpretazione deve considerare
congiuntamente la direzione degli errori, il comportamento sugli input \gls{ood},
la sensibilità alle perturbazioni, la configurazione operativa dei tool, la
normalizzazione degli output black-box e la disponibilità di revisione umana.

In prospettiva forense, il punto più rilevante è che la degradazione prestazionale
non ha sempre lo stesso significato operativo. Un falso negativo sulla classe
\texttt{weapon} può impedire che un contenuto investigativamente rilevante venga
sottoposto a revisione; un falso positivo o un weapon flag su contenuti \gls{ood}
può invece aumentare il carico di triage e generare priorità improprie. Per
questo motivo, le evidenze più significative del capitolo riguardano i failure
mode osservati, più che il solo valore aggregato di accuracy.

La valutazione dei tool commerciali deve essere letta con particolare prudenza:
\gls{magnetaxiomtool}/\gls{magnetai}, \gls{excirefoto2025},
\gls{cellebriteinseyets} e \gls{griffeye}/\gls{t3kcore} sono valutati attraverso
output osservabili e regole di mapping documentate, non tramite accesso a
modelli, soglie o logiche proprietarie interne. Il confronto commerciale è quindi
un confronto operativo sul forensic evaluation bundle, non una certificazione
generale dei prodotti né un confronto tra classificatori omogenei.

I principali limiti metodologici --- attacchi model-dependent generati rispetto a
EfficientNet-B0, spazio non esaustivo delle trasformazioni anti-forensi, metriche
commerciali bundle-weighted e residua sensibilità ai metadati embedded --- sono
ripresi in modo organico nel Capitolo~\ref{chap:conclusions}, insieme alle
implicazioni metodologiche, operative e giuridiche dell'impiego di strumenti
\gls{ai}-assisted nei workflow di \gls{df}.
