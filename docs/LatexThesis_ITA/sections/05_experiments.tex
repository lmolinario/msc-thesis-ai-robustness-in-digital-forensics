...
\caption{Accuracy drop sotto trasformazioni anti-forensi. Il drop è definito come \(\text{accuracy}_{clean}-\text{accuracy}_{perturbed}\): valori positivi indicano una degradazione della prestazione, mentre valori negativi indicano un lieve incremento rispetto alla baseline clean.}
...
\caption{Accuracy drop sotto perturbazioni adversariali. Il drop è definito come \(\text{accuracy}_{clean}-\text{accuracy}_{perturbed}\). La degradazione più marcata riguarda EfficientNet-B0 sotto \gls{sigmazero}, evidenziando la vulnerabilità del modello nello scenario white-box considerato.}
...
\gls{griffeye}/\gls{t3kcore}
& 11\,500 & 0.972 & 0.972 & 0.951 & 0.049 & 0.007 & 0.260 \\
...
\(R_w\) indica il recall per la classe \texttt{weapon}; \gls{fnr} e \gls{fpr}
...
\caption{Confronto globale delle metriche aggregate dei tool commerciali black-box.}
...
\gls{griffeye}/\gls{t3kcore} ottiene la migliore accuracy commerciale aggregata
e il più basso false positive rate. Il confronto tra tool commerciali deve
...
forensic evaluation bundle. Tra gli attacchi adversariali, \gls{sigmazero}
costituisce la condizione più critica, con accuracy pari a 0.959, recall sulla
classe \texttt{weapon} pari a 0.926 e \gls{fnr} pari a 0.074. \gls{fgsm}
produce un comportamento molto vicino, con accuracy pari a 0.960 e \gls{fnr}
pari a 0.072. Tra le trasformazioni anti-forensi, Histogram Modification produce
il valore di \gls{fnr} più elevato, pari a 0.068, mentre Contrast Stretching
produce il valore di \gls{fpr} più elevato, pari a 0.010.
...
Nel complesso, \gls{griffeye}/\gls{t3kcore} mostra il miglior profilo
commerciale aggregato in termini di accuracy, balanced accuracy e contenimento
...
nei file immagine. Per questa ragione, i casi individuati dall'audit sono stati
analizzati dopo il matching con ground truth, predizioni dei modelli proxy e
output normalizzati dei tool commerciali.
...
commerciali black-box. Sui campioni clean, EfficientNet-B0 e ResNet18 risultano
corretti in tutti gli 11 casi, mentre \gls{clip} produce un falso positivo su
un'immagine \texttt{non\_weapon}. Per gli strumenti commerciali, il comportamento
varia a seconda del tool e della configurazione: \gls{magnetaxiomtool}/\gls{magnetai}
classifica correttamente 9 casi clean su 11; \gls{excirefoto2025} classifica
correttamente 9 casi su 11 nelle configurazioni \texttt{D20} e \texttt{D50}, e
11 casi su 11 nella configurazione \texttt{D80}; \gls{cellebriteinseyets}
classifica correttamente tutti gli 11 casi clean; \gls{griffeye}/\gls{t3kcore}
classifica correttamente 10 casi clean su 11.
...
\gls{magnetaxiomtool}/\gls{magnetai} e da \gls{excirefoto2025} nelle
configurazioni più restrittive o intermedie. Questo risultato suggerisce che la
presenza di termini descrittivi nei metadati embedded non determina
automaticamente una classificazione positiva da parte degli strumenti black-box.
Nel caso di \gls{griffeye}/\gls{t3kcore}, \texttt{bundle\_000320} viene
ricodificato come \texttt{non\_weapon} perché l'export contiene bookmark
militari, ma non il bookmark primario \texttt{CORE/Violence/Firearm} richiesto
dal mapping operativo definito nel protocollo.

Nei 4 casi \gls{ood}, il comportamento dei tool commerciali riflette il più
generale rischio di assorbimento di contenuti fuori distribuzione nella classe
operativa \texttt{weapon}. \gls{magnetaxiomtool}/\gls{magnetai},
\gls{excirefoto2025} in configurazione \texttt{D50} e
\gls{excirefoto2025} in configurazione \texttt{D80} producono 4 flag
\texttt{weapon} su 4; \gls{cellebriteinseyets} produce 3 flag \texttt{weapon} su
4; \gls{excirefoto2025} in configurazione \texttt{D20} e
\gls{griffeye}/\gls{t3kcore} producono invece 1 flag \texttt{weapon} su 4. Tali
risultati non consentono di stabilire, sulla base dei soli output osservabili,
se la classificazione derivi dal contenuto visuale, dai metadati embedded o da
una combinazione dei due fattori. Essi vengono quindi interpretati come parte
del più generale rischio operativo associato a input \gls{ood} e
semanticamente weapon-adjacent.

Per quantificare l'impatto potenziale dei casi individuati sulle metriche
aggregate, è stata inoltre calcolata una verifica di esclusione
(\emph{leave-out sensitivity check}) limitata ai 15 file con sensitive hits. Tale
verifica non consente di stabilire un nesso causale tra metadati embedded e
classificazione, poiché gli strumenti commerciali rimangono black-box; consente
però di misurare quanto le metriche aggregate cambierebbero qualora tali file
fossero esclusi dal computo. Le tabelle di controllo sono state generate dallo
script
\codepath{results/scripts/21_generate_embedded_metadata_sensitivity_check.py},
che produce i file
\codepath{results/figures/chapter_5/tab_embedded_metadata_tool_summary.csv} e
\codepath{results/figures/chapter_5/tab_embedded_metadata_sensitivity_delta.csv}.
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{4pt}
\caption{Impatto quantitativo dei sensitive metadata hits sulle metriche aggregate.}
...
11\,500 &
0.130\% &
I casi con sensitive hits rappresentano una quota molto limitata del forensic
evaluation bundle. \\
...
11\,000 &
0.100\% &
L'impatto massimo sulle metriche binarie aggregate è trascurabile, poiché i casi
coinvolti appartengono solo alla componente clean del task
\texttt{weapon}/\texttt{non\_weapon}. \\
...
1\,000 &
1.100\% &
L'impatto è limitato alla baseline clean; la variazione osservata
dell'accuracy, dopo esclusione dei casi, rimane nell'ordine di pochi millesimi. \\

Adversarial robustness &
0 &
5\,000 &
0.000\% &
Nessun caso con sensitive hits appartiene agli artefatti adversariali; le
metriche adversariali non sono quindi influenzate da questo controllo. \\

Anti-forensic robustness &
0 &
5\,000 &
0.000\% &
Nessun caso con sensitive hits appartiene agli artefatti anti-forensi; le
metriche anti-forensi non sono quindi influenzate da questo controllo. \\

OOD weapon flag rate &
4 &
500 &
0.800\% &
L'impatto è limitato al weapon flag rate sui campioni \gls{ood}; la variazione
rimane contenuta e non modifica l'interpretazione qualitativa dei risultati. \\

\bottomrule
\end{tabularx}
\end{table}

La Tabella~\ref{tab:results-embedded-metadata-tool-summary} riporta il
comportamento dei tool commerciali sui soli file con sensitive metadata hits.
Il valore riportato per \gls{excirefoto2025} è distinto per configurazione,
poiché le soglie \texttt{D20}, \texttt{D50} e \texttt{D80} producono profili di
retrieval differenti.
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{4pt}
\caption{Comportamento dei tool commerciali sui file con sensitive metadata hits.}
\label{tab:results-embedded-metadata-tool-summary}
\begin{tabularx}{\textwidth}{@{}p{0.27\textwidth} r r r r r X@{}}
\toprule
\textbf{Sistema} &
\textbf{Clean} &
\textbf{Correct} &
\textbf{FN} &
\textbf{FP} &
\textbf{OOD weapon flags} &
\textbf{Interpretazione} \\
\midrule

\gls{magnetaxiomtool}/\gls{magnetai} &
11 & 9 & 2 & 0 & 4/4 &
Due falsi negativi clean e assorbimento completo dei quattro casi \gls{ood}
nella classe operativa \texttt{weapon}. \\

\gls{excirefoto2025} \texttt{D20} &
11 & 9 & 2 & 0 & 1/4 &
Configurazione più restrittiva: riduce i flag \gls{ood}, ma mantiene due falsi
negativi sui casi clean sensibili. \\

\gls{excirefoto2025} \texttt{D50} &
11 & 9 & 2 & 0 & 4/4 &
Configurazione intermedia: comportamento clean analogo a \texttt{D20}, ma con
maggiore assorbimento dei casi \gls{ood}. \\

\gls{excirefoto2025} \texttt{D80} &
11 & 11 & 0 & 0 & 4/4 &
Configurazione più permissiva: nessun errore sui casi clean sensibili, ma
assorbimento completo dei casi \gls{ood}. \\

\gls{cellebriteinseyets} &
11 & 11 & 0 & 0 & 3/4 &
Nessun errore sui casi clean sensibili, ma tre casi \gls{ood} su quattro vengono
ricondotti alla classe operativa \texttt{weapon}. \\

\gls{griffeye}/\gls{t3kcore} &
11 & 10 & 1 & 0 & 1/4 &
Un falso negativo clean dovuto all'assenza del bookmark primario
\texttt{CORE/Violence/Firearm}; basso assorbimento dei casi \gls{ood}. \\

\bottomrule
\end{tabularx}
\end{table}

La Tabella~\ref{tab:results-embedded-metadata-tool-delta} riporta la variazione
delle metriche commerciali più direttamente interessate, confrontando il valore
originale con il valore ottenuto escludendo i file con sensitive hits.
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{4pt}
\caption{Variazione delle metriche commerciali dopo esclusione dei sensitive metadata hits.}
\label{tab:results-embedded-metadata-tool-delta}
\begin{tabular}{@{}l l r r r@{}}
\toprule
\textbf{Sistema} &
\textbf{Metrica} &
\textbf{Originale} &
\textbf{Senza sensitive hits} &
\textbf{\(\Delta\)} \\
\midrule
...
\vspace{0.4em}
\footnotesize
\textit{Nota.} I valori ``senza sensitive hits'' sono ottenuti escludendo gli
11 casi clean e i 4 casi \gls{ood} individuati dall'audit dei metadati embedded.
Le variazioni riportate misurano l'impatto quantitativo dell'esclusione sulle
metriche aggregate, ma non stabiliscono un nesso causale tra metadati embedded e
decisione del tool.
\end{table}

I risultati del controllo mostrano che l'esclusione dei file con sensitive hits
non modifica in modo sostanziale le metriche aggregate. Le variazioni di clean
accuracy rimangono comprese tra -0.0009 e +0.0015, mentre le variazioni del
weapon flag rate sui campioni \gls{ood} rimangono inferiori a 0.006 in valore
assoluto. Inoltre, poiché nessuno dei 15 casi appartiene agli artefatti
adversariali o anti-forensi, le metriche di robustezza sotto perturbazioni non
sono interessate da questo limite residuale.

Pertanto, i casi individuati vengono mantenuti nel benchmark e discussi come
controllo di sensitività e limite operativo documentato della cecità del bundle,
piuttosto che come fattore invalidante dei risultati sperimentali. In termini
metodologici, questo controllo rafforza la tracciabilità dell'esperimento: il
bundle rimane utilizzabile per la valutazione black-box, ma la presenza
residuale di metadati embedded potenzialmente informativi viene esplicitata e
trattata come limite controllato.
...
0.016 &
0.370 &
Migliore baseline clean tra i proxy, con basso tasso di falsi negativi sulla
...
0.036 &
0.260 &
Migliore accuracy clean tra i tool commerciali e più basso assorbimento OOD tra
le configurazioni commerciali principali considerate, pur con una quota residua
di falsi negativi sulla classe \texttt{weapon}. \\
...
commerciali considerati. \gls{cellebriteinseyets} mantiene tuttavia un recall
\texttt{weapon} leggermente superiore a livello aggregato. Il comportamento sugli
...
\gls{griffeye}/\gls{t3kcore} pari a 0.260. Questi risultati suggeriscono una risposta
operativa più conservativa agli input fuori distribuzione. Tuttavia, poiché i
...
capacità di riconoscimento OOD, ma come comportamento osservabile nel protocollo
...
34 FN &
Migliore profilo commerciale aggregato sotto trasformazioni anti-forensi, con
...
è definito come
\(\text{accuracy}_{clean}-\text{accuracy}_{perturbed}\). W$\rightarrow$NW indica
...
\gls{griffeye}/\gls{t3kcore} ottiene la migliore accuracy anti-forense aggregata,
...
Massima vulnerabilità proxy nello scenario white-box; l'elevata clean accuracy
...
53 FN &
Buona stabilità commerciale aggregata anche nello scenario adversarial, ma
FGSM resta la condizione più critica. \\

Magnet Griffeye / T3K CORE &
0.969 / 0.009 &
Sigma Zero; acc. 0.959; FNR 0.074 &
37 FN &
Migliore accuracy commerciale aggregata nello scenario adversarial, con basso
\gls{fpr} ma sensibilità residua a perturbazioni sparse. \\
...
% ─────────────────────────────────────────────────────────────────────────────
% XAI representative case figure.
% Arguments:
% #1 = figure label
% #2 = interpretive caption
% #3 = input image path
% #4 = IG heatmap path
% #5 = IG overlay path
% #6 = top-attribution mask path
% #7 = case metadata
% ─────────────────────────────────────────────────────────────────────────────
\providecommand{\XAIcaseFigureMask}[7]{%
\begin{figure}[H]
\centering

\begin{minipage}[t]{0.235\textwidth}
    \centering
    \includegraphics[width=\textwidth,height=3.25cm,keepaspectratio]{#3}
    \vspace{1mm}

    \textbf{Input image}\\
    \footnotesize Original sample.
\end{minipage}
\hfill
\begin{minipage}[t]{0.235\textwidth}
    \centering
    \includegraphics[width=\textwidth,height=3.25cm,keepaspectratio]{#4}
    \vspace{1mm}
...
\begin{minipage}[t]{0.235\textwidth}
    \centering
    \includegraphics[width=\textwidth,height=3.25cm,keepaspectratio]{#5}
    \vspace{1mm}

    \textbf{IG overlay}\\
    \footnotesize Attribution over input image.
\end{minipage}
\hfill
\begin{minipage}[t]{0.235\textwidth}
    \centering
    \includegraphics[width=\textwidth,height=3.25cm,keepaspectratio]{#6}
    \vspace{1mm}

    \textbf{Top-attribution mask}\\
    \footnotesize Highest-attribution regions.
\end{minipage}

\caption{#2 \textit{Case metadata:} #7.}
\label{#1}
\end{figure}
}
...
I risultati del Capitolo~\ref{chap:results} confermano che la robustezza
operativa dei sistemi \gls{ai}-based per il triage forense di immagini non può
essere dedotta dalla sola accuracy clean. L'interpretazione dei risultati deve
considerare congiuntamente la direzione degli errori, il comportamento sugli
input \gls{ood}, la sensibilità alle perturbazioni, la configurazione operativa
dei tool e i limiti derivanti dalla valutazione black-box.

Il primo elemento critico riguarda i falsi negativi sulla classe
\texttt{weapon}. In un workflow di triage, una mancata segnalazione può ridurre
la probabilità che un contenuto investigativamente rilevante venga sottoposto a
revisione umana. Questo rischio è evidente nei modelli proxy, in particolare nel
caso di EfficientNet-B0 sotto \gls{sigmazero}, dove l'accuracy scende a 0.015 e
vengono introdotte 489 transizioni
\texttt{weapon}\(\rightarrow\)\texttt{non\_weapon}. Anche \gls{fgsm} produce un
effetto rilevante sullo stesso modello, con accuracy pari a 0.575 e 321
transizioni \texttt{weapon}\(\rightarrow\)\texttt{non\_weapon}. Questi risultati
mostrano che un'elevata baseline clean non garantisce affidabilità in presenza di
input manipolati.

Il secondo elemento riguarda il comportamento sugli input \gls{ood}. I campioni
fuori distribuzione non sono trattati come una terza classe supervisionata, ma
come rischio operativo autonomo. EfficientNet-B0 riconduce alla classe
\texttt{weapon} il 36.96\% delle predizioni \gls{ood}, ResNet18 il 43.76\% e
\gls{clip} l'83.56\%. Anche i tool commerciali mostrano weapon flag rate non
nulli sui campioni \gls{ood}: 36.00\% per \gls{magnetai}, 34.00\% per
\gls{excirefoto2025} in configurazione \texttt{D50}, 29.20\% per
\gls{cellebriteinseyets} e 26.00\% per
\gls{griffeye}/\gls{t3kcore}. Tali valori non rappresentano errori binari in senso
stretto, ma descrivono il rischio di assorbimento operativo di contenuti fuori
distribuzione entro una classe investigativamente sensibile.

Il terzo elemento riguarda la configurazione operativa dei sistemi di semantic
retrieval. Nel caso di \gls{excirefoto2025}, il parametro
\texttt{Distance limit} modifica direttamente il profilo di rischio: la
configurazione \texttt{D20} riduce falsi positivi e weapon flag \gls{ood}, ma
aumenta il rischio di falsi negativi; \texttt{D80} massimizza il recall, ma
accresce il rumore operativo; \texttt{D50} rappresenta un compromesso intermedio
nel protocollo sperimentale. La configurazione del tool deve quindi essere
documentata come parte integrante della valutazione.

La valutazione presenta inoltre limiti sperimentali. I tool commerciali sono
analizzati come sistemi black-box o quasi black-box: non sono disponibili
architettura, training set, soglie decisionali, preprocessing interno,
meccanismi di retrieval, logica di categorizzazione o criteri interni di
generazione dei bookmark. La comparazione tra \gls{magnetaxiomtool}/\gls{magnetai},
\gls{excirefoto2025}, \gls{cellebriteinseyets} e
\gls{griffeye}/\gls{t3kcore} deve quindi essere interpretata come confronto
operativo di triage, non come confronto tra classificatori omogenei e pienamente
ispezionabili. Per la stessa ragione, metriche come AUROC non sono state assunte
come metriche centrali del confronto commerciale.

Un limite controllato riguarda la possibile presenza residuale di metadati
embedded nei file immagine. L'audit ha rilevato 15 file con sensitive hits su
11\,500 file complessivi. Tali casi sono stati analizzati separatamente nella
Sezione~\ref{subsec:results-embedded-metadata-sensitivity} e non modificano in
modo sostanziale l'interpretazione delle metriche aggregate. Essi costituiscono
tuttavia un limite operativo documentato della cecità del bundle.

Infine, la selezione delle perturbazioni non copre in modo esaustivo l'intero
spazio dell'\gls{advml}. Gli attacchi adversariali e le trasformazioni
anti-forensi sono stati selezionati come stressor sperimentali coerenti con la
finalità forense della tesi. In particolare, gli attacchi model-dependent sono
generati rispetto a EfficientNet-B0 e devono essere interpretati alla luce dello
scenario white-box considerato; la loro trasferibilità verso altri modelli e tool
commerciali viene valutata empiricamente, ma non assunta come proprietà generale.

In sintesi, il Capitolo~\ref{chap:results} mostra che la robustezza operativa
degli strumenti \gls{ai}-based in ambito forense è una proprietà
multidimensionale. Essa dipende dalla prestazione clean, dalla direzione degli
errori, dal comportamento \gls{ood}, dal tipo di perturbazione, dalla
configurazione del tool e dal mapping degli output black-box. Questa evidenza
motiva la discussione conclusiva sulle implicazioni operative, metodologiche e
giuridiche dell'uso di strumenti \gls{ai}-assisted nei workflow forensi.