...
output osservabili prodotti dai tool, sui tag assegnati, sulle categorie
esportate, sui risultati di retrieval e sui bookmark semantici
...
0.055 &
0.008 &
Prestazione aggregata sulle cinque famiglie adversariali considerate. \\
...
0.046 &
0.006 &
Prestazione aggregata sulle cinque trasformazioni anti-forensi considerate. \\
...
Nel complesso, \gls{griffeye}/\gls{t3kcore} mostra il miglior profilo commerciale
aggregato in termini di accuracy, balanced accuracy e contenimento dei falsi
positivi. Il \gls{fpr} pari a 0.007 è sensibilmente inferiore rispetto agli altri
strumenti commerciali valutati, mentre il recall sulla classe \texttt{weapon}
rimane elevato ma non massimo. Questo risultato evidenzia un profilo operativo
selettivo: il tool riduce il rumore prodotto da falsi positivi, ma conserva una
quota residua di falsi negativi sulla classe investigativamente rilevante. Il
weapon flag rate sugli input \gls{ood}, pari a 0.260, conferma inoltre che anche
un sistema con basso \gls{fpr} sul task binario può assorbire una parte non
trascurabile di contenuti fuori distribuzione nella classe operativa
\texttt{weapon}.
...
sono interessate da questo limite residuale. Il sensitivity check conferma quindi
che i metadati embedded individuati costituiscono un limite operativo da
documentare, ma non un fattore capace di alterare l'interpretazione complessiva
dei risultati sperimentali.

L'eventuale esclusione dei 15 file non altera l'interpretazione complessiva delle
metriche aggregate dei tool commerciali. Pertanto, i casi individuati vengono mantenuti nel benchmark e discussi come
...
classificatore forense nativo. Il parametro Distance limit definisce il grado di
restrittività operativa del recupero semantico. La configurazione \texttt{D20} è
...
logiche interne di soglia. I loro output possono quindi essere valutati solo a
livello operativo, attraverso categorie osservabili, tag esportati, risultati di retrieval, bookmark
semantici, matching rate, output non mappabili, falsi negativi, falsi positivi e
comportamento sugli input \gls{ood}. Questa limitazione non rappresenta una debolezza del protocollo
di valutazione; al contrario, riflette la reale condizione in cui gli analisti
forensi interagiscono con strumenti commerciali \gls{ai}-assisted.
...
Esse forniscono invece un riferimento diagnostico controllato, limitato al modello proxy white-box, utile
...
Nel complesso, questa sezione chiude l'analisi dei risultati evidenziando i
principali limiti sperimentali e i profili di rischio osservati. Le implicazioni
più generali per l'uso dell'\gls{ai} nei workflow forensi, incluse le esigenze di
tracciabilità, validazione indipendente e controllo human-in-the-loop, sono
riprese nel Capitolo~\ref{chap:conclusions}.
