...
\textit{Magnet Griffeye} è una piattaforma commerciale utilizzata per il triage,
la revisione e l'organizzazione di grandi quantità di contenuti multimediali in
contesti investigativi e forensi. Nell'ambito della presente tesi, lo strumento
è rilevante non come classificatore accademico controllabile, ma come software
commerciale black-box capace di produrre output semantici osservabili
sull'immagine mediante moduli automatici di analisi.

Il componente \textit{T3K CORE} consente di generare bookmark semantici
automatici associati ai contenuti multimediali analizzati. Tali bookmark possono
essere utilizzati dall'analista per filtrare, prioritizzare o rivedere immagini
potenzialmente rilevanti. Dal punto di vista metodologico, questo tipo di output
è diverso sia dalle categorie restituite da \textit{Magnet.AI}, sia dai risultati
di recupero semantico prodotti da \textit{Excire Foto 2025}, sia dalle
classificazioni esportate nel workflow Cellebrite. Tuttavia, esso rientra nella
stessa famiglia di segnali operativi osservabili che possono essere normalizzati
rispetto a una ground truth sperimentale.

La rilevanza di \textit{Magnet Griffeye} per questa tesi risiede quindi nella
possibilità di estendere la valutazione black-box a un ulteriore paradigma di
output: non una probabilità di classe o una decisione supervisionata esplicita,
ma un insieme di bookmark semantici generati automaticamente. Questa
caratteristica rafforza il problema metodologico generale affrontato nella
tesi: strumenti commerciali differenti possono esporre output eterogenei, ma
devono essere ricondotti a una rappresentazione comune solo attraverso regole di
normalizzazione esplicite, documentate e coerenti con il task sperimentale.
...
I software considerati presentano caratteristiche funzionali differenti e non
devono essere interpretati come sistemi direttamente equivalenti.
\textit{Magnet.AI} espone categorie operative applicabili al contenuto delle
immagini; \textit{Excire Foto 2025} è considerato come sistema di ricerca e
recupero semantico di contenuti visivi; \textit{Cellebrite Inseyets} richiede
l'identificazione del componente concretamente utilizzato per la
categorizzazione dei media; \textit{Magnet Griffeye}, attraverso
\textit{T3K CORE}, produce bookmark semantici automatici. Pertanto, una
comparazione operativa tra tali software non può presupporre identità di task,
modelli interni, soglie decisionali o modalità di output.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}p{2.8cm}p{3.5cm}X p{3.2cm}@{}}
        ...
        Consente di valutare il recupero di immagini coerenti con il concetto firearm-oriented mediante un software standalone valutato in un contesto forense controllato &
        ...
        Generazione automatica di bookmark semantici su contenuti multimediali &
        Consente di valutare un ulteriore paradigma di output black-box, basato
        su bookmark semantici normalizzabili rispetto al task sperimentale &
        Tool commerciale black-box; la confrontabilità dipende dal mapping
        operativo tra bookmark esportati e classe sperimentale \texttt{weapon} \\
        \addlinespace
...
\texttt{weapon}, \texttt{non\_weapon} e \texttt{ood}, perturbazioni
adversariali, trasformazioni anti-forensi e un bundle unitario per la
valutazione forense. Rimane inoltre poco esplorata la normalizzazione
comparativa di output black-box eterogenei, quali categorie, classificazioni,
risultati di retrieval e bookmark semantici.
\midrule
