...
Nel protocollo sperimentale, EfficientNet-B0 assume il ruolo di \textit{primary proxy target} per la generazione degli attacchi adversariali model-dependent. Questa scelta non implica che EfficientNet-B0 rappresenti il modello più robusto o più accurato in senso assoluto, né che esaurisca l'analisi comparativa della tesi. Indica invece il modello controllato rispetto al quale vengono costruite, in questa fase della pipeline, le perturbazioni che richiedono accesso al gradiente, al classificatore o al comportamento decisionale del target. Tale impostazione consente di mantenere il protocollo adversarial controllato e confrontabile tra famiglie di perturbazione, evitando di moltiplicare il numero di varianti sperimentali in funzione dell'architettura attaccata. L'obiettivo non è ottimizzare separatamente gli attacchi contro ogni modello, ma generare un banco di prova adversarial coerente e successivamente valutare la trasferibilità delle perturbazioni verso modelli differenti, quali ResNet18 e \gls{clip}.
...
questa scelta non mira ad aumentare artificialmente il numero di campioni, ma a osservare la stabilità del comportamento fuori distribuzione al variare del checkpoint di addestramento. In questo modo, la valutazione \gls{ood} consente di verificare se l'incertezza o l'errore del modello rispetto a input non appartenenti alla distribuzione target sia stabile oppure dipenda sensibilmente dal fold utilizzato per l'addestramento, mantenendo al contempo la coerenza con il protocollo dei proxy model. In termini di famiglia di attacco, il summary distingue
