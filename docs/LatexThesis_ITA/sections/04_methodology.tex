...
bundle}, il protocollo di valutazione degli strumenti commerciali, le metriche,
...
NNel protocollo sperimentale, EfficientNet-B0 assume il ruolo di \textit{primary proxy target}
...
La trasformazione \texttt{color\_shift} viene invece trattata come perturbazione \textit{adversarial-style} model-agnostic. Essa modifica globalmente le componenti cromatiche dell'immagine, senza interrogare il modello e senza utilizzare informazioni di gradiente. Questa distinzione è metodologicamente rilevante perché consente di separare le perturbazioni costruite rispetto a un classificatore specifico da alterazioni visive più generali. All'interno della pipeline, \texttt{color\_shift} viene comunque incluso nel gruppo adversarial perché la sua finalità sperimentale non è simulare un ordinario intervento di post-processing forense, ma verificare se una variazione cromatica semanticamente conservativa possa alterare il comportamento dei classificatori automatici.
...
&
Contiene la vista cieca da importare nei tool commerciali. I file sono rinominati
...
La successiva valutazione operativa con strumenti commerciali utilizza il
bundle come input controllato e separa la fase di importazione dei file dalla
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Commercial forensic tools evaluation protocol}
\label{sec:methodology-commercial-tools}
% ─────────────────────────────────────────────────────────────────────────────

La pipeline operativo-forense di \gls{fairlab} prevede la valutazione di
strumenti commerciali impiegati in contesti professionali di \gls{df}, con
particolare attenzione ai moduli di analisi automatizzata delle immagini. A
differenza dei proxy model descritti nella
Sezione~\ref{sec:methodology-models-classification}, tali strumenti vengono
trattati come sistemi black-box o quasi black-box: la metodologia non assume
accesso alle architetture interne, ai dataset di addestramento, alle soglie
decisionali o alle logiche proprietarie di categorizzazione. L'analisi si
concentra invece sugli output osservabili, esportabili e riconducibili agli
artefatti del \textit{forensic evaluation bundle}.
...
anti-forensi. In questo modo, il protocollo consente di confrontare la risposta
dei tool commerciali con quella dei proxy model accademici, mantenendo tuttavia
...
Gli strumenti considerati nel protocollo comprendono Magnet AXIOM, X-Ways
Forensics, Cellebrite \gls{ufed} e Oxygen Forensic Detective, compatibilmente
con la disponibilità delle rispettive licenze, versioni software e funzionalità
di esportazione. Magnet AXIOM viene considerato per la presenza di funzionalità
di categorizzazione e analisi automatizzata dei contenuti multimediali. X-Ways
Forensics viene incluso come ambiente forense professionale ad approccio
low-level, valutabile anche in combinazione con esportazioni, estensioni o
workflow esterni di analisi visuale. Cellebrite \gls{ufed} e gli ambienti di
analisi collegati vengono considerati per il loro ruolo nell'estrazione e
nell'analisi forense di dispositivi mobili. Oxygen Forensic Detective viene
considerato per le funzionalità di analisi e categorizzazione automatizzata di
contenuti multimediali e artefatti provenienti da dispositivi mobili o sorgenti
cloud.

\begin{table}[H]
\centering
\caption{Strumenti forensi commerciali considerati nella pipeline operativo-forense.}
\label{tab:methodology-commercial-tools}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
...
\texttt{Magnet AXIOM}
&
Valutazione degli output prodotti dai moduli di analisi e categorizzazione
automatizzata dei contenuti multimediali rispetto agli artefatti del bundle. \\
\texttt{X-Ways Forensics}
&
Ambiente forense professionale utilizzato per l'ispezione, la gestione e
l'eventuale integrazione di workflow esterni di classificazione o triage
visuale. \\
\texttt{Cellebrite UFED}
&
Strumento considerato per scenari di acquisizione e analisi forense di
dispositivi mobili, con successiva valutazione degli output disponibili sui
contenuti immagine. \\
\texttt{Oxygen Forensic Detective}
&
Suite considerata per l'analisi di artefatti mobili, cloud e multimediali, con
attenzione alle funzionalità di categorizzazione automatizzata delle immagini. \\
\bottomrule
\end{tabularx}
\end{table}
...
La normalizzazione degli output commerciali richiede particolare cautela,
...
Questa impostazione consente di valutare la robustezza operativa degli strumenti
commerciali senza attribuire loro proprietà interne non verificabili. La
...
strumenti commerciali che possono esportare solo etichette categoriali, stati non standardizzati
...
&
Predisporre casi per il confronto qualitativo con gli output osservabili dei
tool commerciali, ove disponibili. \\
\bottomrule
\end{tabularx}
\end{table}
