...
\begin{figure}[htbp]
    ...
    \node[block] (examination) at (3.8,0)
    {\shortstack{
    ...
    \node[note] (thesisfocus) at (5.7,-2.35)
    {\shortstack{
        \textbf{Focus of this thesis}\\
        \scriptsize Evaluation of the robustness of AI-based image classification tools,\\
    ...
    \end{tikzpicture}
    \caption{Simplified digital forensic workflow and positioning of the present thesis within the examination and analysis phases \cite{nist2006guide,casey2011digital,reith2002examining}.}
...
\begin{figure}[htbp]
    \centering
    % Sostituisci con il tuo file grafico (es. png o pdf)
    \includegraphics[width=0.9\textwidth]{images/adversarial_example_logic}
    \caption{Rappresentazione concettuale di un attacco di \textit{evasion}. Una perturbazione invisibile $\delta$, ottimizzata matematicamente, viene sommata all'input originale $x$ per indurre il classificatore in errore, pur mantenendo l'immagine visivamente coerente per un analista umano.}
...
La letteratura ha proposto numerose tecniche per generare esempi adversariali, differenziate per vincolo di perturbazione, costo computazionale, conoscenza richiesta e trasferibilità. Metodi basati sul gradiente, come il \gls{fgsm}, sfruttano l'informazione fornita dalla funzione di perdita per individuare una direzione di perturbazione efficace \cite{goodfellow2015explaining}. Altri approcci mirano a produrre perturbazioni più ottimizzate, più sparse o localizzate, riducendo il numero di pixel modificati o migliorando la capacità dell'attacco di trasferirsi tra modelli diversi \cite{carlini2017towards,su2019onepixel,zajac2020superdeepfool}. Nel presente capitolo tali tecniche sono richiamate soltanto come fondamento teorico; la selezione e l'applicazione degli attacchi nella pipeline sperimentale vengono discusse nel capitolo metodologico.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lp{5cm}p{5cm}@{}}
        \toprule
        \textbf{Dimensione} & \textbf{Scenari Teorici} & \textbf{Focus della Tesi} \\
        \midrule
        \textbf{Fase dell'attacco} & Poisoning vs Evasion & \textbf{Evasion} (inferenza su dati acquisiti). \\
        \textbf{Conoscenza} & White-box vs Black-box & \textbf{Black-box} (tool commerciali chiusi). \\
        \textbf{Obiettivo} & Targeted vs Untargeted & \textbf{Untargeted} (degradazione delle performance). \\
        \bottomrule
    \end{tabular}
    \caption{Inquadramento del Threat Model adottato per la valutazione sperimentale.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{3.5cm}p{5cm}p{5cm}@{}}
        \toprule
        \textbf{Caratteristica} & \textbf{Attacchi Adversariali} & \textbf{Trasformazioni Anti-Forensi} \\
        \midrule
        \textbf{Bersaglio} & Logica matematica del modello \gls{ai}. & Integrità e tracce del file digitale. \\
        \textbf{Natura} & Perturbazione ottimizzata (rumore). & Manipolazioni reali (JPEG, resizing, filtri). \\
        \textbf{Dipendenza} & Alta (legata ai gradienti/pesi). & Nulla (indipendente dal classificatore). \\
        \textbf{Visibilità} & Spesso impercettibile all'uomo. & Spesso rilevabile (degradazione qualitativa). \\
        \bottomrule
    \end{tabular}
    \caption{Confronto tra le due categorie di manipolazioni analizzate nella tesi.}
