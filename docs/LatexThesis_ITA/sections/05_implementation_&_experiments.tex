...
L'analisi di explainability ha una funzione qualitativa e diagnostica. Non
sostituisce le metriche quantitative riportate nelle sezioni precedenti, ma
serve a osservare come le perturbazioni modifichino le regioni dell'immagine che
contribuiscono alla decisione del modello. In questa tesi, l'analisi è condotta
tramite Integrated Gradients sui modelli proxy, con particolare attenzione ai
casi in cui l'immagine clean viene classificata correttamente mentre la versione
perturbata produce un errore.

\subsection{Selection of representative cases}
\label{subsec:results-xai-selection}

I casi rappresentativi sono selezionati a partire dalle failure rows più
rilevanti, privilegiando immagini per cui:
\begin{itemize}
    \item la predizione clean è corretta;
    \item la predizione perturbata è errata;
    \item la perturbazione appartiene a una condizione sperimentalmente critica;
    \item il confronto visuale tra immagine clean, immagine perturbata e mappa
    di attribuzione è interpretabile.
\end{itemize}

\subsection{Clean versus perturbed attribution maps}
\label{subsec:results-xai-maps}

Al momento della redazione, la selezione dei casi rappresentativi è stata
definita sulla base delle failure rows più rilevanti. Le attribution map tramite
Integrated Gradients saranno inserite nella versione finale del capitolo; la
gallery completa sarà riportata in appendice. La valutazione quantitativa rimane
fondata sulle metriche delle sezioni precedenti.

Le mappe saranno interpretate verificando se, dopo la perturbazione, il modello
continui a concentrarsi sull'oggetto investigativamente rilevante o se
l'attenzione si sposti verso sfondo, texture, bordi o artefatti introdotti dalla
trasformazione.
