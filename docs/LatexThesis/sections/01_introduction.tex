...
additional possibilities for supporting investigative analysis.
In particular, \gls{ml} and \gls{dl} methods may assist with the semantic
classification of images and videos, the recognition of recurring patterns,
and the grouping of visually or semantically similar content. They may also
support the analysis of logs and structured artifacts, anomaly detection, and
the retrieval and prioritization of material potentially relevant to an
investigation.
...
The introduction of \gls{ai} into forensic workflows consequently requires a
critical evaluation that extends beyond nominal task performance. The analysis
must examine system behavior under unexpected or manipulated inputs and
distinguish the operational consequences of false negatives from those of false
positives.

It must also document the system version and active configuration, clarify the
semantics of the observable outputs and the rules used to normalize them, and
preserve traceability from each source item to the corresponding automated
result. Expert verification must remain part of the process throughout the
evaluation and subsequent operational use.
...
results. Direct-target evaluation must be distinguished from empirical transfer
to a different architecture or software system. Within direct-target
generation, white-box and score-based access represent different attacker
capabilities, whereas deterministic model-agnostic transformations follow a
separate experimental rationale.

The effects observed under these conditions must not be interpreted as
equivalent forms of attack success or as interchangeable evidence of system
robustness.
...
system may perform strongly on clean inputs while still increasing missed-target
risk under a specific manipulation or generating additional false-positive
review workload.

It may also produce closed-set assignments on out-of-task content, exhibit
sensitivity to thresholds, semantic-query settings, or output mappings, or
undergo severe degradation under a direct-target adversarial condition.
...
Relevant questions concern whether a manipulation prevents target content from
receiving appropriate review priority or increases the volume of irrelevant
material presented for examination. The analysis must also determine whether
the manipulation alters the balance between recall and selectivity or produces
closed-set assignments on content outside the nominal task.

A further distinction concerns whether the manipulation directly exploits a
specific model or transfers to other architectures and software systems. It is
also necessary to determine whether the observed behavior depends on the active
configuration or on the rules used to normalize the available outputs.
...
1\,000-image binary subset. This design preserves the conceptual and
experimental distinction among supervised binary correctness,
clean-to-perturbed error transitions, and the assignment behavior observed on
the separate \gls{ood} branch.
...
transparent proxy models and selected black-box software configurations used for the
...
The adversarial and anti-forensic branches are treated separately. The
adversarial branch comprises four model-dependent attacks and the deterministic
model-agnostic \gls{colorshift} condition. Because \gls{colorshift} does not
rely on gradients, model parameters, checkpoints, or observable scores, it is
interpreted separately from direct-target and transferred attack success. The
anti-forensic branch comprises plausible image-level modifications that may
arise through ordinary media processing or may be intentionally used to reduce
...
includes human-validated dataset construction, deterministic partitioning,
transparent proxy-model evaluation, controlled perturbation generation,
preparation of a blind forensic evaluation bundle, black-box output
...
\textbf{\gls{fairlab}} protocol. \gls{fairlab} integrates human-validated
dataset construction, deterministic data partitioning, transparent proxy-model
evaluation, and the generation of adversarial and anti-forensic artifacts.

The protocol also encompasses the preparation of a blind forensic evaluation
bundle, the assessment of black-box software, and the application of explicit
output-normalization rules. These components are connected to quantitative
error-oriented metrics, qualitative explainability analysis, and traceability
...
contains 1\,500 images organized into three equally sized branches. These
comprise 500 \texttt{weapon} images, 500 \texttt{non\_weapon} images, and
500 images reserved for the separate \gls{ood} branch.
...
The bundle comprises 1\,000 clean binary items, 500 clean \gls{ood} items,
5\,000 adversarial artifacts, and 5\,000 anti-forensic artifacts.
...
The black-box level comprises four software systems evaluated through six
frozen configurations. These include
...
These six configurations are evaluated exclusively through outputs observable
to the analyst, including tags, bookmarks, exported classification labels, and
...
The protocol therefore distinguishes direct-target attacks generated against
fold-specific EfficientNet-B0 checkpoints from empirical transfer to ResNet18
and the \gls{clip}-based proxy. It separately examines empirical transfer to
the selected black-box configurations.

The deterministic and model-agnostic \gls{colorshift} condition is kept
distinct from these model-dependent attacks, while the anti-forensic
transformations constitute a separate experimental branch.
...
single aggregate score. It examines false negatives affecting
firearm-oriented content and false positives that increase analyst review
workload.

The analysis also considers directional clean-to-perturbed error transitions,
closed-set weapon assignments on the separate \gls{ood} branch, and the
distinction between direct-target and transfer behavior. Configuration-dependent
changes in recall and selectivity, residual embedded-metadata sensitivity, and
the continued need for human verification are incorporated into the
...
when it can be linked to the original source item and its integrity record, as
well as to the model or software version used during processing.

The retained documentation should identify the active checkpoint or operational
configuration, the observable output produced by the system, and the
normalization rule applied to that output. It should also record the
experimental or operational condition under which the item was processed and
preserve the connection with the subsequent human review and final
interpretation.
