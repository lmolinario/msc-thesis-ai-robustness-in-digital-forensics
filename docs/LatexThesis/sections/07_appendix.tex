...
is consistent with the general principles of \gls{df}, in which the preservation
of provenance, artifact integrity, and documentation of operations are essential
requirements for making a digital analysis process verifiable
\cite{casey2011digital,garfinkel2010digital,nist2006guide,iso27037}.
...
evaluation, and preparation of the bundle intended for commercial forensic
tools.
...
commercial forensic tools. The \codepath{explainability/} directory is reserved
for the generation of \gls{xai}-based case studies, whereas
\codepath{docs/LatexThesis_ITA/} contains the LaTeX version of the thesis.
...
connection among image identifier, source, final class, hash, split, possible
...
across the \texttt{weapon}, \texttt{non\_weapon}, and \gls{ood} classes. \\
...
&
Global manifest of the bundle intended for evaluation with commercial forensic
tools. \\
...
operational instructions for import into forensic tools. \\
...
distributed in a balanced manner across three classes: \texttt{weapon},
\texttt{non\_weapon}, and \gls{ood}. The \texttt{weapon} class includes only
realistic individual weapons that are clearly visible; the \texttt{non\_weapon}
class includes realistic negative samples consistent with the task; and the
\gls{ood} class collects borderline or out-of-distribution cases, such as
knives, swords, replicas, airsoft items, synthetic renderings, videogame
images, heavy military vehicles, explosions, war scenes, and degraded or
anomalous images. The official adversarial subset, stored in
\codepath{manual_selection_adversarial_subset.csv}, excludes \gls{ood} examples
and retains only the two operational classes \texttt{weapon} and
\texttt{non\_weapon}, for a total of 1\,000 images.
...
This connection is particularly relevant in the forensic phase, since
commercial tools may rename files, modify the structure of exports, or produce
reports in heterogeneous formats. The use of hashes therefore makes it possible
to maintain a criterion that is independent of the file name and export path.
...
constitutes the connection point between the exports produced by commercial
forensic tools and the final comparative metrics. In the final protocol, it
exclusively supports outputs attributable to \texttt{magnet\_axiom},
\texttt{excire\_foto\_2025}, \texttt{cellebrite\_inseyets}, and
\texttt{griffeye}. Its role is to normalize the exports, map predictions back
to the records of the forensic evaluation bundle, and produce results that are
comparable with the proxy level through mapping based, in preferential order, on
\texttt{bundle\_id}, \gls{sha256} hash, \gls{md5} hash, file name, and manual
audit.
...
broader context of robustness in \gls{ml} and \gls{ai} systems
\cite{bishop2006pattern,biggio2018wild,radford2021learning}.
...
models, whereas anti-forensic transformations simulate realistic manipulations
of the image content or file that may alter the visibility, quality, or
technical traces of the artifact
\cite{stamm2010anti,nowroozi2021survey,croce2020robustbench}.
...
structure strictly separates the blind inputs intended for the software from the
metadata required for the subsequent evaluation through hash-based mapping.
...
software. The \codepath{metadata/} and \codepath{structured_audit_view/}
directories must not be imported, since they contain information that would
reveal true classes, sources, attacks, transformations, and experimental
mapping. This separation reduces the risk of bias induced by file names,
directory structure, or prior knowledge of the ground truth.
...
the effective hashes, correspondence of hashes when present in the manifest,
semantic cleanliness of the blind paths, and separation of metadata from the
inputs intended for the tools.
...
\texttt{Captum} & Generation of attribution maps through Integrated Gradients. \\
\texttt{matplotlib} & Production of plots, curves, experimental figures, and diagnostic visualizations. \\
\bottomrule
\end{tabularx}
\end{table}

The operational-forensic evaluation also includes four software systems observed
in \textit{black-box} mode: \textit{Magnet AXIOM}, version 10.1.0.48673, with
the \textit{Magnet.AI} functionality; \textit{Excire Foto 2025}, version 4.1.5,
used as a standalone general-purpose application with \gls{ai} functionalities;
\textit{Cellebrite Inseyets}, version 10.9, with exports produced in the
\textit{Cellebrite Physical Analyzer} 10.9.0.3029 environment; and
\textit{Magnet Griffeye x64}, version 26.2.108, with the \textit{T3K CORE}
v1.18.0 module. Unlike Python scripts and proxy models, these tools introduce a
component that is not fully automatable, due to licenses, graphical interfaces,
software versions, operational configurations, and export formats. For this
reason, their evaluation is documented through the blind bundle, operational
instructions, normalized exports, and hash-based mapping.
...
In addition to the main manifests, the repository preserves logs, summaries,
and intermediate reports that are useful for verifying the operations performed.
...
function is to make the experimental path inspectable and to support any
subsequent checks.
...
\codepath{split_generation_summary.json}, the attack manifests, and
\codepath{bundle_summary.json}. These files make it possible to reconstruct not
only the final output, but also the status and operational assumptions of the
...
Similarly, the evaluation with commercial forensic tools is not entirely
reproducible through command-line execution, since it depends on proprietary
software, specific versions, interface settings, internal \gls{ai}-based
modules, and export procedures. To mitigate this limitation, the pipeline uses a
common blind input, preserves file hashes, separates the ground truth from the
inputs submitted to the tools, and includes a normalization phase for the
exports. In this way, reproducibility is not understood as a bit-for-bit replica
of every graphical interaction, but as verifiability of the protocol, inputs,
mappings, and final metrics.
...
thesis text, figures, datasets, raw images, forensic tool exports, model
