...
\begin{lstlisting}[float=p, caption={Simplified structure of the experimental repository.}, label={lst:repository-structure}]
...
generation of \gls{xai}-based case studies; complete generated visualization
trees remain local when they cannot be redistributed, while public manifests,
scripts, logs, and selected reporting assets remain auditable. The
\codepath{docs/LatexThesis/} directory contains the official LaTeX version of
the thesis, and \codepath{tools/latex/} contains supporting layout-audit
utilities.
...
&
Official manifest of the frozen source population, containing 1\,500 images
organized into three equally sized branches: 500 \texttt{weapon} images,
500 \texttt{non\_weapon} images, and 500 images reserved for separate
\gls{ood} evaluation. \\
...
The directory names containing \texttt{chapter5} or
...
The final source population is represented by the
\codepath{manual_selection_final_1500.csv} manifest and contains 1\,500 images
organized into three equally sized branches.

The supervised binary component comprises 500 \texttt{weapon} images and
500 \texttt{non\_weapon} images. The \texttt{weapon} class contains realistic
individual firearms and clearly recognizable firearm-related visual content,
whereas the \texttt{non\_weapon} class contains realistic negative samples
consistent with the binary task.

A separate 500-image \gls{ood} branch contains borderline, anomalous, degraded,
synthetic, semantically adjacent, or otherwise out-of-task content. Examples
include knives, swords, replicas, airsoft items, synthetic renderings, video
game images, heavy military vehicles, explosions, and war scenes. This branch
is not a third supervised class and is excluded from proxy-model training and
perturbation generation.

The official adversarial subset, stored in
\codepath{manual_selection_adversarial_subset.csv}, therefore excludes the
\gls{ood} branch and retains only the two supervised operational classes,
...
\texttt{magnet\_axiom}, \texttt{griffeye},
\texttt{excire\_foto\_2025}, and \texttt{cellebrite\_inseyets}. Its role is to normalize
...
The pipeline generates four model-dependent adversarial attacks---\texttt{fgsm},
\texttt{superdeepfool}, \texttt{sigma\_zero}, and \texttt{one\_pixel}---and
the model-agnostic \texttt{color\_shift} comparison condition. The
anti-forensic transformations are
...
model-dependent adversarial attacks evaluate decision vulnerability under the
specified access assumptions, Color Shift provides a shared model-agnostic
perturbation condition, and anti-forensic transformations simulate
operationally plausible manipulations of the image content or file that may
alter the visibility, quality, or technical traces of the
artifact~\cite{stamm2010anti,nowroozi2021survey,croce2020robustbench}.
...
10.1.0.48673, with \gls{magnetai} functionality; \gls{griffeye} x64, version
26.2.108, with the \gls{t3kcore} v1.18.0 module; \gls{excirefoto2025}, version
4.1.5, used as a stand-alone general-purpose application with \gls{ai}-based
functionality; and \gls{cellebriteinseyets}, version 10.9, with exports produced
in the \textit{Cellebrite Physical Analyzer} 10.9.0.3029 environment.
