...
software employed in \gls{df} for the automated classification or semantic retrieval of images. The theoretical framework is
built starting from the foundations of \gls{df} and digital evidence, and then
examines the role of \gls{ai} in forensic workflows, the relevance of images as
digital evidence, the principles of \gls{ml} and \gls{dl} applied to computer
vision, \textbf{\gls{advml}}, anti-forensic techniques applied to visual
content, and the role of \gls{xai} in assessing model reliability. In this
chapter, these techniques are framed at a conceptual level; the rationale for
their selection, the adopted configurations, and the procedures used to apply
them within the experimental protocol are discussed in the methodology chapter.
...
\cite{casey2011digital,garfinkel2010digital,nist2006guide}.
...
describe the digital forensic process through a sequence of logically distinct
phases: collection, examination, analysis, and reporting. The collection phase
...
hash functions, such as \gls{sha256}, recording the values, tools used,
operators, date, and conditions of the activity within the chain of custody
\cite{carrier2005filesystem,iso27037,swgde_guidelines}.
...
digital services. It can make it possible to obtain information of interest
rapidly, but it has limitations with respect to the completeness of the
recovered data and the availability of unallocated areas or deleted artifacts.
...
downstream from them, examining the reliability of automatic support applied to
...
The present thesis is situated precisely at this point in the forensic
...
triage and the examination of already acquired visual content. This distinction
is methodologically essential: proper acquisition ensures preservation of the
artifact, whereas the evaluation conducted in this work concerns the reliability
of subsequent automated selection when the image is clean, out-of-distribution,
degraded, or intentionally manipulated.

% ─────────────────────────────────────────────────────────────────────────────
\section{Artificial Intelligence in Digital Forensics}
\label{sec:background_ai_digital_forensics}
% ─────────────────────────────────────────────────────────────────────────────
The integration of \gls{ai} into \gls{df} responds to the increasing
...
phases of digital evidence \cite{garfinkel2010digital,costantini2019digital}.
...
automatic module cannot be assessed solely through aggregate accuracy metrics,
...
accurate on clean data may show a significant reduction in its classification
capability.
...
played by the automatic system, the limitations of the classification produced,
...
\textit{Image forensics} deals with the study of digital images with the aim of
...
associated with the image acquisition process. These include traces produced by
the photographic sensor of the acquisition device, such as the CMOS or CCD
sensor integrated into a camera or smartphone. In particular, \gls{prnu}
...
automatic tool to correctly recognize the depicted content.
...
deliberately to degrade the image, mask details, or make automatic analysis
more difficult.
...
automatic analysis of images and videos, with the objective of extracting
...
based on the presence or absence of investigatively relevant firearm-oriented
visual content, such as clearly recognizable realistic individual firearms.
Accordingly, the \texttt{weapon} class does not require the geometric
...
automatically learning hierarchical representations of data. In convolutional
neural networks, commonly referred to as \gls{cnn}, the first layers tend to
capture simple visual structures, such as edges, textures, and local contrasts,
...
reducing optimization degradation problems and improving the ability to learn
complex visual representations \cite{he2016deep}. Later models, such as
EfficientNet, introduced more balanced scaling strategies across depth, width,
and image resolution, with the objective of improving the relationship between
performance and computational cost \cite{tan2019efficientnet}.
...
datasets, but it also introduces new critical issues related to prompt
formulation, semantic coverage of the classes, and prediction stability.
...
support forms of automatic description of visual content and contribute to the
...
visual/semantic retrieval results, with respect to the experimental conditions
considered.
...
considered with respect to its behavior on heterogeneous, degraded, or
out-of-domain data. The presence of out-of-distribution images, borderline
cases, or visually ambiguous content may in fact produce incorrect
classifications or classifications associated with confidence levels that are
not sufficiently cautious. In this sense, evaluating behavior on \gls{ood}
inputs is closely connected to operational robustness, because it makes it
possible to observe whether images external or ambiguous with respect to the
task are mapped with excessive confidence to the categories defined by the
classifier \cite{hendrycks2017baseline,fort2021exploring}.

This setting methodologically justifies the introduction of an experimental
\texttt{ood} class, distinct from the main operational classes. In the context
of the present thesis, this class makes it possible to represent borderline,
anomalous, or not fully task-aligned content with respect to the semantic
domain of the \texttt{weapon}/\texttt{non\_weapon} task, without presupposing
the implementation of a specific \textit{OOD detection} mechanism in the
software evaluated in a black-box setting.
...
assessment in which automatic predictions are considered analytical support
...
reduce the likelihood of being correctly identified by an automatic tool.
...
the automatic production of an incorrect judicial decision, but rather in the
...
specific content will be correctly identified by an automatic tool. The main
...
automatic classification system, and they are more generally aimed at
...
applied to observe the stability of automatic support in the presence of
...
investigatively relevant content will be correctly flagged or prioritized for
...
\subsection{Adversarial and Adversarial-Style Perturbations Considered in the Experimental Protocol}
...
\textbf{\gls{superdeepfool}} is considered as an iterative attack inspired by
...
gradients, without ensuring real resistance to adaptive attacks
...
respect to the classical distinction between model-dependent adversarial
attacks and anti-forensic transformations. In this thesis, it is treated as a
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}l l X@{}}
        ...
        evaluate the effect of controlled color variations on automatic
        outputs. \\
        \bottomrule
    \end{tabularx}
...
In the present thesis, these techniques are recalled to frame the domain of
...
characteristics exploited by automatic tools. For this reason, compression is
...
In the present work, steganography is recalled as a broader component of the
...
therefore lies in distinguishing between ordinary degradations within the digital image life cycle
and transformations applied with the purpose of
...
the reliability of automatic tools, it is necessary to include such
...
its automatic processing. For this reason, the evaluation of \gls{ai}-based
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Anti-Forensic Transformations Considered in the Experimental Protocol}
...
image forensics tools and automatic classifiers \cite{barni2018cf}.
...
response of models with respect to dark, overexposed, or regions with low tonal separation.
...
automatic system also depends on the stability of its predictions, its ability
...
more complete assessment of its reliability, especially when the automatic
...
automatic errors in evidence selection. This setting makes it possible to move
...
automatic system may contribute to the selection, classification, or
...
supported by \gls{ai}-based modules, these requirements do not disappear; rather,
they also extend to the functioning of the automatic tool. It therefore becomes
necessary to document not only the origin and handling of the data, but also
the role played by the model, the conditions of use, the available metrics, the
known limitations, and any human intervention in the assessment of the result.
...
the use of AI-based systems to support forensic analysis must be assessed with
...
content. In both cases, the output of the automatic system may affect evidence
...
conditions of uncertainty, and intervene when the automatic result appears
...
The opacity of \gls{dl} models introduces further critical issues. If a system
...
In light of these profiles, the robustness of \gls{ai}-based systems also has
legal relevance, not only technical relevance. A model vulnerable to
...
automatic system may be used responsibly, the conditions that may generate
