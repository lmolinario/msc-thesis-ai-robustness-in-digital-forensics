...
maximum predicted-class probability indicators, confusion matrices,
...
bundle accuracy, weapon recall, and weapon precision of approximately 0.958
and similar overall false-negative and false-positive rates.
...
\item 500 images reserved for the separate \gls{ood} analysis.
\end{itemize}
...
\begin{itemize}
\item the \texttt{Possible weapons} tag for
\gls{magnetaxiomtool}/\gls{magnetai};
\item the \texttt{CORE/Violence/Firearm} bookmark for
\gls{griffeye}/\gls{t3kcore};
\item membership in the union of eight firearm-oriented semantic-query result
sets for \gls{excirefoto2025};
\item the exported \texttt{Armi}, \texttt{Pistola}, and \texttt{Fucile}
classification labels for \gls{cellebriteinseyets}.
\end{itemize}
...
\item false positives increasing human-review workload;
...
\item direct-target and transfer behavior under adversarial conditions;
\item residual metadata sensitivity;
...
The attribution maps explain only the selected EfficientNet-B0 output under the
adopted attribution target, baseline, and integration procedure.
...
\begin{itemize}
\item the \texttt{Possible weapons} tag for Magnet.AI;
\item the conservative \texttt{CORE/Violence/Firearm} bookmark mapping for
Griffeye/T3K CORE;
\item the eight firearm-oriented query sets and \texttt{Distance limit} for
Excire Foto 2025;
\item the exported \texttt{Armi}, \texttt{Pistola}, and \texttt{Fucile}
labels for Cellebrite Inseyets.
\end{itemize}
...
\begin{itemize}
\item certification of any software product;
\item a universal ranking of the evaluated systems;
...
The binary class distribution should also be varied. The balanced
500/\allowbreak500 population used in this thesis supports controlled comparison
...
nominal binary task. Potential extensions include:

\begin{itemize}
\item calibrated model-specific probability estimates;
\item abstention thresholds;
\item dedicated \gls{ood} detectors;
\item open-set classification methods;
\item analyst-visible unknown or ambiguous states;
\item selective-classification procedures that defer uncertain cases to human
review.
\end{itemize}
...
\gls{clip}-based proxy would permit a more systematic distinction among:

\begin{itemize}
\item direct-target vulnerability;
\item cross-architecture transferability;
\item attack-specific behavior;
\item architecture-specific robustness.
\end{itemize}
...
\begin{itemize}
\item repeated JPEG encoding;
\item platform-specific recompression;
\item multiple resize ratios;
\item cropping and partial occlusion;
\item screenshot acquisition;
\item color-space conversion;
\item stronger or weaker blurring;
\item combined tonal and geometric transformations;
\item sequential adversarial and anti-forensic manipulations.
\end{itemize}
...
outputs support more than one defensible interpretation. Examples include:

\begin{itemize}
\item broader bookmark combinations for
\gls{griffeye}/\gls{t3kcore};
\item alternative unions of exported categories for
\gls{cellebriteinseyets};
\item additional Magnet.AI tags;
\item different semantic-query sets and distance settings for
\gls{excirefoto2025}.
\end{itemize}
...
Section~\ref{subsec:implementation-explainability}. Future work should include:

\begin{itemize}
\item explicit numerical convergence criteria;
\item multiple attribution baselines;
\item larger and systematically sampled case populations;
\item comparison among different attribution methods;
\item stability analysis across checkpoints;
\item comparison between clean and manipulated variants of the same source
image;
\item quantitative measures of attribution consistency.
\end{itemize}
...
Such evaluations could compare workflows with and without automated triage and
measure:

\begin{itemize}
\item analyst review time;
\item number of images examined;
\item relevant items detected or missed;
\item frequency of automatic-output overrides;
\item effect of false positives on attention allocation;
\item effect of false negatives on final case-level findings;
\item usefulness of uncertainty, \gls{ood}, and explainability indicators.
\end{itemize}
...
\begin{itemize}
\item source and artifact schemas;
\item executable scripts;
\item configuration records;
\item software-version information;
\item manifests and stable identifiers;
\item cryptographic digests;
\item sanitized normalized outputs;
\item aggregate and condition-level metrics;
\item validation and compilation reports.
\end{itemize}
...
forensic workflows. They define the conditions under which such use is more
defensible:

\begin{itemize}
\item task-specific validation on representative data;
\item explicit definition of the evaluated threat model;
\item documentation of software and model versions;
\item configuration and mapping management;
\item separate analysis of \gls{ood} behavior;
\item preservation of source-to-output traceability;
\item visibility of false negatives and false positives;
\item systematic human verification.
\end{itemize}
