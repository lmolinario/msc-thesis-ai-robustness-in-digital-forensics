...
Chapter~\ref{chap:state-of-the-art} by formalizing \gls{fairlab} as a
reproducible and auditable experimental protocol that combines controlled
experimental conditions with operationally relevant forensic scenarios.

The chapter presents the overall methodology and research assumptions, followed
...
The \gls{fairlab} protocol defines a structured methodological procedure for evaluating
...
that do not constitute clean instances of either binary class. Since classifiers
may produce predictions with high maximum predicted-class
probabilities outside their training distribution
\cite{hendrycks2017baseline,fort2021exploring}, these inputs must
...
Although the review itself does not rely on automated pre-labels,
retaining explicit human control over semantic assignments is consistent
with trustworthy-AI guidance, which emphasizes human agency and oversight
in the use of AI systems \cite{EUAI2020}.
...
defined in Section~\ref{sec:methodology-traceability-controls}.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Proxy-Model Evaluation Methodology}
...
classification behavior, robustness degradation, maximum predicted-class
probability patterns, error direction, and transferability under reproducible
...
The proxy-model evaluation methodology serves three methodological
objectives. First, it provides a transparent reference for measuring
classification performance and robustness degradation. Second, it enables the
generation of model-dependent adversarial examples under explicit white-box or
score-based access assumptions. Third, it supports the evaluation of whether
perturbations generated against one controlled model transfer to other proxy
architectures or to the selected black-box software systems.
...
The perturbation methodology evaluates classification behavior under controlled
...
than multiple near-duplicate variants of the same mechanism. Its selection
should account for diversity in access assumptions and algorithmic mechanisms,
while also considering computational feasibility, scalability, and
reproducibility across experimental runs.

The selected conditions should preserve sufficient task-relevant semantic
content and retain operational plausibility. They should also support
cross-system comparison and, where applicable, transferability analysis, while
remaining compatible with the evaluated image formats and software systems.
...
Each generated artifact must remain explicitly associated with the original clean
sample, its reference operational class, and the corresponding experimental
fold. The retained information must also identify the attack family or
model-agnostic condition, as well as the source model, checkpoint, and access
assumption when applicable. In addition, the generation parameters and resulting
output artifact must be recorded together with all information necessary to
reconstruct the generation condition and to verify the identity, integrity, and
...
For each evaluated system, the protocol follows the same general workflow. Only
the blind evaluation inputs are imported or indexed, after which the relevant
automated analysis or retrieval functionality is enabled. The complete blind
input view is then processed under a documented configuration, and all observable
outputs required for matching and interpretation are exported. These outputs are
subsequently aligned with the experimental control metadata and normalized into
common operational prediction states. Comparable metrics are then computed while
preserving any outcomes classified as \texttt{unknown}.
...
The \gls{fairlab} evaluation metrics cover three complementary dimensions of
...
cannot be independently verified. The protocol therefore prioritizes measures
...
For each proxy model and manipulated condition, the maximum-probability shift
is defined as:
...
where \(c\) denotes the mean maximum predicted-class probability under the
indicated condition. Because the predicted class may change, this quantity does
not necessarily compare the probabilities assigned to the same semantic class.
...
meets or exceeds a predefined high-Max-P threshold;
...
Maximum-probability-based \gls{ood} indicators are available only for proxy
models. They remain diagnostic intra-model quantities because the selected
black-box systems do not expose homogeneous numerical outputs suitable for an
equivalent cross-system probability analysis.
...
For the selected black-box software systems, the protocol evaluates the matching
rate between exported outputs and blind-bundle inputs and computes the
true-positive, false-negative, true-negative, and false-positive counts on
interpretable records. These counts support the calculation of weapon recall,
the weapon false-negative rate, the false-positive rate, and, where permitted by
the normalized outputs, accuracy and balanced accuracy. The evaluation also
records the number and rate of \texttt{unknown} outcomes, together with any
system-specific not-detected or uncategorized rates exposed by the software and
any supplementary operational miss counts that are explicitly defined.
...
prediction transitions, and intra-model maximum-probability variation when
available. \\
...
maximum-probability indicators. The branch remains excluded from binary
...
Figure~\ref{fig:methodology-metric-layers} summarizes the metric structure
...
\scriptsize binary predictions and intra-model Max-P diagnostics
}};
...
\scriptsize unknown outcomes, and proxy-only Max-P indicators
}};
...
\caption[Metric structure]{Metric structure. Condition-specific binary,
...
High-Max-P criteria are applied only within the selected proxy model and serve
exclusively as case-selection indicators. They must not be interpreted as
calibrated probabilities or compared directly with numerical outputs from
...
which the model retains a high maximum predicted-class probability. \\
...
The adopted procedure should be compatible with the selected inspectable proxy
model and should explicitly document the attribution target and reference
baseline whenever required by the chosen method. The attribution process must
be reproducible, applied consistently across the selected clean and manipulated
cases, and traceable to the corresponding model checkpoint, input sample, and
prediction record. The resulting outputs must also be presented as diagnostic
attribution evidence rather than as definitive causal explanations.
...
The traceability structure distinguishes four complementary elements. The first
is \textit{sample identity}, which establishes a stable reference to the
original image. The second is \textit{artifact lineage}, which records the
relationship between an original sample and any derived adversarial,
anti-forensic, blind-bundle, or explainability artifact.

The third element is \textit{experimental context}, which associates each input
or derived artifact with its reference operational assignment, partition, model
configuration, perturbation condition, and evaluation branch. The fourth is
\textit{output provenance}, which connects model predictions, software exports,
normalization decisions, metrics, and qualitative case studies to the
...
Automated consistency controls should verify, where applicable, the
correspondence between expected and generated artifacts, the uniqueness of
stable and derived identifiers, and the integrity of the generated files. They
should also detect missing or unresolved paths and confirm consistency among
source records, derived artifacts, and evaluation outputs. In addition, the
controls should assess the completeness of software exports and normalized
predictions while preserving a clear separation between blind inputs and
