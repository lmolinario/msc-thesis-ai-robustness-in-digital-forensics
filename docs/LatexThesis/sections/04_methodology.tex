...
proxy models. The second level applies the same experimental samples and
derived artifacts to selected \gls{ai}-based software systems evaluated under
black-box or near-black-box access conditions. This parallel design supports a
structured comparison between experimentally controlled model behavior and the
observable operational behavior of the selected software systems, without
assuming access to their internal architectures or decision processes.
...
first is a controlled evaluation based on the proxy classification models
formalized in Section~\ref{sec:methodology-models-classification}. These fully
inspectable models provide a reproducible experimental reference without being
treated as replicas or functionally equivalent substitutes for proprietary
systems.
...
operational labels \texttt{weapon} and \texttt{non\_weapon}. The \gls{ood}
...
for the \texttt{weapon}/\texttt{non\_weapon} task.
...
annotation must not be treated as a definitive reference assignment. These
...
The candidate pool is subsequently subjected to a documented
