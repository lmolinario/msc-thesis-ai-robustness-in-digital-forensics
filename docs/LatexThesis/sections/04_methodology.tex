...
The protocol integrates dataset construction, human validation, controlled
...
uses fully inspectable experimental reference models, hereafter referred to as
proxy models, to study controlled classification behavior, robustness
degradation, transferability, confidence patterns, and error direction. These
models are not intended to replicate, reconstruct, or reverse engineer the
architectures or decision processes of proprietary systems. The second level
applies the same experimental samples and derived artifacts to selected
\gls{ai}-based software systems evaluated as black-box or near-black-box
components. This parallel design supports a structured comparison between
experimentally controlled model behavior and the observable operational
behavior of the selected black-box software systems, without assuming access
to their internal architectures or decision processes.
...
automated image-classification systems under clean, out-of-distribution,
adversarial, and anti-forensic conditions. Its purpose is to make the evaluation
process reproducible and auditable while remaining sufficiently general to
accommodate different datasets, classification models, software systems, and
...
property of a classifier, but also in terms of its potential operational
...
confident in-domain labels to out-of-distribution inputs.
...
In this thesis, this term denotes fully inspectable experimental reference
models used to study controlled classification behavior, robustness
degradation, transferability, confidence patterns, and error direction under
reproducible conditions. These models are not replicas, functionally equivalent
substitutes, or reverse-engineered reconstructions of the proprietary
architectures or decision processes of the evaluated software systems. Instead,
they provide a controlled reference through which the effects of the selected
evaluation conditions can be systematically examined.

The second level consists of the operational evaluation of selected
\gls{ai}-based software systems under black-box or near-black-box access
conditions. At this level, the systems are assessed exclusively through their
observable inputs and outputs. No assumptions are made regarding their internal
model architectures, training data, confidence calibration, or decision logic.
This distinction is particularly relevant for proprietary systems whose
internal components are only partially documented or are not accessible to the
evaluator \cite{sanna2024preliminary,sanna2025pixelperfect}.
...
selected black-box software systems are not caused by inconsistencies in the
evaluated data. The corpus includes a binary in-domain component used for clean
and perturbed evaluation, together with a separate \gls{ood} branch. The latter
is not treated as a third supervised class. Instead, it is used to examine
whether out-of-distribution, borderline, synthetic, or otherwise
distributionally different inputs are assigned to the operational classes of
the binary task.
...
input manipulation. The resulting clean samples, clean \gls{ood} inputs,
adversarial artifacts, and anti-forensic artifacts form a common evaluation
bundle. This bundle supports both the controlled analysis of the proxy
classification models and the operational analysis of the selected black-box
software systems.
...
robustness degradation. Explainability methods support the qualitative
examination of representative decisions and failure cases for a selected
inspectable proxy model; they are not extended to the selected black-box
software systems. Traceability controls preserve the relationships among source
samples, derived artifacts, experimental conditions, and observed outputs.
...
\caption[FAIR-Lab experimental protocol]{FAIR-Lab experimental protocol. The
...
operational labels \texttt{weapon} and \texttt{non_weapon}. The \gls{ood}
designation identifies samples reserved for a separate analysis of
distributional mismatch. These designations must not be interpreted as legal
qualifications of the depicted content or as universal semantic categories.
They are operational definitions established specifically for the experimental
protocol adopted in this thesis.
...
firearm-related target category is visible. The \texttt{non_weapon} class
...
synthetic, borderline, drawn from a different visual distribution, or affected
by degradation already present in the source material. The \gls{ood} branch is
therefore not modeled as a third supervised class within the binary task.
...
The assignment criteria were formalized before the experimental corpus was
...
The \texttt{exclude} designation does not represent an additional analytical
category. It functions exclusively as a quality-control disposition during
...
distinguish errors involving the known binary classes from predictions produced
for samples that should not be treated as ordinary positive or negative
instances.

The \gls{ood} branch must not be interpreted as a homogeneous semantic class or
as a subclass of \texttt{non_weapon}. It is a methodological collection of
distributionally different and borderline cases used to assess how an automated
system assigns such inputs relative to the known operational classes. For this
reason, \gls{ood} samples remain separate from the binary subset. They are not
subjected to the adversarial attacks or anti-forensic transformations generated
for the \texttt{weapon}/\texttt{non_weapon} task.

The methodology further distinguishes among classification performance,
robustness, and operational evaluation. Within the binary task, classification
performance concerns the ability of a system to assign the expected label to
an image. Robustness concerns the stability of that behavior when the input is
degraded, altered, or deliberately manipulated. Operational evaluation concerns
the consequences of the observed output for the forensic workflow.
...
and in-domain assignments produced for \gls{ood} samples. This distinction
prevents performance on clean data from being interpreted as a sufficient
guarantee of operational robustness under investigative conditions.
...
a sufficiently varied candidate pool that captures part of the diversity
...
dataset and the requirements of the experimental environment. For example,
minimum dimensional requirements may differ depending on the classification
models or software systems being evaluated. The methodology therefore specifies
...
tabular files, structured documents, databases, or tool-specific formats,
provided that traceability and consistency are maintained.

Technical preparation does not determine the final semantic assignment. In
particular, the source location, directory name, file name, or any pre-existing
annotation must not be treated as definitive experimental ground truth. These
elements may be retained as descriptive or audit variables, but semantic
assignment must be based on the explicit operational criteria defined for the
experiment.
...
dominated by a single repository or acquisition method. Sampling and
composition-control decisions must therefore be based on the objectives of the
experimental design, the required class composition, and the need to preserve
...
the review status, operational assignment, exclusion disposition, reviewer
...
technology, the representation must provide a stable association among each
...
human-in-the-loop review. The final operational assignment is determined
exclusively according to the criteria defined in
...
supports the consistent application of the operational assignment criteria.
Second, it preserves the rationale underlying inclusion, exclusion, and
class-assignment decisions. Third, it provides the traceability required to
reconstruct the transition from the initial candidate pool to the frozen
...
\texttt{weapon}, the in-domain negative class \texttt{non_weapon}, the
separate \gls{ood} branch, or the review-only \texttt{exclude} disposition.
...
\texttt{non_weapon} with sufficient semantic clarity. Borderline, anomalous,
synthetic, replica-like, or otherwise distributionally different content is
retained within the separate \gls{ood} branch rather than being forced into one
of the binary classes. The branch may also include images affected by
degradation already present in the source material. Such images remain distinct
from the adversarial and anti-forensic artifacts generated during subsequent
experimental phases. Unusable or semantically irrelevant samples are excluded.

The resulting assignments are operational designations defined for the
experimental protocol. They must not be interpreted as definitive
determinations of the legal, material, or evidentiary nature of the depicted
objects. This distinction is particularly important for visually ambiguous
cases, for which assignment to the \gls{ood} branch represents a deliberate
...
to dataset freezing. Freezing establishes a stable reference dataset whose
composition must remain unchanged throughout the subsequent evaluation phases.
Any correction or modification introduced after freezing must therefore be
explicitly documented and treated as a new version of the experimental corpus.
...
\gls{ai}-based software systems. This shared foundation ensures that differences
between the two evaluation levels are not caused by inconsistencies in dataset
composition.

The binary evaluation subset is derived from the frozen corpus by retaining
only the \texttt{weapon} and \texttt{non_weapon} samples. This subset supports
...
experimental question: how an automated system assigns anomalous, borderline,
or distributionally different inputs relative to the known operational classes.
...
Explicit decision criteria, structured review records, and a frozen dataset
nevertheless make the process auditable and support reproducibility at the
procedural level. They do not, by themselves, eliminate the subjectivity
associated with semantic assessment.
...
\texttt{weapon} or \texttt{non_weapon} sample relative to the known
...
\textit{proxy classification models}. These models serve as experimental
reference models for studying controlled classification behavior, robustness
degradation, transferability, confidence patterns, and error direction under
reproducible conditions.

Proxy classification models are not intended to replicate, reconstruct, or
reverse engineer the internal architectures or decision processes of the
selected black-box software systems. They are also not functionally equivalent
substitutes for those systems. Their role is to provide an experimental
environment in which model architecture, training procedure, class mapping,
checkpoints, and decision behavior can be inspected and controlled.
...
\texttt{non_weapon} and \texttt{weapon}. Their numerical encoding is an
...
\draw[dashedarrow] (ood.south) |- (bundle.east);
...
\mathrm{Precision}
\cdot
\mathrm{Recall}\textit{{\text{\texttt{weapon}}}
}{
\mathrm{Precision}
+
\mathrm{Recall}}{\text{\texttt{weapon}}}
}
\end{equation}
...
\section*{\mathrm{Accuracy}_{\mathrm{clean}}}
\mathrm{Accuracy}_{p}
\end{equation}
...
\left|
\left{
i \in \mathcal{C}\textit{{\mathrm{clean}}
:
\hat{y}^{(p)}}{i} \neq y_{i}
\right}
\right|
...
For samples whose reference operational class is \texttt{non_weapon}, an
...
for the \texttt{weapon} and \texttt{non_weapon} classes. \
...
\texttt{non_weapon} samples, interpreted according to their different effects
