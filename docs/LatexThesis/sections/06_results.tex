...
with particular attention to false negatives on the investigatively relevant
\texttt{weapon} class, false positives on \texttt{non\_weapon} content,
operational assignments of \gls{ood} inputs, degradation under manipulated
conditions, and the continued need for human review
...
common corpus contains 11\,500 distinct image artifacts:

\begin{equation}
\begin{aligned}
    N_{\mathrm{bundle}}
    &= 1\,000\ \text{clean binary images}
    + 500\ \text{\gls{ood} images} \\
    &\quad + 5\,000\ \text{adversarial variants}
    + 5\,000\ \text{anti-forensic variants} \\
    &= 11\,500.
\end{aligned}
\end{equation}

The proxy-model and black-box evaluations use different output representations
but cover the same experimental conditions. Each clean or perturbed binary
artifact is evaluated by all three proxy architectures through its assigned
...
Accuracy drop, attack success rate, induced-error counts, directional error
transitions, and the confusion-matrix layout follow the definitions established
in Section~\ref{sec:methodology-evaluation-metrics}. Positive accuracy-drop
...
exhibits a different operational profile from the two fully fine-tuned CNN
...
zero-shot mechanism. Within the \gls{clip}-based proxy, the observed score distribution indicates
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Out-of-Distribution Behavior}
\label{sec:results-ood}
% ─────────────────────────────────────────────────────────────────────────────

The \gls{ood} evaluation examines how the three proxy architectures behave on
the separate 500-image out-of-distribution branch. As established in
...
This behavior indicates a semantic tendency toward the \texttt{weapon} class
...
\caption{Distribution of predictive confidence on \gls{ood} samples for each proxy model. The plot highlights a distribution more concentrated around intermediate values for \gls{clip} than for the two fully fine-tuned CNN proxies}
...
risk that differs from ordinary binary misclassification. An operational \texttt{weapon} assignment
...
ambiguous content. The behavior of ResNet18 and EfficientNet-B0 shows that a model
...
\gls{cellebriteinseyets}. Every configuration produced one normalized decision
for each of the 11\,500 bundle items. Therefore, no bundle item remained without
an interpretable black-box output, although non-bundle rows present in some raw
...
through the selected proprietary systems therefore constitutes a black-box
...
1\,000 clean images and 10\,000 perturbed variants. They describe observable
...
excluding \gls{ood} samples. \textit{OOD W-rate} indicates the proportion of
\gls{ood} samples operationally mapped to the \texttt{weapon} class. The values
are bundle-weighted and reflect the experimental composition of the bundle, not a general validation of the selected software systems.
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/fig_forensic_tools_global_metrics_comparison.pdf}
\caption{Global comparison of the aggregate metrics of the selected black-box
software systems on the forensic evaluation bundle. The plot compares accuracy,
balanced accuracy, \texttt{weapon} recall, bundle \acrshort{fnr}, bundle
\acrshort{fpr}, and \gls{ood} weapon flag rate after output normalization}
...
Figure~\ref{fig:results-forensic-tools-ood-weapon-flag-rate} confirms that every
selected configuration maps a non-zero proportion of \gls{ood} inputs to the
operational \texttt{weapon} class. This behavior may increase human-review
workload or assign investigative priority to content that is semantically
ambiguous or external to the nominal task.

\subsection{Magnet AXIOM and Magnet.AI Results}
...
interpreted as standard classification errors. However, the fact that 180
...
the output. This characteristic prevents direct comparison with the confidence
distributions of the proxy models and constitutes an operational limitation for
the traceability and interpretability of the output.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Magnet Griffeye and T3K CORE Results}
...
decreases to 0.926. This result suggests residual sensitivity to sparse or
localized perturbations, although without a performance collapse comparable to
that observed in some proxy models.
...
equal to 0.951, remains high but is not the highest. Indeed, \gls{cellebriteinseyets} maintains slightly higher recall on
...
tendency of the system to map out-of-distribution or semantically
...
represent a binary error in the strict sense, because \gls{ood} samples do not
belong to the \texttt{weapon}/\texttt{non\_weapon} ground truth; rather, it
describes the degree of operational absorption of out-of-distribution inputs
into the binary triage task.
...
perturbed, out-of-distribution, or semantically ambiguous inputs.
...
a combination of the two factors. These cases are therefore interpreted as part
of the broader operational risk associated with \gls{ood} and semantically
weapon-adjacent inputs.
...
and discussed as a documented operational limitation of bundle blindness, not
as an invalidating factor for the experimental results. The residual presence of embedded metadata therefore remains a documented
limitation of the blind-bundle design and must be considered when interpreting
the affected clean and \gls{ood} results.
...
clean, \gls{ood}, adversarial, and anti-forensic artifacts and which error
profiles may affect forensic triage.
...
the binary task, whereas the OOD weapon rate measures how frequently images
...
read jointly: a system may achieve good accuracy on clean data while still
producing operational risk if it tends to absorb anomalous inputs into the
class of investigative interest.


\begin{table}[!htbp]
\centering
\caption{Comparison between clean performance and OOD behavior of the proxy models and selected black-box software systems}
...
on the \texttt{weapon} class; however, a non-negligible proportion of OOD
...
0.012 &
0.836 &
Lower clean accuracy than the two fully fine-tuned CNN proxies, but a very limited FNR; the
main operational risk is the high weapon flag rate on OOD inputs. \\

Magnet AXIOM and Magnet.AI &
Black-box software &
0.950 &
0.070 &
0.360 &
...
Magnet Griffeye and T3K CORE &
Black-box software &
0.978 &
0.036 &
0.260 &
Highest clean accuracy among the selected software systems under the adopted
...
selected software systems with good clean accuracy exhibit non-zero weapon flag
...
\gls{magnetai} reaches an OOD weapon flag rate of 0.360,
...
cross-system comparison, to absorb out-of-distribution samples into the
\texttt{weapon} class.
...
shows that \gls{excirefoto2025} under the \texttt{D80} setting reaches an OOD
...
reported values must not be interpreted as evidence of actual OOD recognition
...
criticality more attributable to operational noise than to missed detection of
\texttt{weapon}. \\
...
Good aggregate black-box profile under anti-forensic transformations, although
...
34 FN &
Most selective black-box profile under anti-forensic transformations, with a
...
indicates transitions from \texttt{weapon} to \texttt{non\_weapon}; FN
indicates false negatives; FNR indicates false negative rate; FPR indicates
false positive rate.
\end{table}
...
anti-forensic degradation, but the tendency to absorb \gls{ood} inputs into the
\texttt{weapon} class.
...
Limited aggregate degradation; the model-dependent results reflect cross-model
transfer and do not demonstrate direct adversarial robustness. The distinct OOD
risk also remains. \\
...
53 FN &
Good aggregate black-box stability even in the adversarial scenario, with an
...
37 FN &
Highest aggregate black-box accuracy in the adversarial scenario; the low
...
different risk profiles. \gls{griffeye}/\gls{t3kcore} exhibits the most
...
profile but with greater false negative risk. This heterogeneity confirms that
the operational robustness of selected black-box software systems is
system-dependent and must be interpreted with respect to the type of observable
...
configuration, accuracy decreases to 0.742, while FNR improves to 0.074 and
FPR increases to 0.442. Under the \texttt{D80} configuration, recall improves
further, but FPR rises to 0.662. The same software system can therefore support
...
reflects the real condition in which forensic analysts interact with selected
proprietary \gls{ai}-assisted software systems.


Figure~\ref{fig:results-operational-risk-summary} summarizes the main
quantitative risk dimensions discussed in the comparative analysis. The figure
...
\begin{figure}[H]
   ...
   \caption[Operational risk summary across evaluated systems]{Operational risk
        summary across proxy models and selected black-box software systems. The figure
        reports quantitative indicators relevant to forensic image triage: clean false
        negative rate, bundle false positive rate on the interpretable binary bundle,
        out-of-distribution weapon flag rate, adversarial false negative rate,
        anti-forensic false negative rate, adversarial accuracy drop, and anti-forensic
        accuracy drop. The values are derived from the proxy-model metrics and the
        normalized black-box metrics reported in Chapter~\ref{chap:results}.
        Interpretability differences between inspectable proxy models and black-box
        software systems are discussed separately and are not encoded as a numerical
        score in the figure}    \label{fig:results-operational-risk-summary}
...
\begin{flushleft}
\footnotesize
\textit{Note.} Clean FNR is computed on the unperturbed binary subset. Bundle
FPR is computed on the interpretable binary bundle, including clean,
adversarial, and anti-forensic samples and excluding \gls{ood} samples.
Adversarial FNR and anti-forensic FNR are aggregated across the corresponding
perturbation families. OOD W-rate reports the proportion of \gls{ood} samples
...
automatically transferred to selected software systems, which adopt proprietary
mechanisms for classification, tagging, retrieval, or semantic bookmark
generation. Second, selected software systems show differentiated risk
profiles: some privilege recall, others reduce false positives, and others
depend strongly on the retrieval configuration or the adopted semantic mapping.
Third, \gls{ood} behavior remains a distinct operational risk: it may increase the
analyst's workload, produce misleading prioritization, and generate
weapon-related flags on material that does not belong to the expected binary
task.
...
highlighting which image regions contributed to the classifier decision in
specific success or failure scenarios.
...
heatmap, attribution overlay on the original image, and mask of the regions with
the highest attribution. The attribution mask must not be interpreted as a
semantic segmentation or as object ground truth, but as a visual summary of the
areas that most contributed to the proxy-model prediction. Similarly, the
reported confidence is the proxy-model score and not a calibrated probability.
...
out-of-distribution evaluation branch and not a third supervised class.
\end{table}
...
risk that differs from an ordinary false positive. The sample belongs to the separate \gls{ood} evaluation branch, but it is
absorbed by the binary decision boundary and classified
...
and less semantically stable correlations. From a methodological perspective, this case supports the choice to preserve
\texttt{ood} as a separate dataset and evaluation branch, rather than treating
...
confidence. The case is relevant because the transformation is not an adversarial
...
transformations that may remain visually recognizable to human analysts can
...
failure directly observable, showing that an apparently certain prediction may
still derive from unreliable input conditions. In an investigative context, this
...
\gls{ood} case highlights the risk of absorbing out-of-distribution inputs into
the \texttt{weapon} class; the anti-forensic case shows that low-level visual
...
content from being prioritized for review. A false positive, or an operational
\texttt{weapon} assignment on an \gls{ood} input, may instead increase analyst
workload, dilute prioritization, and assign investigative relevance to ambiguous
content. Aggregate accuracy therefore provides only a partial description of
the forensic risk profile.
...
\begin{itemize}
    ...
    out-of-distribution imagery encountered in operational investigations.
    ...
    must not be interpreted as reconstructions of the proprietary models used
    ...
    proportions support operational comparison but do not constitute identical
    ...
    the proprietary black-box systems.

    \item The embedded-metadata audit identified a small residual information
    channel in the blind bundle. The leave-out analysis indicates negligible
...
These limitations do not invalidate the experimental findings; they delimit the
conditions under which the conclusions are supported. The chapter therefore
provides a protocol-specific assessment of operational robustness rather than a software-system certification, universal robustness ranking, or
...
Their outputs should remain associated with the source artifact, system version,
