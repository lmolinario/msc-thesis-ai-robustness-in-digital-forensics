...
operational configurations. Magnet AXIOM/Magnet.AI and Griffeye/T3K CORE each
contribute one configuration; the D20, D50, and D80 Excire Foto 2025 settings
are retained as three separate configurations; and Cellebrite Inseyets
contributes one configuration. Each configuration produces one normalized decision for every
...
5
&
One purposively selected case for each predefined diagnostic category,
associated with 20 thesis-level visual assets. \\
...
\gls{efficientnetb0} achieves the strongest overall clean-condition performance,
...
\gls{resnet18} shows slightly lower but still high clean-condition performance, with
...
\gls{fpr} of 0.134, substantially higher than those of \gls{efficientnetb0} and
\gls{resnet18}.
...
Figure~\ref{fig:results-clean-confusion-matrices}. \gls{efficientnetb0} minimizes
the total number of errors, producing 22 incorrect decisions. \gls{resnet18} produces
...
the direction of the errors. \gls{efficientnetb0} provides the strongest overall
balance, with eight false negatives and 14 false positives. \gls{resnet18} produces
...
Two principal indicators are considered. The \emph{weapon assignment rate}
corresponds to the proportion of the 2\,500 responses assigned to
\texttt{weapon}. The \emph{high-maximum-probability rate} instead represents
the proportion of responses whose maximum predicted-class probability is at
least 0.90.
...
\gls{efficientnetb0} and \gls{resnet18} assign lower proportions of the \gls{ood}
responses to \texttt{weapon}. \gls{efficientnetb0} produces 924 such responses,
corresponding to a rate of 0.3696, whereas \gls{resnet18} produces 1\,094, for a rate
of 0.4376.

Their model-specific output distributions nevertheless contain substantially
more responses above the 0.90 threshold. \gls{efficientnetb0} produces 1\,252
high-maximum-probability responses, corresponding to a rate of 0.5008.
\gls{resnet18} produces 1\,617, corresponding to the highest rate among the three
...
\gls{resnet18} exhibits the highest high-maximum-probability rate despite assigning
fewer responses to \texttt{weapon}. \gls{efficientnetb0} presents an intermediate
...
represent the same degree of uncertainty for \gls{efficientnetb0}, \gls{resnet18}, and the
...
proxy remain below the predefined 0.90 threshold, whereas \gls{efficientnetb0} and
\gls{resnet18} produce substantial populations above it. This difference does not
...
separate behaviors. The first is the tendency to route distributionally
different material toward the firearm-oriented review category. The second is
the tendency to produce high maximum predicted-class probabilities for forced
closed-set assignments.
...
\gls{efficientnetb0} remains above 0.965 accuracy under every transformation.
...
\gls{resnet18} is most affected by Histogram Modification. Its accuracy decreases
...
For \gls{efficientnetb0}, Gaussian Blur introduces 17 errors among samples that were
...
For \gls{resnet18}, Histogram Modification introduces 39 errors and produces the
...
degradation pattern. Gaussian Blur primarily affects \gls{efficientnetb0} through
new \texttt{weapon}-to-\texttt{non\_weapon} transitions, whereas Histogram
Modification produces the largest effects for \gls{resnet18} and the most relevant
...
\gls{efficientnetb0} checkpoints. \gls{fgsm}, \gls{sigmazero}, and
...
Evaluation of these artifacts on \gls{efficientnetb0} constitutes direct-target
assessment.

The same artifacts were subsequently evaluated on \gls{resnet18} and the
...
against \gls{resnet18} or \gls{clip}
...
\gls{efficientnetb0} checkpoints. Their evaluation on \gls{resnet18} and the
...
\gls{efficientnetb0} column reflects the fact that the four model-dependent attacks
...
The most severe result is observed for \gls{efficientnetb0} under
...
\gls{fgsm} also produces severe direct-target degradation. \gls{efficientnetb0}
...
\gls{superdeepfool} has a smaller direct-target effect. \gls{efficientnetb0}
...
\gls{efficientnetb0}, one in each direction, corresponding to an ASR of 0.0020.
...
largest negative shift among the selected \gls{efficientnetb0} conditions, at
...
\gls{efficientnetb0} do not produce equivalent effects on the other proxy
architectures. The largest transferred degradation occurs under FGSM.
\gls{resnet18} accuracy decreases from 0.964 to 0.918, corresponding to a drop of
...
\gls{resnet18} accuracy increases by 0.004 and the \gls{clip}-based proxy accuracy
increases by 0.017. These aggregate increases nevertheless coexist with three
induced errors for \gls{resnet18} and eight for \gls{clip}. The result again confirms
...
SuperDeepFool induces no errors among clean-correct \gls{resnet18} samples and three
among clean-correct \gls{clip} samples. One Pixel induces one error for
\gls{resnet18} and none for \gls{clip} under the frozen evaluation.
...
architecture. \gls{efficientnetb0} achieves the highest clean accuracy but undergoes
...
\gls{resnet18} and the \gls{clip}-based proxy.
...
\gls{efficientnetb0}-oriented perturbations were less effective against the other
proxy models. A direct attack generated against \gls{resnet18} or the
...
\gls{excirefoto2025} under the \texttt{D20}, \texttt{D50}, and
\texttt{D80} semantic-distance settings, and
...
analytically distinct populations. The first consists of 11\,000 binary items
with a reference operational assignment of \texttt{weapon} or
\texttt{non\_weapon}. The second consists of 500 \gls{ood} inputs, which are
analyzed separately through their observable weapon assignment rate.
...
fold-specific \gls{efficientnetb0} checkpoints. Their evaluation through the
...
0.038
&
Aggregate transfer behavior across four \gls{efficientnetb0}-oriented attacks and
...
generated against fold-specific \gls{efficientnetb0} checkpoints and were only
...
were generated against fold-specific \gls{efficientnetb0} checkpoints. Their
...
\gls{efficientnetb0} target. This difference is evidence of limited transfer under
...
Within the evaluated range, \texttt{D20} represents the most restrictive
retrieval configuration, \texttt{D50} represents the intermediate
configuration, and \texttt{D80} represents the most permissive configuration.
...
populations generated against \gls{efficientnetb0} and the model-agnostic Color
...
\gls{efficientnetb0} checkpoints and subsequently processed by Excire. Their effects
...
against fold-specific \gls{efficientnetb0} checkpoints. Their evaluation through
...
only that artifacts generated against \gls{efficientnetb0} do not reproduce their
...
negatives and a higher \gls{ood} weapon assignment rate than \gls{efficientnetb0}. \\
...
profile. \gls{efficientnetb0} and Griffeye/T3K CORE both achieve a clean accuracy of
0.978, but their outputs arise from different systems and mappings.
\gls{efficientnetb0} is a supervised transparent proxy, whereas the Griffeye result
uses only the \texttt{CORE/Violence/Firearm} bookmark as the positive observable
output.

Cellebrite Inseyets and \gls{resnet18} both achieve a clean accuracy of 0.964, but
...
differs by system: \gls{efficientnetb0} and Magnet.AI show increased missed-target
risk under blurring; \gls{resnet18} produces bidirectional errors under Histogram
...
fold-specific \gls{efficientnetb0} checkpoints. Their evaluation on \gls{efficientnetb0}
constitutes direct-target assessment. Their evaluation on \gls{resnet18}, the
...
& Limited family-level transfer from \gls{efficientnetb0}-oriented artifacts; this is
not a direct white-box assessment of \gls{resnet18}. \\
...
robustness against attacks optimized directly against the \gls{clip}-based proxy. \\
...
\vspace{0.4em}
\footnotesize
\textit{Note.} The adversarial family contains four \gls{efficientnetb0}-oriented
model-dependent artifact populations and the model-agnostic Color Shift
condition. Only the \gls{efficientnetb0} rows represent direct-target evaluation for
...
\gls{efficientnetb0} under Sigma-Zero. Accuracy decreases from 0.978 to 0.015, and
...
FGSM also produces severe direct-target degradation for \gls{efficientnetb0},
...
The corresponding artifacts transfer only partially to \gls{resnet18} and the
...
The black-box systems do not reproduce the \gls{efficientnetb0} Sigma-Zero collapse.
...
configurations. These comprise the clean \gls{fnr}, the bundle-level
\gls{fpr}, the \gls{ood} weapon-assignment rate, and the adversarial- and
anti-forensic-family \gls{fnr} values. The figure also reports the degradation
in accuracy produced by adversarial and anti-forensic conditions relative to the
corresponding clean baseline.
...
No row simultaneously minimizes all seven indicators. \gls{efficientnetb0} combines
a low clean \gls{fnr} with severe direct-target adversarial degradation.
\gls{resnet18} exhibits a more moderate profile under the transferred attacks but a
higher \gls{ood} weapon assignment rate than \gls{efficientnetb0}.
...
\gls{efficientnetb0} results describe attacks optimized directly against its
fold-specific checkpoints. The corresponding \gls{resnet18}, \gls{clip}, and
...
assessment.
% ─────────────────────────────────────────────────────────────────────────────
\section{Explainability Analysis and Purposively Selected Cases}
...
individual \gls{efficientnetb0} outputs. It is not treated as a primary robustness
metric, a quantitative validation of model reliability, or an evidentiary
interpretation of the analyzed images.

The analysis is limited to the transparent \gls{efficientnetb0} proxy. It does not
explain the behavior of \gls{magnetaxiomtool}/\gls{magnetai},
\gls{griffeye}/\gls{t3kcore},
\gls{excirefoto2025}, or
\gls{cellebriteinseyets}. These systems remain black-box within the
...
output actually produced by \gls{efficientnetb0}.
...
Each retained case comprises four visual assets: the model input, the
\gls{ig} attribution-magnitude heatmap, the attribution overlay on the input,
and a mask retaining the image locations associated with the largest 10\% of
absolute attribution magnitudes.
...
examples for each of five diagnostic categories. One case from each category
was subsequently included in the thesis through a purposive selection process.
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{3.2pt}
\caption{Purposively selected \gls{efficientnetb0} cases for qualitative analysis
using \gls{ig}}
...
& \texttt{weapon}
& 1.000
& Direct-target adversarial false positive illustrating that a maximum
...
produced by \gls{efficientnetb0}. It is not treated as calibrated. For
...
{Correct prediction on clean input \texttt{xai\_case\_0001}. \gls{efficientnetb0}
...
\gls{efficientnetb0} prediction agrees with the reference operational assignment.
...
whereas \gls{efficientnetb0} predicts \texttt{non\_weapon} with a maximum
...
{\gls{ood} case \texttt{xai\_case\_0009}. \gls{efficientnetb0} assigns an
...
remains \texttt{weapon}, whereas \gls{efficientnetb0} predicts
...
is not optimized against the \gls{efficientnetb0} decision boundary. It is a
...
Sigma-Zero false positive in the direct-target \gls{efficientnetb0} evaluation. The
...
The result requires a clear distinction among the predicted class, the maximum
predicted-class probability, and the operational reliability of the
prediction. These three quantities must not be conflated.
...
workflow, the figures support critical review of the purposively selected cases
while preserving the central role of the human analyst in the final
interpretation.
% ─────────────────────────────────────────────────────────────────────────────
...
different conditions. \gls{efficientnetb0} achieves the highest clean accuracy among
the proxy models but undergoes severe direct-target degradation under
\gls{sigmazero} and substantial degradation under \gls{fgsm}. \gls{resnet18} and the
\gls{clip}-based proxy exhibit more limited effects from the same
\gls{efficientnetb0}-oriented artifacts, but these results measure transfer behavior
...
\gls{efficientnetb0} direct-target collapse. Their normalized outputs nevertheless
...
\item \textbf{Controlled class balance.}
...
No independent second annotation was conducted; consequently,
inter-annotator agreement could not be assessed. The labels are therefore controlled and traceable but remain
...
degraded, synthetic, non-firearm weapons, and semantically out-of-task content.
...
Their evaluation on \gls{resnet18}, the \gls{clip}-based proxy, and the selected
...
against \gls{resnet18}, the \gls{clip}-based proxy, or any commercial software system.
Limited transfer must not be interpreted as general direct-attack robustness.

\item \textbf{Proxy-model role.}
\gls{efficientnetb0}, \gls{resnet18}, and the \gls{clip}-based binary-head model are
...
The black-box findings are specific to the evaluated software versions, enabled
...
The \gls{magnetaxiomtool}/\gls{magnetai} results depend on the
\texttt{Possible weapons} tag. The \gls{griffeye}/\gls{t3kcore} results depend
on the conservative use of \texttt{CORE/Violence/Firearm} as the primary
positive bookmark. The \gls{excirefoto2025} results depend on the eight
firearm-oriented semantic queries, union-based retrieval mapping, and selected
\texttt{Distance limit}. The \gls{cellebriteinseyets} results depend on the
union of the exported \texttt{Armi}, \texttt{Pistola}, and \texttt{Fucile}
labels.
...
\gls{efficientnetb0} cases. The cases illustrate different operating conditions and
...
zero baseline in the preprocessed input space. Because numerical convergence
was not established for the retained figures, they are used exclusively as
qualitative diagnostic representations, and no quantitative conclusion is
derived from their attribution values.

The maps must not be interpreted as semantic segmentations, causal
explanations, proofs of feature correctness, or explanations of the selected
...
conclusions are supported. The study provides a protocol-specific assessment of
operational robustness; it does not certify any evaluated software product,
establish a universal ranking of models or commercial systems, constitute a
complete adversarial-security assessment, or validate a production-grade
firearm detector.

The findings support the use of automated image-analysis systems only as
traceable triage components within a \textit{human-in-the-loop} forensic
workflow. Automated outputs should remain associated with the source item,
model or software version, checkpoint or operational configuration, observable
mapping, evaluated condition, and applicable limitations. Missed detections,
false-positive workload, \gls{ood} assignments, configuration-dependent
behavior, and incorrect predictions accompanied by high maximum predicted-class
probabilities must remain visible to the analyst rather than being concealed
behind a single aggregate metric.
...
following software or model updates, and explicit procedures for reviewing and
overriding automated outputs. Final classification and evidentiary
interpretation remain the responsibility of the qualified human analyst and
require contextual assessment, corroboration, chain-of-custody controls, and
expert judgment.
