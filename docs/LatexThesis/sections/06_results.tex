...
\(\left[\left[\mathrm{TN},\mathrm{FP}\right],
\left[\mathrm{FN},\mathrm{TP}\right]\right]\).
...
\begin{table}[!htbp]
\centering
\caption{Clean performance and OOD behavior of proxy models and selected
...
0.030 &
0.438 &
Solid clean baseline, but with a higher FNR than EfficientNet-B0 and a
higher OOD weapon assignment rate. The model-specific score distribution
...
0.012 &
0.836 &
Lower clean accuracy than the two fully fine-tuned CNN proxies, but a very
limited FNR; the main distinguishing behavior is the high OOD weapon assignment
rate. \\
...
of 0.070 and an OOD weapon assignment rate of 0.360. \\
...
0.036 &
0.260 &
Highest clean accuracy and lowest OOD weapon assignment rate among the selected
...
and the OOD weapon assignment rate. \\
...
0.032 &
0.292 &
High clean recall on the \texttt{weapon} class and an OOD weapon assignment
...
cross-system comparison, to map out-of-distribution samples to the
\texttt{weapon} class.
...
Limited degradation under the considered transformations; the distinct OOD
...
FPR is computed on the complete binary bundle, including clean,
...
{\gls{ood} case \texttt{xai\_case\_0009}. An out-of-distribution input is
