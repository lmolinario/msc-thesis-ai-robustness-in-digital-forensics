...
Unless a subsection explicitly states otherwise, these population,
accuracy-drop, attack-transfer, \gls{ood}, and probability-interpretation rules
...
binary metrics, including the architecture-specific mean maximum predicted-class
probability defined in Section~\ref{sec:results-scope}.
...
Among the two fully fine-tuned \glspl{cnn}, \gls{efficientnetb0} provides the
strongest overall balance, with 0.978 accuracy and macro-F1, an \gls{fnr} of
0.016, and an \gls{fpr} of 0.028.
\gls{resnet18} follows at 0.964 accuracy and macro-F1, with an \gls{fnr} of
0.030 and an \gls{fpr} of 0.042. The \gls{clip}-based proxy has lower overall
accuracy (0.927) but the highest weapon recall (0.988) and lowest \gls{fnr}
(0.012), at the cost of the highest \gls{fpr} (0.134). Its lower model-specific
Max-P value (0.559) accompanies this positive-class tendency but is not a
cross-architecture calibration result.
...
Figure~\ref{fig:results-clean-confusion-matrices}.
...
The direction of error is operationally decisive: the three models respectively
produce 8/14, 15/21, and 6/67 false negatives/false positives, for 22, 36, and
73 errors in total. Thus, the
\gls{clip}-based proxy minimizes missed targets while substantially increasing
review workload, whereas \gls{efficientnetb0} offers the strongest balance.
These distinct clean profiles form the baseline for the following robustness
analyses.
...
The \gls{clip}-based proxy assigns 0.8356 of the responses to \texttt{weapon},
far more than \gls{efficientnetb0} (0.3696) or \gls{resnet18} (0.4376), yet none
of its responses reaches Max-P 0.90. Conversely, \gls{efficientnetb0} and
\gls{resnet18} produce high-Max-P rates of 0.5008 and 0.6468. Assignment
direction and output concentration are therefore distinct diagnostics.
...
The comparison confirms the separation between assignment frequency and
architecture-specific Max-P behavior; as established in
Section~\ref{sec:results-scope}, it is not a cross-model uncertainty ranking.
...
The full distributions confirm these architecture-specific score profiles.
Operationally, a positive \gls{ood} assignment can increase unrelated review
volume, while a high Max-P indicates only a peaked closed-set output. Neither is
supervised correctness or stand-alone evidence, and both require human review
...
The model-specific Max-P shifts are small and negative. Gaussian Blur also
shows why aggregate and transition metrics remain separate: \gls{clip} accuracy
increases by 0.007 while four clean-correct samples become incorrect.
...
Thus, comparatively stable aggregate accuracy under the frozen transformations
coexists with architecture-specific decision changes and cannot support a
...
The table exposes three distinct profiles that the software-specific
subsections examine in detail. Griffeye/T3K CORE has the highest bundle
accuracy and lowest \gls{fpr}; Cellebrite is comparatively balanced; and
Magnet.AI has a larger perturbed missed-target burden than its clean result.
Excire most clearly demonstrates configuration sensitivity: moving from
\texttt{D20} to \texttt{D80} reduces the \gls{fnr} while increasing both the
\gls{fpr} and \gls{ood} weapon-assignment rate. Greater retrieval coverage
therefore exchanges missed-target risk for additional review workload.
...
All configurations assign some \gls{ood} inputs to \texttt{weapon}, with rates
ranging from 0.238 (Excire \texttt{D20}) to 0.522 (Excire \texttt{D80}). These
are not supervised false positives, but they can increase unrelated review
volume. No configuration simultaneously minimizes missed targets, false
positives, and \gls{ood} assignments; suitability therefore depends on the
intended workflow and the consequence of each error direction.
...
Under the mapping documented in
Section~\ref{subsec:implementation-magnet}, the exported
\texttt{Possible weapons} tag is the positive observable output; its absence is
mapped to \texttt{non\_weapon}. No unsupported \texttt{unknown} state occurs in
...
As defined in Section~\ref{sec:results-scope}, the adversarial-family results
measure transfer from \gls{efficientnetb0}; Color Shift and the anti-forensic
transformations are model-agnostic.

On the separate \gls{ood} branch, 180 of 500 inputs receive the positive tag
(0.360), an operational review-load indicator rather than a supervised false
positive. Magnet.AI exposes no comparable numerical uncertainty output for this
mapping.
...
These results characterize only the frozen Magnet.AI version, category,
configuration, population, and mapping. The tag remains a traceable triage aid
requiring human verification, not an evidentiary conclusion.
...
controlled raw output but are not included as primary positives, as documented
in Section~\ref{subsec:implementation-griffeye}.
...
The adversarial-family values are transfer results under the common scope. On
the separate \gls{ood} branch, 130 of 500 inputs receive the firearm bookmark
(0.260), which can affect review workload but is not a supervised false
positive.
...
The profile is bounded to the frozen version, modules, population, and bookmark
mapping. The automatic bookmark remains a traceable triage signal requiring
human verification, not an evidentiary conclusion.
...
Using the union-based mapping documented in
Section~\ref{subsec:implementation-excire}, this subsection compares the same
eight firearm-oriented queries at \texttt{D20}, \texttt{D50}, and
\texttt{D80}. Within the evaluated range, these settings are respectively the
most restrictive, intermediate, and most permissive retrieval configurations;
the terms describe observable behavior, not the internal scoring mechanism.
...
The conclusion is bounded to the frozen version, eight-query set, distance
settings, population, and union mapping. Excire outputs remain triage
indicators requiring documented configuration and human verification.
...
labels, the image is assigned to \texttt{non\_weapon}, as documented in
Section~\ref{subsec:implementation-cellebrite}.
...
The adversarial-family values are transfer results under the common scope. On
the separate \gls{ood} branch, 146 of 500 inputs are assigned to the positive
state (0.292), a review-load indicator rather than a supervised false positive.
...
Overall, the frozen Cellebrite configuration presents a comparatively balanced
bundle-level profile and limited aggregate degradation. This observation is
bounded to its version, configuration, exported labels, mapping, and population;
the classifications remain traceable triage indicators requiring human review.
...
Similar clean accuracies do not imply equivalent operating profiles because the
systems, mappings, and prediction units differ. The \gls{clip}-based proxy most
clearly separates nominal accuracy from \gls{ood} behavior: it has only six
clean false negatives but assigns 0.836 of fold-specific \gls{ood} responses to
\texttt{weapon}, as discussed in Section~\ref{sec:results-ood}. Black-box
reference rates are lower (0.260--0.360), although
Excire \texttt{D80} reaches 0.522. Clean errors and \gls{ood} review-load
tendency must therefore be read jointly and within configuration.
...
Figure~\ref{fig:results-forensic-tools-antiforensic-drop-heatmap} extends the
comparison to all Excire settings. Anti-forensic
family-level degradation is smaller than the strongest direct-target effects,
but the dominant burden differs: blurring increases missed targets for
\gls{efficientnetb0} and Magnet.AI, Histogram Modification produces
bidirectional ResNet18 errors, and Excire \texttt{D50} expands false-positive
workload. The \gls{clip} row further shows that a small aggregate improvement
can coexist with 13 newly induced errors. These are mapping- and
configuration-specific operating points, not intrinsic system properties.

% ─────────────────────────────────────────────────────────────────────────────
\paragraph{Robustness under adversarial and adversarial-style conditions.}
% ─────────────────────────────────────────────────────────────────────────────

The direct-target, transfer, and model-agnostic distinctions defined in
Section~\ref{sec:results-scope} apply to this comparison.
...
Table~\ref{tab:results-comparative-adversarial} and
Figure~\ref{fig:results-forensic-tools-adversarial-drop-heatmap} distinguish a
severe direct-target collapse from much smaller transferred effects. Sigma-Zero
drives \gls{efficientnetb0} to 0.015
accuracy with 963 induced errors, while transferred FGSM produces the largest
missed-target burden for Magnet.AI and Cellebrite (96 and 53 false negatives).
Griffeye's largest burden occurs under Sigma-Zero (37 false negatives), whereas
Excire \texttt{D50} responds chiefly through false-positive expansion
(\gls{fpr} 0.442). The D20/D80 heatmap rows confirm that transfer behavior also
depends on the retrieval setting; inaccessible internal causes cannot be
...
The same queries therefore support different priorities: \texttt{D20} favors
selectivity, \texttt{D80} retrieval coverage, and \texttt{D50} an intermediate
point. The other black-box profiles likewise depend on the frozen Griffeye
bookmark, Cellebrite label union, and Magnet.AI tag. These reproducible
recodings are not semantically equivalent internal taxonomies; interpretation
is limited to observable decisions and requires the version, configuration,
export, and mapping to remain recorded.
...
No row minimizes all seven indicators. Low clean missed-target rates can
coexist with adversarial degradation, high \gls{ood} routing, or increased
false-positive workload; the black-box reference profiles likewise exchange
selectivity, recall, and balance according to their frozen mappings.
...
Accordingly, the comparison identifies bounded tradeoffs rather than a
universally superior system. Outputs remain traceable triage support associated
with their source, version, configuration, mapping, condition, and limitations;
final assessment remains human.
% ─────────────────────────────────────────────────────────────────────────────
...
The heatmap and overlay provide a qualitative reference for comparing the
spatial attribution pattern with the selected failure cases; agreement with the
reference label does not validate either the model or the attribution method.
...
Because the map targets the incorrect predicted class, it supports case-level
inspection but cannot determine whether the failure derives from appearance,
scale, occlusion, context, source regularities, preprocessing, or their
interaction.
...
The output is not a supervised false positive; it shows how an out-of-task
input is absorbed into a positive closed-set class. Its 0.999 Max-P is a peaked
checkpoint output, not proof of correctness, calibration, or forensic
relevance, and the map does not establish a firearm-specific feature.
...
not require model knowledge. The predicted-class map does not identify whether
contrast redistribution, local or global intensity changes, preprocessing, or
another internal effect caused the error.
...
The reference assignment establishes that the strongly peaked output is wrong:
Max-P 1.000 is not correctness, calibrated certainty, or forensic reliability.
The predicted-class map neither explains the Sigma-Zero optimization nor
identifies stable semantic features responsible for the error.
...
The failure record, not the visualization, establishes the error and shows why
output concentration cannot replace expert assessment.

% ─────────────────────────────────────────────────────────────────────────────
\paragraph{Cross-case interpretation.}
% ─────────────────────────────────────────────────────────────────────────────

Together, the cases contrast a clean reference, a nominal false negative, a
peaked closed-set \gls{ood} assignment, a model-agnostic false negative, and a
direct-target false positive at Max-P 1.000.
