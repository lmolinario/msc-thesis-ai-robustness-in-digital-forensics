...
\gls{colorshift}; used as controlled stressors for the proxy and black-box
evaluation. \\
...
attack, but an operationally plausible risk: in an investigative workflow,
...
effects on the proxy models,
%l'ho visto anche prima nella metodologia ma cosa intendi con proxy models? non l'ho mai sentito... se è qualcosa che hai definito tu va spiegato

especially when compared with the adversarial
...
autonomous objective of \gls{advml}, but as experimental stressors
%anche qui, l'ho visto prima nella metodologia ma cosa intendi per stressor? non è un termine frequente
useful for evaluating the operational reliability of \gls{ai}-based systems in forensic
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Selected Black-Box Software Evaluation} %ne hai già parlato nella metodologia, non ha senso ripetere

\label{sec:results-commercial}
% ─────────────────────────────────────────────────────────────────────────────

The evaluation of selected software systems represents the black-box level of
the experimental pipeline. Unlike the proxy models analyzed in the previous
sections, these systems do not provide access to the model architecture,
training dataset, decision thresholds, internal preprocessing, or complete
classification logic. The evaluation is therefore based exclusively on the
observable outputs produced by the software systems, such as assigned tags,
exported categories, retrieval results, and semantic bookmarks, as well as on
the matching between the files of the forensic evaluation bundle and the
experimental ground truth.

The comparison includes four selected software systems:
...
normalized black-box profiles: \gls{magnetaxiomtool}/\gls{magnetai},
\gls{excirefoto2025} \texttt{D20}, \gls{excirefoto2025} \texttt{D50},
\gls{excirefoto2025} \texttt{D80}, \gls{cellebriteinseyets}, and
\gls{griffeye}/\gls{t3kcore}.

The selected software systems are not treated as homogeneous classifiers.
\gls{magnetai} provides operational image categorization outputs;
\gls{excirefoto2025} operates as a semantic retrieval engine;
\gls{cellebriteinseyets} produces media analysis outputs exported from the
Cellebrite environment; and \gls{griffeye}/\gls{t3kcore} produces automatic
semantic bookmarks. The comparison is therefore operational and black-box: each
output is mapped to the \texttt{weapon}/\texttt{non\_weapon} task through
explicit and documented normalization rules.

The adversarial perturbations included in the bundle must not be interpreted as
white-box attacks optimized against the proprietary software systems. They are
artifacts generated within the controlled proxy-model protocol and are used
here to observe, in black-box mode, the operational sensitivity of selected
software systems to manipulated inputs. Moreover, the aggregate metrics for
these systems are \emph{bundle-weighted}: they describe behavior within the
forensic evaluation bundle, whose composition is intentionally oriented toward
robustness analysis, rather than providing an absolute estimate of overall
product quality.

To prevent the evaluated software from using semantic information derived from
file names, paths, or folder structure, the evaluation was conducted using the
blind view of the forensic evaluation bundle. The files were imported with
neutral names such as \texttt{bundle\_*.jpg}, while information related to
class, source, fold, attack, transformation, hash, and ground truth was kept
separate in the audit manifests. After processing, the exported outputs were
normalized and mapped back to the original samples through bundle identifiers
and \gls{sha256}/\gls{md5} hashes. This setting makes it possible to
distinguish among correctly flagged images, unflagged images, false negatives
on the \texttt{weapon} class, false positives on the \texttt{non\_weapon}
class, non-interpretable outputs, and behavior on \gls{ood} samples.

Table~\ref{tab:results-commercial-overview} reports a bundle-weighted summary
of the metrics for the selected systems and operational configurations
considered. For all black-box profiles, normalization produced complete
...
Bundle \gls{fnr} and bundle \gls{fpr} indicate the false negative rate and
false positive rate computed on the interpretable binary bundle, including
clean, adversarial, and anti-forensic samples and excluding \gls{ood} samples.
\textit{OOD W-rate} indicates the proportion of \gls{ood} samples operationally
mapped to the \texttt{weapon} class. Binary metrics are computed on \texttt{weapon}/\texttt{non\_weapon}
...
selected software systems in an absolute sense, but the behavior observed after output
...
retrieval threshold: \texttt{D20} reduces the false positive rate and the
weapon flag rate on \gls{ood} samples, but increases false negatives;
\texttt{D80} maximizes recall on the \texttt{weapon} class, but markedly
increases false positives and \texttt{weapon} flags on \gls{ood} samples.
\texttt{D50} therefore represents an intermediate configuration, useful for the
operational comparison with \gls{magnetai}, \gls{cellebriteinseyets}, and
\gls{griffeye}/\gls{t3kcore}.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{images/fig_forensic_tools_ood_weapon_flag_rate.pdf}
\caption{Weapon flag rate of the selected black-box software systems on \gls{ood} samples.}
...
selected software systems with good performance on the binary task exhibit a non-zero
...
consolidated black-box software result of the pipeline. The outputs produced by
the module were normalized by mapping the \texttt{Possible weapons} tag as an
...
output of the module with the experimental ground truth of the forensic
evaluation bundle, without assuming access to the internal decision logic of
...
The evaluation of \gls{cellebriteinseyets} constitutes part of the black-box
level of the experimental pipeline. The system is evaluated as a commercial
software platform for \gls{ai}-assisted media analysis, using exclusively the
observable outputs exported and normalized from the
\texttt{Cellebrite Physical Analyzer 10.9.0.3029} environment. Consistent with
the methodology defined in Chapter~\ref{chap:methodology}, \gls{ufed} is not
treated as an image classifier, but only as part of the broader Cellebrite
ecosystem for data acquisition and management.
...
black-box output paradigm into the experimental protocol. Unlike systems that
provide categories, classifications, or retrieval results, \gls{griffeye} was
evaluated through the automatic semantic bookmarks generated by \gls{t3kcore}.
Consistent with the methodology defined in Chapter~\ref{chap:methodology},
these bookmarks are treated as observable operational outputs and not as local
explanations of the internal decision-making process.
...
system performance. A more permissive mapping, including weapon-adjacent
...
systems may also index metadata embedded in image files. For this reason, the
check is retained in Chapter~\ref{chap:results} as a methodological audit, but
not as a primary robustness result.

The audit examined 11\,500 files in the blind directory
...
\gls{ai}-based classifiers to be observed in a controlled and transparent
manner in the presence of clean, \gls{ood}, anti-forensic, and adversarial
inputs. The second level concerns the selected software systems, evaluated as
black-box systems through observable and exportable outputs only. The third
level concerns the forensic evaluation bundle, which makes it possible to
expose both proxy models and selected software systems to the same families of
inputs, while keeping the white-box evaluation of the proxy models distinct
from the operational black-box evaluation of the selected software systems.
...
predictions. Selected software systems, by contrast, provide proprietary
categories, semantic retrieval results, media analysis outputs, or automatic
semantic bookmarks that must be normalized into the operational labels
\texttt{weapon}, \texttt{non\_weapon}, and, where necessary, non-informative or
non-mappable categories. The value of the comparison is therefore methodological
and operational: it makes it possible to understand which types of errors may
emerge when \gls{ai}-based automation is used to support forensic image triage.
...
The \gls{xai} figures reported in this section follow the structure introduced
in Chapter~\ref{chap:methodology}: input image, \gls{ig} heatmap, attribution
...
operational configuration of the selected software systems, normalization of
black-box outputs, and the availability of human review.
...
priorities. For this reason, the most significant findings in this chapter
concern the observed failure modes, rather than only the aggregate value of
accuracy.

The evaluation of selected software systems must be interpreted with particular
caution: \gls{magnetaxiomtool}/\gls{magnetai}, \gls{excirefoto2025},
...
models, thresholds, or proprietary decision logic. The black-box comparison is
...
Four limitations are particularly relevant for interpreting the results of this
chapter. First, the adversarial perturbations are primarily generated against
EfficientNet-B0 and therefore represent a controlled white-box stress scenario,
not a complete attack space against all evaluated systems. Second, the
anti-forensic transformations cover a representative but non-exhaustive set of
image-level manipulations. Third, the metrics of selected software systems are
bundle-weighted and depend on the adopted output normalization rules. Fourth,
the residual metadata audit shows that blindness was primarily enforced at the
level of filenames, paths, and directory structure, while complete metadata
neutralization should be considered in future evaluations.

These limitations do not invalidate the experimental findings, but define their
scope of validity. The results should therefore be interpreted as a
protocol-specific assessment of operational robustness, suitable for deriving
methodological and forensic implications, rather than as a general product
certification or universal robustness ranking.
