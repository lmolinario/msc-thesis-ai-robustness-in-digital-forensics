...
them according to the methodological protocol defined in
...
aggregate perturbed, adversarial-family, and anti-forensic-family binary
results. The separate \gls{ood} result is discussed below.
...
Table~\ref{tab:results-griffeye-condition-metrics} separates the clean,
aggregate perturbed, adversarial-family, and anti-forensic-family binary
results. The separate \gls{ood} result is discussed below.
...
Table~\ref{tab:results-cellebrite-condition-metrics} separates the clean,
aggregate perturbed, adversarial-family, and anti-forensic-family binary
results. The separate \gls{ood} result is discussed below.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Interpretive Boundaries of the Experimental Results}
\label{sec:results-limitations}
% ─────────────────────────────────────────────────────────────────────────────

The results reported in this chapter characterize the observable behavior of
the evaluated systems under the frozen experimental protocol. They must not be
interpreted as universal estimates of robustness, as an absolute ranking of the
evaluated systems, or as validation of their behavior outside the documented
dataset, software versions, configurations, output mappings, and threat model.

Several distinctions are particularly important when interpreting the reported
metrics. First, false negatives and false positives have different operational
consequences for forensic triage, and aggregate accuracy may conceal changes in
their direction or distribution. Second, a \texttt{weapon} assignment on an
\gls{ood} input is not a supervised false positive, because the \gls{ood}
population has no binary reference assignment. Third, the four model-dependent
adversarial attacks were optimized against fold-specific
\gls{efficientnetb0} checkpoints; their effects on \gls{resnet18}, the
\gls{clip}-based proxy, and the selected black-box configurations therefore
represent empirical transfer rather than matched direct-target robustness.

The black-box results must likewise be interpreted as properties of the frozen
observable configurations rather than intrinsic characteristics of the software
families. Tags, bookmarks, semantic retrieval results, and exported
classification labels were normalized through explicit operational mappings,
and alternative defensible mappings or future software versions could produce
different operating profiles. Maximum predicted-class probabilities available
for the proxy models are also architecture-specific diagnostic outputs and are
not treated as calibrated or directly comparable measures of correctness.

Finally, the manipulated populations are derived from the same 1\,000 binary
source images, and the proxy-model and black-box \gls{ood} rates use different
prediction units. These properties constrain the statistical interpretation of
the reported aggregate values and reinforce the protocol-specific nature of the
comparison.

The broader methodological limitations of the study are discussed in
Section~\ref{sec:conclusions-limitations}, while the implications for forensic
use, configuration management, traceability, and continued human review are
developed in Section~\ref{sec:conclusions-operational-implications}.
