...
artifacts, not a white-box attack against the commercial products.
...
product-quality estimates or as a general ranking among heterogeneous software
systems.
...
are bundle-weighted and reflect the experimental composition of the bundle, not
a general product validation.
\end{table}
...
not the internal accuracy of the selected products in an absolute sense. The
...
as an invalidating factor for the experimental results. The residual presence
of embedded metadata nevertheless suggests that, in future studies or
...
these assignments potentially more relevant in an automatic prioritization process.
The selected software systems present more limited values: \gls{magnetai}
reaches an OOD weapon flag rate of 0.360, \gls{excirefoto2025} under the
\texttt{D50} configuration reaches 0.340, \gls{cellebriteinseyets} reaches
0.292, and \gls{griffeye}/\gls{t3kcore} reaches 0.260. These results indicate
a lower operational tendency of the selected software systems, under the
adopted protocol, to absorb out-of-distribution samples into the
\texttt{weapon} class. However, since the internal mechanisms, thresholds, and
calibration of the selected software systems are not accessible, these values
must not be interpreted as evidence of actual OOD recognition capability, but
as observable behavior after black-box normalization and with respect to the
adopted mapping rules.
...
behavior of selected software systems.
...
\texttt{D20} is more suitable for containing noise, \texttt{D80} for maximizing
recall, and \texttt{D50} for balancing false negatives and false positives.
...
\gls{ai}-assisted commercial software.
...
\begin{itemize}
    ...
    quantitative influence on the reported aggregate metrics, but future
    evaluations should include metadata-neutralized and metadata-preserving
    bundle variants as an explicit sensitivity control.
\end{itemize}
...
provides a protocol-specific assessment of operational robustness rather than a
product certification, universal robustness ranking, or evidentiary validation
