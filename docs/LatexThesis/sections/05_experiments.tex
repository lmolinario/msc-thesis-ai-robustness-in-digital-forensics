...
\caption{Global comparison of the aggregate metrics of the selected black-box software systems on the forensic evaluation bundle. The plot compares accuracy, balanced accuracy, \texttt{weapon} recall, \acrshort{fnr}, \acrshort{fpr}, and \gls{ood} weapon flag rate after output normalization.}
...
commercial systems with good performance on the binary task exhibit a non-zero
...
remains inaccessible. The module maintains good overall performance and high
...
\texttt{weapon} and \texttt{non_weapon}; the 500 \gls{ood} images are analyzed
separately through the weapon flag rate.

Table~\ref{tab:results-cellebrite-global-metrics} reports the overall metrics
of \gls{cellebriteinseyets} on the binary
\texttt{weapon}/\texttt{non_weapon} task. The system achieves an accuracy of
...
\begin{table}[H]
\centering
\caption{Overall performance of \gls{cellebriteinseyets} on the binary \texttt{weapon}/\texttt{non_weapon} task.}
...
\footnotesize
\textit{Note.} The metrics are computed on the 11,000 images belonging to the
binary \texttt{weapon}/\texttt{non_weapon} task. The 500 \gls{ood} images are
...
belong to the \texttt{weapon}/\texttt{non_weapon} ground truth; rather, it
...
\begin{table}[H]
\centering
\caption{Overall performance of \gls{griffeye}/\gls{t3kcore} on the binary \texttt{weapon}/\texttt{non_weapon} task.}
...
the \texttt{weapon}/\texttt{non_weapon} ground truth; rather, it describes the
...
Overall, \gls{griffeye}/\gls{t3kcore} exhibits the most selective black-box
...
substantially lower than those of the other selected software systems, while
...
\texttt{weapon}, \texttt{non_weapon}, and, where necessary, non-informative or
...
indicates transitions from \texttt{weapon} to \texttt{non_weapon}; FN
...
in the \texttt{weapon}(\rightarrow)\texttt{non_weapon} direction. Although
...
\texttt{non_weapon} images are present. \gls{clip}, instead, remains
...
target, indicated in the experimental files as \texttt{predicted_label}. This
...
{Correct case on a clean input \texttt{xai_case_0001}. The EfficientNet-B0
...
{False negative on a clean input \texttt{xai_case_0006}. A \texttt{clean}
...
{\textbf{scenario}: \texttt{clean};
\textbf{manual label}: \texttt{weapon};
\textbf{prediction}: \texttt{non_weapon};
...
EfficientNet-B0 classifies it as \texttt{non_weapon} with moderate confidence.
...
{\gls{ood} case \texttt{xai_case_0009}. An out-of-distribution sample is
...
\texttt{non_weapon} class alone.

\XAIcaseFigureMaskGrid
{fig:xai-case4-antiforensic-failure}
{Anti-forensic false negative \texttt{xai_case_0010}. A \texttt{weapon} image
...
\texttt{weapon}, but the model predicts \texttt{non_weapon} with high
...
{High-confidence adversarial false positive \texttt{xai_case_0015}. A
\texttt{non_weapon} image perturbed using \gls{sigmazero} is classified as
\texttt{weapon}.}
...
\textbf{manual label}: \texttt{non_weapon};
\textbf{prediction}: \texttt{weapon};
\textbf{confidence}: 1.000}
...
\texttt{non_weapon}, but, after the \gls{sigmazero} perturbation, it is
...
The main methodological limitations---model-dependent attacks generated against
EfficientNet-B0, the non-exhaustive space of anti-forensic transformations,
bundle-weighted metrics for selected software systems, and residual sensitivity
to embedded metadata---are addressed systematically in
Chapter~\ref{chap:conclusions}, together with the methodological, operational,
and legal implications of using \gls{ai}-assisted software systems in \gls{df}
workflows.
