...
\label{chap:results}
% ─────────────────────────────────────────────────────────────────────────────
This chapter presents the experimental results of the \textsc{FAIR-Lab}
evaluation pipeline, with particular attention to the operational robustness of
...
solely on accuracy, but rather to evaluate how proxy models and commercial
black-box tools react to clean inputs, \gls{ood} samples, anti-forensic
transformations, and adversarial perturbations. The interpretation of the
results therefore maintains an explicitly forensic focus: false negatives on
the \texttt{weapon} class, operational absorption of out-of-distribution inputs,
traceability of the bundle, normalization of black-box outputs, and the need
for human review
...
This chapter reports the results obtained by applying the \textsc{FAIR-Lab}
...
concerns the commercial black-box tools, for which the evaluation is based
exclusively on observable outputs, normalized exports, and matching against the
experimental ground truth.

The objective of this chapter is not to construct an absolute ranking among
models and tools, nor to transform the thesis into an \gls{advml} benchmark.
Rather, the focus is to evaluate the operational robustness of \gls{ai}-based
systems with respect to conditions that are relevant to \gls{df} workflows:
clean inputs, \gls{ood} samples, anti-forensic transformations, adversarial
perturbations, and processing through commercial tools. From this perspective,
particular importance is assigned to false negatives on the \texttt{weapon}
class, false positives on \texttt{non\_weapon} content, behavior on
out-of-distribution samples, and the ability to trace each result back to the
corresponding experimental artifact.
...
\textbf{Consolidated} &
11\,500 files prepared for the evaluation of the proxy models and commercial
black-box tools. \\
...
\gls{excirefoto2025} &
\textbf{Consolidated} &
Evaluated as standalone general-purpose semantic retrieval software with
\gls{ai}-based functionalities, under the operational configurations
\texttt{D20}, \texttt{D50}, and \texttt{D80}. \\

\gls{cellebriteinseyets} &
\textbf{Consolidated} &
Evaluated as a commercial black-box tool for media analysis, through observable
outputs exported and normalized from the \texttt{Cellebrite Physical Analyzer
10.9.0.3029} environment. \\
...
\texttt{weapon}/\texttt{non\_weapon} subset. \gls{ood} samples, instead, are
kept separate: they are not used to compute supervised accuracy, but to measure
the tendency of systems to map out-of-distribution content to one of the two
operational classes.
...
proxy models and for the black-box evaluation of commercial tools. \\
...
produced by the experimental pipeline. For the proxy models, clean and perturbed
performance is derived from \codepath{results/metrics/final_core_metrics.csv},
the confusion matrices from
...
\codepath{results/metrics/final_robustness_metrics.csv}. For commercial tools,
the aggregate metrics are derived from
...
tools are therefore analyzed through a common representation that is independent
of the native export format.
...
This caution is even more relevant in the comparison with commercial black-box
tools, which do not necessarily expose numerical scores, decision thresholds, or
homogeneous uncertainty measures. For this reason, confidence is used in this
chapter as an auxiliary diagnostic element, not as autonomous evidentiary
measure.
...
the controlled experimental level and the commercial black-box level. The blind
view of the bundle allows the tools to process the files without direct
exposure to labels, attack names, fold information, or experimental metadata,
whereas the manifests and \gls{sha256}/\gls{md5} hashes make it possible to map
each normalized output back to the corresponding artifact. This approach makes
it possible to distinguish among classification errors, missed detections,
uncategorized outputs, non-interpretable mappings, and limitations specific to
the export formats of individual tools.
...
the behavior of the models under the subsequent conditions must be interpreted,
...
\texttt{non\_weapon}; consequently, false negatives indicate images containing
weapons classified as non-relevant, while false positives indicate images not
containing weapons classified as \texttt{weapon}.
...
distinguishing nominal performance from the level of certainty expressed by the
model.
...
slightly lower but still high performance, with an accuracy of 0.964, a macro-F1
of 0.964, and a mean confidence of 0.974. \gls{clip} achieves an accuracy of
0.927 and a macro-F1 of 0.927, but exhibits a different operational profile from
the two supervised models: recall on the \texttt{weapon} class is very high,
equal to 0.988, whereas the \gls{fpr} is substantially higher, equal to 0.134.
Moreover, the mean confidence of \gls{clip} is 0.559, much lower than that
observed for EfficientNet-B0 and ResNet18.
...
binary head with respect to the specific task, not as the result of a zero-shot
mechanism. The lower mean confidence also suggests that the greater tendency to
flag images as \texttt{weapon} does not necessarily correspond to high-certainty
predictions.
...
different operational profile of the three models: EfficientNet-B0 minimizes the
total number of errors, ResNet18 maintains an intermediate behavior, whereas
\gls{clip} reduces false negatives on the \texttt{weapon} class at the cost of a
...
From a forensic perspective, the most critical datum is represented by false
...
overall accuracy, but must also consider the different balance among sensitivity
to the investigatively relevant class, risk of improper flags, and level of
confidence associated with the predictions.

Overall, the clean baseline shows that the proxy models achieve high
performance under nominal conditions. However, the difference among accuracy,
\gls{fpr}, \gls{fnr}, and mean confidence already highlights, even in the
absence of perturbations, an aspect that is relevant for forensic triage: models
with similar accuracy may produce different operational risk profiles,
...
synthetic renderings, videogame images, war scenes, military vehicles,
...
predictions with confidence at least equal to 0.9. In this section, these values
must not be interpreted as accuracy or supervised error on a third class, but as
...
indicates the number of predictions with confidence at least equal to 0.9;
...
The results show three distinct profiles. \gls{clip} classifies a very high
proportion of \gls{ood} samples as \texttt{weapon}, equal to 0.8356, but does so
with a moderate mean confidence, equal to 0.5358, and with no predictions above
the high-confidence threshold. The null high-confidence rate for \gls{clip}
therefore indicates that no \gls{ood} prediction exceeds the confidence
threshold of 0.9. EfficientNet-B0 and ResNet18, by contrast, classify a lower
proportion of \gls{ood} samples as \texttt{weapon}, equal to 0.3696 and 0.4376,
respectively, but with much higher mean confidence values. In particular,
ResNet18 has a high-confidence rate of 0.6468, whereas EfficientNet-B0 reaches
0.5008.
...
assigning high confidence to those predictions. The case of \gls{clip}, instead,
shows the opposite behavior: a greater tendency to classify samples as
\texttt{weapon}, but with lower confidence. This distinction is essential in a
forensic context, where system confidence must not be interpreted as
stand-alone evidence, but as auxiliary information subject to human verification
...
respect to model-agnostic transformations that may arise in realistic
operational scenarios. Unlike adversarial attacks, these transformations do not
...
modification of the tonal distribution, and contrast normalization
\cite{stamm2010anti,yaacoub2021digital,hendrycks2019benchmarking}.

Consistently with the convention defined in
Section~\ref{sec:results-scope}, performance drop is interpreted as the
variation with respect to the clean baseline. Positive values indicate a
reduction in performance, whereas values equal to zero or negative values are
treated as stability or distribution-specific oscillations, not as evidence of
general robustness.
...
ResNet18 instead shows the greatest degradation under
...
attributable to realistic conditions of image circulation, compression,
...
perturbations designed to alter classifier behavior
\cite{szegedy2014intriguing,goodfellow2015explaining,biggio2018wild}. In the
context of this thesis, these perturbations are not analyzed as an autonomous
objective of \gls{advml}, but as experimental stressors useful for evaluating
the operational reliability of \gls{ai}-based systems in forensic scenarios.
The main interest therefore does not concern only the ability of an attack to
reduce accuracy, but the risk that images belonging to the \texttt{weapon}
class are reclassified as \texttt{non\_weapon}, generating potentially critical
false negatives in an investigative triage process.

The model-dependent attacks are generated against EfficientNet-B0. This choice
reflects a white-box scenario in which the adversary has full access to a
single target model; transferability toward different architectures is
therefore evaluated empirically, rather than assumed a priori
\cite{biggio2018wild,nowroozi2021survey,barni2018cf}. \gls{colorshift} is
instead treated as a model-agnostic adversarial-style perturbation.

Consistently with the convention defined in Section~\ref{sec:results-scope},
performance drop is interpreted as the variation with respect to the clean
baseline. In this section, any negative or null values are not interpreted as
...
images containing weapons: the highest clean baseline among the proxy models
does not prevent severe degradation when the model is exposed to perturbations
optimized against it.

\gls{fgsm}, also generated against EfficientNet-B0, produces a relevant
degradation, reducing accuracy to 0.575 and producing an accuracy drop of
0.403. \gls{superdeepfool} instead produces a more limited effect on the same
model, with an accuracy of 0.931 and a drop of 0.047. \gls{onepixel} and
...
black-box evaluation of the selected commercial tools and software. The bundle
is designed to provide the tools with a blind and semantically neutral input,
while separately preserving the information required for matching, export
normalization, and result auditing
...
Section~\ref{sec:results-scope}, Table~\ref{tab:results-bundle-operational-layout}
describes the bundle from an operational perspective, distinguishing among the
content submitted to the tools, blind naming, and the role of metadata in the
audit phase. The total consists of 11\,500 files, including 1000 clean samples,
500 \gls{ood} samples, 5000 adversarial samples, and 5000 anti-forensic samples.
...
directory intended for the evaluated tools, in which files are renamed with
...
evaluation, the tools must import only the directory:
...
\codepath{structured_audit_view/} directory. This separation reduces the risk of
bias induced by file names, paths, classes, attack names, or split structure.
...
path, blind path, structured path, \gls{sha256} hash, \gls{md5} hash, file size,
and extension. This structure makes it possible to connect each output exported
by a tool to the corresponding experimental artifact without exposing the
ground truth to the tool during the import phase.

The consistency checks verified that the bundle identifiers are unique, that
the effective \gls{sha256} hashes are unique, that the computed hashes match
those reported in the manifest when available, that the blind paths are
semantically neutral, and that the experimental metadata are separated from the
operational input of the tools. These checks are essential to make the bundle
suitable for black-box evaluation, since they make it possible to distinguish
among classification errors, unprocessed files, uncategorized outputs,
limitations of the export format, and matching problems
...
context of black-box tools. Its function is not merely organizational, but
methodological: it preserves the reproducibility of the experiment and reduces
the exposure of the tools to unnecessary semantic information. At the same time,
it enables post-export normalization based on verifiable identifiers and hashes,
which is a necessary condition for distinguishing genuine classification errors
from artifacts of the export process or limitations of the operational mapping.

A specific control on residual embedded metadata in the blind view of the bundle
is discussed in
Section~\ref{subsec:results-embedded-metadata-sensitivity}, after the
presentation of the normalized outputs of the commercial tools, since its
interpretation requires comparison with the observed black-box results.
...
bundle-weighted and reflect the experimental composition of the bundle, not an
absolute or certifying evaluation of the products.
\end{table}
...
the adopted normalization rule, not as stand-alone evidence of the intrinsic
superiority of the tool. A more permissive mapping, including weapon-adjacent
...
Overall, the commercial comparison does not identify a universally superior
tool, but highlights different risk profiles. \gls{griffeye}/\gls{t3kcore}
