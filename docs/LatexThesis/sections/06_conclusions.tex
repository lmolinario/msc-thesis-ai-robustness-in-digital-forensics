...
the introduction of a new classifier, but in the formalization of an auditable
...
checks, normalized black-box outputs, and qualitative analysis are explicitly
...
frozen and reproducible experimental basis. The final dataset includes 1\,500
images balanced across three evaluation categories: \texttt{weapon},
\texttt{non\_weapon}, and \texttt{ood}. The official binary subset includes
1\,000 images intended for the generation and evaluation of perturbations.
Starting from this basis, five families of adversarial perturbations were
produced, namely \gls{fgsm}, \gls{superdeepfool}, \gls{sigmazero},
\gls{onepixel}, and \gls{colorshift}, as well as five model-agnostic
anti-forensic transformations, namely \gls{jpegrecompression},
\gls{resampleresize}, \gls{gaussianblur}, \gls{histogrammodification}, and
\gls{contraststretching}. This organization made it possible to observe the
behavior of the evaluated systems under clean, out-of-distribution, degraded,
and deliberately manipulated conditions.
...
adversarial, and anti-forensic conditions. The bundle was prepared for
black-box evaluation through blind naming, separate manifests, neutral
identifiers, and \gls{sha256}/\gls{md5} hashes. This structure makes it
possible to connect each final output to the corresponding experimental
artifact, while keeping content, test condition, provenance, identifier, and
metric result separate. In this way, the evaluation of selected software systems
does not depend on implicit steps or undocumented manual associations, but on
verifiable and reconstructible artifacts.
...
ResNet18, and \gls{clip}, and the selected software systems included in the
final evaluation: \gls{magnetaxiomtool}/\gls{magnetai},
\gls{excirefoto2025}, \gls{cellebriteinseyets}, and
\gls{griffeye}/\gls{t3kcore}. \gls{excirefoto2025} was evaluated as
general-purpose software with \gls{ai}-based semantic retrieval functionality,
not as an exclusively forensic product; similarly, \gls{cellebriteinseyets} was
treated as a commercial black-box software platform for \gls{ai}-assisted media
analysis, whereas \gls{griffeye}/\gls{t3kcore} was evaluated through the
automatic semantic bookmarks generated by \gls{t3kcore}. The observable outputs
of the selected software systems were exported, normalized, and operationally
recoded with respect to the \texttt{weapon}/\texttt{non\_weapon} task, while
keeping the evaluation of \gls{ood} samples separate. The qualitative analysis
using \gls{ig} on representative EfficientNet-B0 cases complements this
interpretation by providing diagnostic support for selected failure modes
without replacing the quantitative metrics.
...
489 \texttt{weapon}\(\rightarrow\)\texttt{non\_weapon} transitions. This result
