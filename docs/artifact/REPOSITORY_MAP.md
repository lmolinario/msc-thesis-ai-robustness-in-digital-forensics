...
This document maps the curated public repository to the MSc thesis research
workflow.
...
| `results/` | Frozen metrics, Chapter 6 reporting outputs, and validators |
...
The paths `results/figures/chapter_5/` and
`explainability/manifests/chapter5/`, together with related identifiers and
script names, predate the final split between implementation (Chapter 5) and
results (Chapter 6). They are retained as frozen artifact identifiers. Their
names do not define the current thesis chapter structure.
...
Image corpora and generated image directories are restored or regenerated
locally and are excluded from current `main`.
...
The four model-dependent attacks are generated against fold-specific
EfficientNet-B0 checkpoints. `color_shift` is deterministic and model-agnostic.
Public `main` retains scripts, manifests, and reports; generated perturbation
images remain local or controlled.
...
The registry records 15 fold-aware checkpoints and their SHA-256 digests.
...
The canonical commercial table contains 69,000 sanitized decisions and excludes
complete raw-export paths and unrelated proprietary metadata.
...
- preserve the five cases and twenty image assets discussed in Chapter 6.
...
The complete historical XAI output tree is not distributed on current `main`.
Thesis-ready assets are stored under `docs/LatexThesis/images/`.
...
OOD accounting uses 500 unique images evaluated by five fold-specific
checkpoints, yielding 2,500 predictions per architecture and 7,500 rows across
three architectures.
...
Authoritative defense entry point:
...
`docs/LatexThesis/tesi.bib`; complete third-party publication copies are not part
of the curated release tree.

The final chapter structure is documented in `docs/README.md` and
...
These tools perform non-destructive local checks and are not part of the
numbered experimental pipeline.
...
Historical development records are preserved outside the curated branch through
the protected snapshot documented in `docs/artifact/ARCHIVE_SNAPSHOT.md`.
