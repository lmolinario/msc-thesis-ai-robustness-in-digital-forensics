...
Generates Chapter 5 reporting figures and tables from the consolidated metric files.
The authoritative numerical sources remain under `results/metrics/`.

## `21_generate_embedded_metadata_sensitivity_check.py`

Reproduces the frozen embedded-metadata leave-out sensitivity analysis. Its canonical
commercial-tool prediction input is:
...
The analysis is descriptive and does not establish causal influence of metadata on a
proprietary decision process.
...
metadata audit and does not replace the frozen detailed workflow unless explicitly
run and validated.
...
- OOD accounting (`500 images x 5 folds = 2,500 predictions per architecture`);
...
Audits all Chapter 5 reporting assets against the English and Italian LaTeX trees.
It identifies:

- asset IDs referenced by each thesis;
...
- asset IDs not referenced by either thesis.
...
No duplicate or unreferenced asset should be deleted solely from this audit. Removal
requires a separate review of LaTeX references, binary equivalence, and the role of the
asset as a reproducibility/reporting output.
