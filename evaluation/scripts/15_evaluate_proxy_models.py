...
def binary_metrics(df: pd.DataFrame, group: dict[str, Any]) -> dict[str, Any]:
    ...
    f1_w = None if not prec_w or not rec_w else 2 * prec_w * rec_w / (prec_w + rec_w)
    f1_n = None if not prec_n or not rec_n else 2 * prec_n * rec_n / (prec_n + rec_n)
    return {
        **group,
        "total": total,
        "accuracy": acc,
        "balanced_accuracy": None if rec_w is None or rec_n is None else (rec_w + rec_n) / 2,
...
def comparative_metrics(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for keys, group in pert.groupby(["evaluated_model", "attack_family", "attack_name", "attack_target_model"], dropna=False):
        ...
        rows.append({
            ...
            "induced_error_rate_clean_correct": safe_div(int(induced.sum()), int(clean_ok.sum())),
            ...
            "confidence_delta_mean": pd.to_numeric(group["confidence_delta_vs_clean"], errors="coerce").mean(),
        })
    return rows
...
def main() -> None:
    ...
    write_csv(METRICS_DIR / "proxy_model_clean_metrics.csv", grouped_metrics(clean_df, ["evaluated_model"]))
    write_csv(METRICS_DIR / "proxy_model_adversarial_metrics.csv", grouped_metrics(adv_df, ["evaluated_model", "attack_name", "attack_target_model"]))
    write_csv(METRICS_DIR / "proxy_model_anti_forensic_metrics.csv", grouped_metrics(anti_df, ["evaluated_model", "attack_name"]))
    write_csv(METRICS_DIR / "proxy_model_ood_metrics.csv", ood_metrics(df, args.high_confidence_threshold))
    write_csv(METRICS_DIR / "proxy_model_comparative_metrics.csv", comparative_metrics(df))
